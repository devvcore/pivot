...
import type {
  ...
  RevenueRecognition,
  SubscriptionHealth,
  ProductRoadmapHealth,
  TechDebtPrioritization,
  ReleaseVelocity,
  BugTrendAnalysis,
  ApiPerformance,
  UserExperienceScore,
  ...
  MarketTrendRadar,
  IndustryBenchmarkIndex,
  CustomerIntelPlatform,
  PriceSensitivityModel,
  DemandSignalAnalysis,
  DigitalMaturityIndex,
  CloudMigrationReadiness,
  AutomationRoi,
  DigitalWorkplace,
  CybersecurityPosture,
  TechVendorConsolidation,
  ...
  EcosystemMapping,
  AllianceStrategy,
  ChannelPartnerHealth,
  CoSellingPipeline,
  IntegrationMarketplace,
  BrandEquityIndex,
  SentimentDashboard,
  MediaShareOfVoice,
  CrisisCommsReadiness,
  ThoughtLeadership,
  BrandConsistency,
  MonetizationModel,
  FreeTrialConversion,
...
export async function synthesizeRevenueOps(
  packet: BusinessPacket,
  questionnaire: Questionnaire
): Promise<RevenueOps | null> {
  const apiKey = process.env.GEMINI_API_KEY;
  if (!apiKey) return null;
  ...
  const schema = `{
  "summary": "2-3 sentence overview of revenue operations alignment and efficiency",
  "metrics": [{
    "metric": "RevOps metric name",
    "current": "Current value",
    "target": "Target value",
  ...
  "overallAlignment": 0,
  "pipelineAccuracy": "Percentage accuracy of pipeline forecasts",
  "forecastBias": "Description of forecast bias direction and magnitude",
  "topGap": "The single largest gap in revenue operations",
  "recommendations": ["recommendation 1", "..."]
}`;

  const prompt = `You are a revenue operations strategist specializing in aligning sales, marketing, and customer success teams to maximize revenue efficiency, pipeline accuracy, and forecast reliability.
...
Produce a comprehensive Revenue Operations analysis:

1. SUMMARY: 2-3 sentence overview of RevOps alignment across sales, marketing, and CS.
2. METRICS (5-7): For each RevOps metric — metric name, current value, target value, gap, owner, trend.
3. OVERALL ALIGNMENT (0-100): Composite score of cross-functional revenue alignment.
4. PIPELINE ACCURACY: How accurate are pipeline forecasts?
5. FORECAST BIAS: Direction and magnitude of forecast bias.
...
${schema}`;
  ...
  try {
    console.log("[Pivot] Generating Revenue Ops (Wave 37)...");
    const result = await callJson(genai, prompt);
    return result as unknown as RevenueOps;
  } catch (e) {
    console.warn("[Pivot] Revenue Ops (Wave 37) synthesis failed:", e);
    return null;
  }
}

export async function synthesizeBillingOptimization(
  packet: BusinessPacket,
  questionnaire: Questionnaire
): Promise<BillingOptimization | null> {
  const apiKey = process.env.GEMINI_API_KEY;
  if (!apiKey) return null;
  ...
  const schema = `{
  "summary": "2-3 sentence overview of billing health, leakage, and automation gaps",
  "issues": [{
    "issue": "Billing issue description",
    "revenue": "Revenue impacted",
    "frequency": "How often this occurs",
    "rootCause": "Root cause of the issue",
    "fix": "Recommended fix",
    "priority": "High / Medium / Low"
  }],
  "totalLeakage": "Total revenue lost to billing issues",
  "automationRate": "Percentage of billing processes that are automated",
  "errorRate": "Billing error rate",
  "topFix": "Single highest-impact billing fix",
  "recommendations": ["recommendation 1", "..."]
}`;

  const prompt = `You are a billing operations expert specializing in revenue leakage detection, billing automation, error reduction, and payment optimization.
...
Produce a comprehensive Billing Optimization analysis:

1. SUMMARY: 2-3 sentence overview of billing health and the biggest leakage or automation opportunity.
2. ISSUES (4-6): For each billing issue — description, revenue impacted, frequency, root cause, recommended fix, priority.
3. TOTAL LEAKAGE: Total revenue lost to billing errors and inefficiencies.
4. AUTOMATION RATE: Percentage of billing processes currently automated.
5. ERROR RATE: Current billing error rate.
...
${schema}`;
  ...
  try {
    console.log("[Pivot] Generating Billing Optimization (Wave 37)...");
    const result = await callJson(genai, prompt);
    return result as unknown as BillingOptimization;
  } catch (e) {
    console.warn("[Pivot] Billing Optimization (Wave 37) synthesis failed:", e);
    return null;
  }
}

export async function synthesizeContractIntelligence(
  packet: BusinessPacket,
  questionnaire: Questionnaire
  ...
  const schema = `{
  "summary": "2-3 sentence overview of contract portfolio health and renewal pipeline",
  "contracts": [{
    "contract": "Contract name or identifier",
    "value": "Contract value",
    "renewalDate": "Renewal or expiration date",
    "risk": "Risk assessment for this contract",
    "opportunity": "Upsell or renegotiation opportunity",
    "action": "Recommended next action"
  }],
  "totalValue": "Total value of all contracts analyzed",
  "renewalPipeline": "Value of contracts up for renewal in the next cycle",
  "atRiskRevenue": "Revenue at risk from contract churn or downgrades",
  "autoRenewalRate": "Percentage of contracts with auto-renewal clauses",
  "recommendations": ["recommendation 1", "..."]
}`;

  const prompt = `You are a contract intelligence analyst specializing in contract lifecycle management, renewal optimization, risk identification, and revenue protection.
...
Produce a comprehensive Contract Intelligence analysis:

1. SUMMARY: 2-3 sentence overview of contract portfolio health and renewal outlook.
2. CONTRACTS (4-6): For each key contract — name, value, renewal date, risk, opportunity, recommended action.
3. TOTAL VALUE: Total value of all contracts in the portfolio.
4. RENEWAL PIPELINE: Value of contracts coming up for renewal.
5. AT-RISK REVENUE: Revenue at risk from contract churn or downgrades.
...
${schema}`;
  ...
  try {
    console.log("[Pivot] Generating Contract Intelligence (Wave 37)...");
    const result = await callJson(genai, prompt);
    return result as unknown as ContractIntelligence;
  } catch (e) {
    console.warn("[Pivot] Contract Intelligence (Wave 37) synthesis failed:", e);
    return null;
  }
}

export async function synthesizeCommissionTracking(
  packet: BusinessPacket,
  questionnaire: Questionnaire
): Promise<CommissionTracking | null> {
  const apiKey = process.env.GEMINI_API_KEY;
  if (!apiKey) return null;
  ...
  const schema = `{
  "summary": "2-3 sentence overview of commission structure effectiveness and cost efficiency",
  "plans": [{
    "role": "Sales role or team",
    "structure": "Commission structure (flat, tiered, accelerator, etc.)",
    "quota": "Quota target",
    "attainment": "Average quota attainment",
    "effectiveness": "Plan effectiveness assessment",
    "cost": "Cost of commissions for this role"
  }],
  "totalCommissions": "Total commission spend",
  "avgAttainment": "Average quota attainment across all roles",
  "costOfSales": "Commission cost as percentage of revenue",
  "topPerformer": "Highest-performing commission plan and why",
  "recommendations": ["recommendation 1", "..."]
}`;

  const prompt = `You are a sales compensation analyst specializing in commission plan design, quota attainment analysis, cost-of-sales optimization, and incentive alignment.
...
Produce a comprehensive Commission Tracking analysis:

1. SUMMARY: 2-3 sentence overview of commission structure health and cost efficiency.
2. PLANS (4-6): For each commission plan — role, structure type, quota, attainment, effectiveness, cost.
3. TOTAL COMMISSIONS: Total commission spend across all plans.
4. AVG ATTAINMENT: Average quota attainment across all roles.
5. COST OF SALES: Commission cost as a percentage of revenue.
...
${schema}`;
  ...
  try {
    console.log("[Pivot] Generating Commission Tracking (Wave 37)...");
    const result = await callJson(genai, prompt);
    return result as unknown as CommissionTracking;
  } catch (e) {
    console.warn("[Pivot] Commission Tracking (Wave 37) synthesis failed:", e);
    return null;
  }
}

export async function synthesizeRevenueRecognition(
  packet: BusinessPacket,
  questionnaire: Questionnaire
  ...
  const schema = `{
  "summary": "2-3 sentence overview of revenue recognition practices and compliance posture",
  "rules": [{
    "stream": "Revenue stream name",
    "recognitionMethod": "Recognition method (point-in-time, over time, milestone, etc.)",
    "deferredRevenue": "Amount of deferred revenue for this stream",
    "compliance": "Compliance status (ASC 606 / IFRS 15)",
    "risk": "Risk assessment",
    "action": "Recommended action"
  }],
  "deferredTotal": "Total deferred revenue across all streams",
  "recognizedTotal": "Total recognized revenue in current period",
  "complianceScore": "Overall compliance score or rating",
  "topRisk": "Single biggest revenue recognition risk",
  "recommendations": ["recommendation 1", "..."]
}`;

  const prompt = `You are a revenue recognition specialist with deep expertise in ASC 606 / IFRS 15 compliance, deferred revenue management, and accounting standards for multi-element arrangements.
...
Produce a comprehensive Revenue Recognition analysis:

1. SUMMARY: 2-3 sentence overview of revenue recognition practices and compliance.
2. RULES (4-6): For each revenue stream — stream name, recognition method, deferred revenue, compliance status, risk, recommended action.
3. DEFERRED TOTAL: Total deferred revenue across all streams.
4. RECOGNIZED TOTAL: Total recognized revenue in the current period.
5. COMPLIANCE SCORE: Overall compliance score or rating.
...
${schema}`;
  ...
  try {
    console.log("[Pivot] Generating Revenue Recognition (Wave 37)...");
    const result = await callJson(genai, prompt);
    return result as unknown as RevenueRecognition;
  } catch (e) {
    console.warn("[Pivot] Revenue Recognition (Wave 37) synthesis failed:", e);
    return null;
  }
}

export async function synthesizeSubscriptionHealth(
  packet: BusinessPacket,
  questionnaire: Questionnaire
  ...
  const schema = `{
  "summary": "2-3 sentence overview of subscription business health and growth trajectory",
  "metrics": [{
    "metric": "Subscription metric name",
    "value": "Current value",
    "benchmark": "Industry benchmark",
    "trend": "Improving / Stable / Declining",
    "segment": "Customer segment this applies to",
    "action": "Recommended action"
  }],
  "overallScore": 0,
  "mrrGrowth": "Month-over-month MRR growth rate",
  "churnRate": "Monthly churn rate",
  "expansionRate": "Net expansion revenue rate",
  "recommendations": ["recommendation 1", "..."]
}`;

  const prompt = `You are a subscription business analyst specializing in SaaS metrics, MRR/ARR growth analysis, churn diagnostics, and expansion revenue optimization.
...
Produce a comprehensive Subscription Health analysis:

1. SUMMARY: 2-3 sentence overview of subscription health and growth trajectory.
2. METRICS (5-7): For each subscription metric — metric name, current value, benchmark, trend, segment, recommended action.
3. OVERALL SCORE (0-100): Composite subscription health score.
4. MRR GROWTH: Month-over-month MRR growth rate.
5. CHURN RATE: Monthly churn rate.
...
${schema}`;
  ...
  try {
    console.log("[Pivot] Generating Subscription Health (Wave 37)...");
    const result = await callJson(genai, prompt);
    return result as unknown as SubscriptionHealth;
  } catch (e) {
    console.warn("[Pivot] Subscription Health (Wave 37) synthesis failed:", e);
    return null;
  }
}
...
export async function synthesizeProductRoadmapHealth(
  packet: BusinessPacket,
  questionnaire: Questionnaire
): Promise<ProductRoadmapHealth | null> {
  const apiKey = process.env.GEMINI_API_KEY;
  if (!apiKey) return null;
  ...
  const schema = `{
  "summary": "2-3 sentence overview of product roadmap health and execution risk",
  "items": [{
    "feature": "Feature or initiative name",
    "quarter": "Target quarter (e.g., Q2 2025)",
    "status": "On Track / At Risk / Delayed / Completed",
    "alignment": "Strategic alignment assessment",
    "impact": "Expected business impact",
    "risk": "Key risk for this item"
  }],
  "overallHealth": 0,
  "onTrackPercentage": "Percentage of roadmap items on track",
  "topRisk": "Single biggest roadmap execution risk",
  "stakeholderAlignment": "Assessment of stakeholder alignment on roadmap priorities",
  "recommendations": ["recommendation 1", "..."]
}`;

  const prompt = `You are a product strategy consultant specializing in roadmap health assessment, feature prioritization, execution risk management, and stakeholder alignment.
...
Produce a comprehensive Product Roadmap Health analysis:

1. SUMMARY: 2-3 sentence overview of roadmap health and biggest execution risk.
2. ITEMS (5-7): For each roadmap item — feature name, target quarter, status, strategic alignment, expected impact, key risk.
3. OVERALL HEALTH (0-100): Composite roadmap health score.
4. ON-TRACK PERCENTAGE: Percentage of roadmap items currently on track.
5. TOP RISK: The single biggest risk to roadmap execution.
...
${schema}`;
  ...
  try {
    console.log("[Pivot] Generating Product Roadmap Health (Wave 38)...");
    const result = await callJson(genai, prompt);
    return result as unknown as ProductRoadmapHealth;
  } catch (e) {
    console.warn("[Pivot] Product Roadmap Health (Wave 38) synthesis failed:", e);
    return null;
  }
}

export async function synthesizeTechDebtPrioritization(
  packet: BusinessPacket,
  questionnaire: Questionnaire
): Promise<TechDebtPrioritization | null> {
  const apiKey = process.env.GEMINI_API_KEY;
  if (!apiKey) return null;
  ...
  const schema = `{
  "summary": "2-3 sentence overview of technical debt burden and prioritization strategy",
  "items": [{
    "area": "Codebase area or system component",
    "severity": "Critical / High / Medium / Low",
    "impact": "Business impact of this debt",
    "effort": "Estimated effort to resolve",
    "roi": "Return on investment of fixing this debt",
    "priority": "Priority ranking"
  }],
  "totalDebt": "Estimated total cost of technical debt",
  "criticalItems": 0,
  "avgAge": "Average age of technical debt items",
  "velocityImpact": "How much tech debt is slowing development velocity",
  "recommendations": ["recommendation 1", "..."]
}`;

  const prompt = `You are a software architecture consultant specializing in technical debt assessment, codebase health analysis, refactoring prioritization, and engineering velocity optimization.
...
Produce a comprehensive Tech Debt Prioritization analysis:

1. SUMMARY: 2-3 sentence overview of technical debt burden and its impact on velocity.
2. ITEMS (5-7): For each tech debt item — area, severity, business impact, effort to fix, ROI, priority.
3. TOTAL DEBT: Estimated total cost of accumulated technical debt.
4. CRITICAL ITEMS: Number of critical-severity debt items requiring immediate attention.
5. AVG AGE: Average age of technical debt items.
...
${schema}`;
  ...
  try {
    console.log("[Pivot] Generating Tech Debt Prioritization (Wave 38)...");
    const result = await callJson(genai, prompt);
    return result as unknown as TechDebtPrioritization;
  } catch (e) {
    console.warn("[Pivot] Tech Debt Prioritization (Wave 38) synthesis failed:", e);
    return null;
  }
}

export async function synthesizeReleaseVelocity(
  packet: BusinessPacket,
  questionnaire: Questionnaire
): Promise<ReleaseVelocity | null> {
  const apiKey = process.env.GEMINI_API_KEY;
  if (!apiKey) return null;
  ...
  const schema = `{
  "summary": "2-3 sentence overview of release velocity, DORA metrics, and deployment health",
  "metrics": [{
    "metric": "Release metric name",
    "current": "Current value",
  ...
  "avgCycleTime": "Average cycle time from commit to production",
  "deployFrequency": "Deployment frequency (daily, weekly, etc.)",
  "changeFailRate": "Percentage of deployments causing failures",
  "mttr": "Mean time to recovery from failures",
  "recommendations": ["recommendation 1", "..."]
}`;

  const prompt = `You are a DevOps and engineering effectiveness consultant specializing in DORA metrics, release management, CI/CD optimization, and deployment reliability.
...
Produce a comprehensive Release Velocity analysis:

1. SUMMARY: 2-3 sentence overview of release velocity and deployment health.
2. METRICS (5-7): For each release metric — metric name, current value, previous period value, trend, benchmark, recommended action.
3. AVG CYCLE TIME: Average time from code commit to production deployment.
4. DEPLOY FREQUENCY: How often the team deploys to production.
5. CHANGE FAIL RATE: Percentage of deployments that cause failures.
...
${schema}`;
  ...
  try {
    console.log("[Pivot] Generating Release Velocity (Wave 38)...");
    const result = await callJson(genai, prompt);
    return result as unknown as ReleaseVelocity;
  } catch (e) {
    console.warn("[Pivot] Release Velocity (Wave 38) synthesis failed:", e);
    return null;
  }
}

export async function synthesizeBugTrendAnalysis(
  packet: BusinessPacket,
  questionnaire: Questionnaire
): Promise<BugTrendAnalysis | null> {
  const apiKey = process.env.GEMINI_API_KEY;
  if (!apiKey) return null;
  ...
  const schema = `{
  "summary": "2-3 sentence overview of bug trends, quality trajectory, and critical issues",
  "trends": [{
    "category": "Bug category (UI, backend, security, performance, etc.)",
    "openCount": 0,
    "closedCount": 0,
    "trend": "Increasing / Stable / Decreasing",
    "severity": "Critical / High / Medium / Low distribution",
    "avgResolutionTime": "Average time to resolve bugs in this category"
  }],
  "totalOpen": 0,
  "criticalBugs": 0,
  "resolutionRate": "Percentage of bugs resolved within SLA",
  "qualityTrend": "Overall quality trajectory assessment",
  "recommendations": ["recommendation 1", "..."]
}`;

  const prompt = `You are a software quality analyst specializing in bug trend analysis, defect pattern recognition, quality metrics tracking, and resolution process optimization.
...
Produce a comprehensive Bug Trend Analysis:

1. SUMMARY: 2-3 sentence overview of bug trends and quality trajectory.
2. TRENDS (5-7): For each bug category — category name, open count, closed count, trend direction, severity distribution, average resolution time.
3. TOTAL OPEN: Total number of currently open bugs.
4. CRITICAL BUGS: Number of critical-severity bugs currently open.
5. RESOLUTION RATE: Percentage of bugs resolved within SLA.
...
${schema}`;
  ...
  try {
    console.log("[Pivot] Generating Bug Trend Analysis (Wave 38)...");
    const result = await callJson(genai, prompt);
    return result as unknown as BugTrendAnalysis;
  } catch (e) {
    console.warn("[Pivot] Bug Trend Analysis (Wave 38) synthesis failed:", e);
    return null;
  }
}

export async function synthesizeApiPerformance(
  packet: BusinessPacket,
  questionnaire: Questionnaire
): Promise<ApiPerformance | null> {
  const apiKey = process.env.GEMINI_API_KEY;
  if (!apiKey) return null;
  ...
  const schema = `{
  "summary": "2-3 sentence overview of API performance, reliability, and bottlenecks",
  "endpoints": [{
    "endpoint": "API endpoint or service name",
    "latency": "Average response latency",
    "errorRate": "Error rate for this endpoint",
    "throughput": "Requests per second or minute",
    "availability": "Uptime percentage",
    "trend": "Improving / Stable / Degrading"
  }],
  "overallAvailability": "System-wide API availability percentage",
  "avgLatency": "System-wide average latency",
  "errorBudget": "Remaining error budget status",
  "topBottleneck": "Single biggest API performance bottleneck",
  "recommendations": ["recommendation 1", "..."]
}`;

  const prompt = `You are an API reliability engineer specializing in performance monitoring, latency optimization, error budget management, and service-level objective tracking.
...
Produce a comprehensive API Performance analysis:

1. SUMMARY: 2-3 sentence overview of API performance health and reliability.
2. ENDPOINTS (5-7): For each key endpoint — endpoint name, latency, error rate, throughput, availability, trend.
3. OVERALL AVAILABILITY: System-wide API availability percentage.
4. AVG LATENCY: System-wide average response latency.
5. ERROR BUDGET: Current error budget status and remaining margin.
...
${schema}`;
  ...
  try {
    console.log("[Pivot] Generating API Performance (Wave 38)...");
    const result = await callJson(genai, prompt);
    return result as unknown as ApiPerformance;
  } catch (e) {
    console.warn("[Pivot] API Performance (Wave 38) synthesis failed:", e);
    return null;
  }
}

export async function synthesizeUserExperienceScore(
  packet: BusinessPacket,
  questionnaire: Questionnaire
): Promise<UserExperienceScore | null> {
  const apiKey = process.env.GEMINI_API_KEY;
  if (!apiKey) return null;
  ...
  const schema = `{
  "summary": "2-3 sentence overview of user experience quality and key friction points",
  "dimensions": [{
    "dimension": "UX dimension (usability, accessibility, performance, aesthetics, etc.)",
    "score": 0,
    "benchmark": 0,
    "topIssue": "Top issue in this dimension",
    "improvement": "Recommended improvement",
    "impact": "Expected business impact of improvement"
  }],
  "overallScore": 0,
  "nps": "Net Promoter Score or assessment",
  "taskCompletionRate": "Percentage of users completing key tasks",
  "topFriction": "Single biggest UX friction point",
  "recommendations": ["recommendation 1", "..."]
}`;

  const prompt = `You are a UX research and design strategist specializing in user experience measurement, usability assessment, friction analysis, and experience optimization.
...
Produce a comprehensive User Experience Score analysis:

1. SUMMARY: 2-3 sentence overview of UX quality and the most impactful friction point.
2. DIMENSIONS (5-7): For each UX dimension — dimension name, score (0-100), benchmark, top issue, recommended improvement, expected business impact.
3. OVERALL SCORE (0-100): Composite user experience score.
4. NPS: Net Promoter Score or qualitative assessment.
5. TASK COMPLETION RATE: Percentage of users completing key tasks successfully.
...
${schema}`;
  ...
  try {
    console.log("[Pivot] Generating User Experience Score (Wave 38)...");
    const result = await callJson(genai, prompt);
    return result as unknown as UserExperienceScore;
  } catch (e) {
    console.warn("[Pivot] User Experience Score (Wave 38) synthesis failed:", e);
    return null;
  }
}
...
export async function synthesizeWorkforcePlanning(
  packet: BusinessPacket,
  questionnaire: Questionnaire
): Promise<WorkforcePlanning | null> {
  const apiKey = process.env.GEMINI_API_KEY;
  if (!apiKey) return null;
  ...
  const schema = `{
  "summary": "2-3 sentence overview of workforce planning gaps and hiring strategy",
  "segments": [{
    "segment": "Workforce segment (engineering, sales, operations, etc.)",
    "currentHeadcount": 0,
    "plannedHeadcount": 0,
    "gap": 0,
    "timeline": "Hiring timeline for this segment",
    "cost": "Estimated cost to fill this gap"
  }],
  "totalGap": 0,
  "hiringBudget": "Total hiring budget needed",
  "attritionRate": "Current attrition rate",
  "timeToFill": "Average time to fill open positions",
  "recommendations": ["recommendation 1", "..."]
}`;

  const prompt = `You are a workforce planning strategist specializing in headcount planning, talent gap analysis, hiring budget optimization, and organizational design.
...
Produce a comprehensive Workforce Planning analysis:

1. SUMMARY: 2-3 sentence overview of workforce planning gaps and hiring priorities.
2. SEGMENTS (5-7): For each workforce segment — segment name, current headcount, planned headcount, gap, hiring timeline, estimated cost.
3. TOTAL GAP: Total headcount gap across all segments.
4. HIRING BUDGET: Total budget needed to close all gaps.
5. ATTRITION RATE: Current employee attrition rate.
...
${schema}`;
  ...
  try {
    console.log("[Pivot] Generating Workforce Planning (Wave 39)...");
    const result = await callJson(genai, prompt);
    return result as unknown as WorkforcePlanning;
  } catch (e) {
    console.warn("[Pivot] Workforce Planning (Wave 39) synthesis failed:", e);
    return null;
  }
}

export async function synthesizeSkillsGapAnalysis(
  packet: BusinessPacket,
  questionnaire: Questionnaire
  ...
  const schema = `{
  "summary": "2-3 sentence overview of critical skills gaps and training priorities",
  "gaps": [{
    "skill": "Skill name",
    "currentLevel": "Current proficiency level",
    ...
    "gap": "Gap assessment",
    "affectedRoles": 0,
    "trainingOption": "Recommended training or upskilling option"
  }],
  "criticalGaps": 0,
  "trainingBudget": "Estimated training budget needed",
  "topPriority": "Single highest-priority skill to develop",
  "readinessScore": 0,
  "recommendations": ["recommendation 1", "..."]
}`;

  const prompt = `You are a talent development consultant specializing in skills gap analysis, competency framework design, training needs assessment, and workforce upskilling strategy.
...
Produce a comprehensive Skills Gap Analysis:

1. SUMMARY: 2-3 sentence overview of critical skills gaps and organizational readiness.
2. GAPS (5-7): For each skills gap — skill name, current level, required level, gap assessment, number of affected roles, training option.
3. CRITICAL GAPS: Number of skills gaps rated critical.
4. TRAINING BUDGET: Estimated budget needed for training and upskilling.
5. TOP PRIORITY: The single highest-priority skill to develop first.
...
${schema}`;
  ...
  try {
    console.log("[Pivot] Generating Skills Gap Analysis (Wave 39)...");
    const result = await callJson(genai, prompt);
    return result as unknown as SkillsGapAnalysis;
  } catch (e) {
    console.warn("[Pivot] Skills Gap Analysis (Wave 39) synthesis failed:", e);
    return null;
  }
}

export async function synthesizeRemoteWorkEffectiveness(
  packet: BusinessPacket,
  questionnaire: Questionnaire
): Promise<RemoteWorkEffectiveness | null> {
  const apiKey = process.env.GEMINI_API_KEY;
  if (!apiKey) return null;
  ...
  const schema = `{
  "summary": "2-3 sentence overview of remote work effectiveness and collaboration health",
  "metrics": [{
    "metric": "Remote work metric name",
    "remoteScore": "Score or value for remote workers",
    "officeScore": "Score or value for office workers",
    "difference": "Delta between remote and office",
    "trend": "Improving / Stable / Declining",
    "action": "Recommended action"
  }],
  "overallEffectiveness": 0,
  "collaborationScore": "Cross-team collaboration quality score",
  "productivityIndex": "Remote productivity index vs. office baseline",
  "employeeSatisfaction": "Remote employee satisfaction rating",
  "recommendations": ["recommendation 1", "..."]
}`;

  const prompt = `You are a remote work strategist specializing in distributed team effectiveness, hybrid work models, remote collaboration optimization, and employee engagement in remote settings.
...
Produce a comprehensive Remote Work Effectiveness analysis:

1. SUMMARY: 2-3 sentence overview of remote work effectiveness and key challenges.
2. METRICS (5-7): For each metric — name, remote score, office score, difference, trend, recommended action.
3. OVERALL EFFECTIVENESS (0-100): Composite remote work effectiveness score.
4. COLLABORATION SCORE: Assessment of cross-team collaboration quality.
5. PRODUCTIVITY INDEX: Remote productivity compared to office baseline.
...
${schema}`;
  ...
  try {
    console.log("[Pivot] Generating Remote Work Effectiveness (Wave 39)...");
    const result = await callJson(genai, prompt);
    return result as unknown as RemoteWorkEffectiveness;
  } catch (e) {
    console.warn("[Pivot] Remote Work Effectiveness (Wave 39) synthesis failed:", e);
    return null;
  }
}

export async function synthesizeTeamVelocity(
  packet: BusinessPacket,
  questionnaire: Questionnaire
): Promise<TeamVelocity | null> {
  const apiKey = process.env.GEMINI_API_KEY;
  if (!apiKey) return null;
  ...
  const schema = `{
  "summary": "2-3 sentence overview of team velocity, sprint health, and blockers",
  "teams": [{
    "team": "Team name",
    "velocity": "Sprint velocity (story points or tasks completed)",
    "capacity": "Team capacity",
    "utilization": "Utilization percentage",
    "trend": "Improving / Stable / Declining",
    "bottleneck": "Primary bottleneck for this team"
  }],
  "avgVelocity": "Average velocity across all teams",
  "sprintCompletion": "Average sprint completion rate",
  "topBlocker": "Single biggest blocker slowing teams",
  "improvementRate": "Rate of velocity improvement over recent sprints",
  "recommendations": ["recommendation 1", "..."]
}`;

  const prompt = `You are an agile delivery consultant specializing in team velocity analysis, sprint health diagnostics, capacity planning, and engineering throughput optimization.
...
Produce a comprehensive Team Velocity analysis:

1. SUMMARY: 2-3 sentence overview of team velocity health and key blockers.
2. TEAMS (4-6): For each team — team name, velocity, capacity, utilization, trend, primary bottleneck.
3. AVG VELOCITY: Average velocity across all teams.
4. SPRINT COMPLETION: Average sprint completion rate.
5. TOP BLOCKER: The single biggest blocker slowing team velocity.
...
${schema}`;
  ...
  try {
    console.log("[Pivot] Generating Team Velocity (Wave 39)...");
    const result = await callJson(genai, prompt);
    return result as unknown as TeamVelocity;
  } catch (e) {
    console.warn("[Pivot] Team Velocity (Wave 39) synthesis failed:", e);
    return null;
  }
}

export async function synthesizeBurnoutRisk(
  packet: BusinessPacket,
  questionnaire: Questionnaire
): Promise<BurnoutRisk | null> {
  const apiKey = process.env.GEMINI_API_KEY;
  if (!apiKey) return null;
  ...
  const schema = `{
  "summary": "2-3 sentence overview of burnout risk across the organization",
  "indicators": [{
    "indicator": "Burnout indicator name",
    "score": 0,
    "threshold": 0,
    "affectedTeams": "Teams most affected",
    "trend": "Increasing / Stable / Decreasing",
    "intervention": "Recommended intervention"
  }],
  "overallRisk": 0,
  "highRiskTeams": 0,
  "avgWorkload": "Average workload assessment across teams",
  "topCause": "Single biggest driver of burnout risk",
  "recommendations": ["recommendation 1", "..."]
}`;

  const prompt = `You are an organizational psychologist specializing in employee burnout detection, workload analysis, well-being program design, and sustainable performance management.
...
Produce a comprehensive Burnout Risk analysis:

1. SUMMARY: 2-3 sentence overview of burnout risk and organizational well-being.
2. INDICATORS (5-7): For each burnout indicator — indicator name, score (0-100), threshold, affected teams, trend, recommended intervention.
3. OVERALL RISK (0-100): Composite burnout risk score (higher = more risk).
4. HIGH-RISK TEAMS: Number of teams in the high-risk zone.
5. AVG WORKLOAD: Average workload assessment across the organization.
...
${schema}`;
  ...
  try {
    console.log("[Pivot] Generating Burnout Risk (Wave 39)...");
    const result = await callJson(genai, prompt);
    return result as unknown as BurnoutRisk;
  } catch (e) {
    console.warn("[Pivot] Burnout Risk (Wave 39) synthesis failed:", e);
    return null;
  }
}

export async function synthesizeLearningDevelopment(
  packet: BusinessPacket,
  questionnaire: Questionnaire
): Promise<LearningDevelopment | null> {
  const apiKey = process.env.GEMINI_API_KEY;
  if (!apiKey) return null;
  ...
  const schema = `{
  "summary": "2-3 sentence overview of learning and development effectiveness",
  "programs": [{
    "program": "Program name",
    "participants": 0,
    "completionRate": "Completion rate percentage",
    "impact": "Measured impact on performance",
    "cost": "Program cost",
    "roi": "Return on investment"
  }],
  "totalInvestment": "Total L&D investment",
  "avgHoursPerEmployee": "Average training hours per employee",
  "skillsImproved": 0,
  "topProgram": "Highest-impact L&D program",
  "recommendations": ["recommendation 1", "..."]
}`;

  const prompt = `You are a learning and development strategist specializing in corporate training effectiveness, skills development ROI, employee growth programs, and organizational learning culture.
...
Produce a comprehensive Learning & Development analysis:

1. SUMMARY: 2-3 sentence overview of L&D effectiveness and investment returns.
2. PROGRAMS (4-6): For each program — program name, participant count, completion rate, measured impact, cost, ROI.
3. TOTAL INVESTMENT: Total investment in learning and development.
4. AVG HOURS PER EMPLOYEE: Average training hours per employee per year.
5. SKILLS IMPROVED: Number of key skills measurably improved through L&D.
...
${schema}`;
  ...
  try {
    console.log("[Pivot] Generating Learning & Development (Wave 39)...");
    const result = await callJson(genai, prompt);
    return result as unknown as LearningDevelopment;
  } catch (e) {
    console.warn("[Pivot] Learning & Development (Wave 39) synthesis failed:", e);
    return null;
  }
}
...
export async function synthesizeRegulatoryRisk(
  packet: BusinessPacket,
  questionnaire: Questionnaire
): Promise<RegulatoryRisk | null> {
  const apiKey = process.env.GEMINI_API_KEY;
  if (!apiKey) return null;
  ...
  const schema = `{
  "summary": "2-3 sentence overview of regulatory risk exposure and preparedness",
  "items": [{
    "regulation": "Regulation name (e.g., GDPR, SOX, HIPAA)",
    "jurisdiction": "Applicable jurisdiction",
    "complianceStatus": "Compliant / Partially Compliant / Non-Compliant",
    "deadline": "Key compliance deadline",
    "risk": "Risk assessment",
    "action": "Recommended action"
  }],
  "overallExposure": "Overall regulatory risk exposure level",
  "criticalDeadlines": 0,
  "fineRisk": "Estimated financial exposure from non-compliance",
  "preparednessScore": 0,
  "recommendations": ["recommendation 1", "..."]
}`;

  const prompt = `You are a regulatory compliance expert specializing in multi-jurisdiction regulatory risk assessment, compliance program design, and regulatory change management.
...
Produce a comprehensive Regulatory Risk analysis:

1. SUMMARY: 2-3 sentence overview of regulatory risk exposure and compliance posture.
2. ITEMS (5-7): For each regulation — regulation name, jurisdiction, compliance status, deadline, risk, recommended action.
3. OVERALL EXPOSURE: Overall regulatory risk exposure level.
4. CRITICAL DEADLINES: Number of critical compliance deadlines in the near term.
5. FINE RISK: Estimated financial exposure from potential non-compliance.
...
${schema}`;
  ...
  try {
    console.log("[Pivot] Generating Regulatory Risk (Wave 40)...");
    const result = await callJson(genai, prompt);
    return result as unknown as RegulatoryRisk;
  } catch (e) {
    console.warn("[Pivot] Regulatory Risk (Wave 40) synthesis failed:", e);
    return null;
  }
}

export async function synthesizeContractManagement(
  packet: BusinessPacket,
  questionnaire: Questionnaire
): Promise<ContractManagement | null> {
  const apiKey = process.env.GEMINI_API_KEY;
  if (!apiKey) return null;
  ...
  const schema = `{
  "summary": "2-3 sentence overview of contract management health and risk",
  "contracts": [{
    "contract": "Contract name or identifier",
    "counterparty": "Counterparty name",
    "value": "Contract value",
    "status": "Active / Expiring / Under Negotiation / Terminated",
    "renewalDate": "Renewal or expiration date",
    "risk": "Risk assessment for this contract"
  }],
  "totalValue": "Total value of managed contracts",
  "expiringNext90": 0,
  "autoRenewalPercentage": "Percentage of contracts with auto-renewal",
  "topRisk": "Single biggest contract management risk",
  "recommendations": ["recommendation 1", "..."]
}`;

  const prompt = `You are a contract management specialist with expertise in contract lifecycle optimization, counterparty risk assessment, renewal management, and terms negotiation strategy.
...
Produce a comprehensive Contract Management analysis:

1. SUMMARY: 2-3 sentence overview of contract portfolio health and risk exposure.
2. CONTRACTS (5-7): For each key contract — contract name, counterparty, value, status, renewal date, risk.
3. TOTAL VALUE: Total value of all managed contracts.
4. EXPIRING NEXT 90: Number of contracts expiring in the next 90 days.
5. AUTO-RENEWAL PERCENTAGE: Percentage of contracts with auto-renewal clauses.
...
${schema}`;
  ...
  try {
    console.log("[Pivot] Generating Contract Management (Wave 40)...");
    const result = await callJson(genai, prompt);
    return result as unknown as ContractManagement;
  } catch (e) {
    console.warn("[Pivot] Contract Management (Wave 40) synthesis failed:", e);
    return null;
  }
}

export async function synthesizeIpStrategy(
  packet: BusinessPacket,
  questionnaire: Questionnaire
): Promise<IpStrategy | null> {
  const apiKey = process.env.GEMINI_API_KEY;
  if (!apiKey) return null;
  ...
  const schema = `{
  "summary": "2-3 sentence overview of intellectual property portfolio and strategy",
  "assets": [{
    "asset": "IP asset name",
    "type": "Patent / Trademark / Copyright / Trade Secret",
    "status": "Active / Pending / Expired",
    "jurisdiction": "Jurisdiction of protection",
    "expirationDate": "Expiration or renewal date",
    "value": "Estimated value of this asset"
  }],
  "portfolioValue": "Total estimated IP portfolio value",
  "pendingApplications": 0,
  "expiringProtections": 0,
  "competitiveAdvantage": "Assessment of IP-driven competitive advantage",
  "recommendations": ["recommendation 1", "..."]
}`;

  const prompt = `You are an intellectual property strategist specializing in IP portfolio management, patent strategy, trademark protection, trade secret governance, and competitive IP positioning.
...
Produce a comprehensive IP Strategy analysis:

1. SUMMARY: 2-3 sentence overview of IP portfolio health and strategic positioning.
2. ASSETS (4-6): For each IP asset — asset name, type, status, jurisdiction, expiration date, estimated value.
3. PORTFOLIO VALUE: Total estimated value of the IP portfolio.
4. PENDING APPLICATIONS: Number of IP applications currently pending.
5. EXPIRING PROTECTIONS: Number of IP protections expiring soon.
...
${schema}`;
  ...
  try {
    console.log("[Pivot] Generating IP Strategy (Wave 40)...");
    const result = await callJson(genai, prompt);
    return result as unknown as IpStrategy;
  } catch (e) {
    console.warn("[Pivot] IP Strategy (Wave 40) synthesis failed:", e);
    return null;
  }
}

export async function synthesizeLegalSpendAnalysis(
  packet: BusinessPacket,
  questionnaire: Questionnaire
): Promise<LegalSpendAnalysis | null> {
  const apiKey = process.env.GEMINI_API_KEY;
  if (!apiKey) return null;
  ...
  const schema = `{
  "summary": "2-3 sentence overview of legal spending efficiency and optimization opportunities",
  "items": [{
    "category": "Legal spend category (litigation, contracts, regulatory, employment, IP, etc.)",
  ...
  "totalSpend": "Total legal spend",
  "budgetVariance": "Overall budget variance",
  "costPerMatter": "Average cost per legal matter",
  "topCategory": "Highest spend category",
  "recommendations": ["recommendation 1", "..."]
}`;

  const prompt = `You are a legal operations analyst specializing in legal spend management, outside counsel optimization, budget forecasting, and legal department efficiency.
...
Produce a comprehensive Legal Spend Analysis:

1. SUMMARY: 2-3 sentence overview of legal spending patterns and efficiency.
2. ITEMS (5-7): For each legal spend category — category name, actual spend, budget, variance, trend, optimization opportunity.
3. TOTAL SPEND: Total legal department spend.
4. BUDGET VARIANCE: Overall variance from legal budget.
5. COST PER MATTER: Average cost per legal matter.
...
${schema}`;
  ...
  try {
    console.log("[Pivot] Generating Legal Spend Analysis (Wave 40)...");
    const result = await callJson(genai, prompt);
    return result as unknown as LegalSpendAnalysis;
  } catch (e) {
    console.warn("[Pivot] Legal Spend Analysis (Wave 40) synthesis failed:", e);
    return null;
  }
}

export async function synthesizePolicyCompliance(
  packet: BusinessPacket,
  questionnaire: Questionnaire
): Promise<PolicyCompliance | null> {
  const apiKey = process.env.GEMINI_API_KEY;
  if (!apiKey) return null;
  ...
  const schema = `{
  "summary": "2-3 sentence overview of internal policy compliance posture",
  "areas": [{
    "area": "Policy area (data privacy, information security, HR, finance, operations, etc.)",
    "status": "Compliant / Partially Compliant / Non-Compliant",
    "lastReview": "Date of last policy review",
    "gapCount": 0,
    "risk": "Risk assessment for this area",
    "nextAction": "Recommended next action"
  }],
  "overallCompliance": "Overall compliance percentage or rating",
  "policiesReviewed": 0,
  "gaps": 0,
  "topPriority": "Single highest-priority compliance gap",
  "recommendations": ["recommendation 1", "..."]
}`;

  const prompt = `You are a corporate compliance officer specializing in internal policy governance, compliance gap analysis, policy lifecycle management, and organizational compliance culture.
...
Produce a comprehensive Policy Compliance analysis:

1. SUMMARY: 2-3 sentence overview of internal policy compliance health.
2. AREAS (5-7): For each policy area — area name, compliance status, last review date, gap count, risk, recommended next action.
3. OVERALL COMPLIANCE: Overall compliance rating or percentage.
4. POLICIES REVIEWED: Number of policies reviewed in the assessment.
5. GAPS: Total number of compliance gaps identified.
...
${schema}`;
  ...
  try {
    console.log("[Pivot] Generating Policy Compliance (Wave 40)...");
    const result = await callJson(genai, prompt);
    return result as unknown as PolicyCompliance;
  } catch (e) {
    console.warn("[Pivot] Policy Compliance (Wave 40) synthesis failed:", e);
    return null;
  }
}

export async function synthesizeAuditReadiness(
  packet: BusinessPacket,
  questionnaire: Questionnaire
  ...
  const schema = `{
  "summary": "2-3 sentence overview of audit readiness and outstanding findings",
  "areas": [{
    "area": "Audit area (financial, operational, IT, compliance, etc.)",
    "readiness": "Ready / Partially Ready / Not Ready",
    "lastAudit": "Date of last audit",
    "findings": 0,
    "resolvedFindings": 0,
    "risk": "Risk assessment for this area"
  }],
  ...
  "openFindings": 0,
  "criticalGaps": 0,
  "nextAuditDate": "Date of next scheduled audit",
  "recommendations": ["recommendation 1", "..."]
}`;

  const prompt = `You are an audit readiness consultant specializing in internal audit preparation, finding remediation, control testing, and audit program management across financial, operational, and IT domains.
...
Produce a comprehensive Audit Readiness analysis:

1. SUMMARY: 2-3 sentence overview of audit readiness and critical gaps.
2. AREAS (5-7): For each audit area — area name, readiness status, last audit date, total findings, resolved findings, risk.
3. OVERALL READINESS (0-100): Composite audit readiness score.
4. OPEN FINDINGS: Total number of unresolved audit findings.
5. CRITICAL GAPS: Number of critical gaps that could impact audit outcomes.
...
${schema}`;
  ...
  try {
    console.log("[Pivot] Generating Audit Readiness (Wave 40)...");
    const result = await callJson(genai, prompt);
    return result as unknown as AuditReadiness;
  } catch (e) {
    console.warn("[Pivot] Audit Readiness (Wave 40) synthesis failed:", e);
    return null;
  }
}
...
export async function synthesizeSalesMethodology(
  packet: BusinessPacket,
  questionnaire: Questionnaire
  ...
  const schema = `{
  "summary": "2-3 sentence overview of sales methodology effectiveness and adoption",
  "stages": [{
    "stage": "Sales stage name (e.g., Discovery, Qualification, Proposal, etc.)",
    "activities": "Key activities performed in this stage",
    "tools": "Tools and resources used",
    "exitCriteria": "Criteria to advance to next stage",
    "avgDuration": "Average time spent in this stage",
    "conversionRate": "Stage-to-stage conversion rate"
  }],
  "methodology": "Name of sales methodology in use (MEDDIC, Challenger, SPIN, etc.)",
  "adoptionRate": "Percentage of team consistently following the methodology",
  "avgDealCycle": "Average deal cycle length",
  "winRate": "Overall win rate",
  "recommendations": ["recommendation 1", "..."]
}`;

  const prompt = `You are a sales methodology consultant specializing in sales process design, methodology adoption, deal cycle optimization, and win rate improvement across B2B and B2C sales organizations.
...
Produce a comprehensive Sales Methodology analysis:

1. SUMMARY: 2-3 sentence overview of current sales methodology effectiveness.
2. STAGES (5-7): For each sales stage — stage name, key activities, tools used, exit criteria, average duration, conversion rate.
3. METHODOLOGY: The primary sales methodology in use or recommended.
4. ADOPTION RATE: Percentage of team consistently following the methodology.
5. AVG DEAL CYCLE: Average deal cycle length from first contact to close.
...
${schema}`;
  ...
  try {
    console.log("[Pivot] Generating Sales Methodology (Wave 41)...");
    const result = await callJson(genai, prompt);
    return result as unknown as SalesMethodology;
  } catch (e) {
    console.warn("[Pivot] Sales Methodology (Wave 41) synthesis failed:", e);
    return null;
  }
}

export async function synthesizePipelineVelocity(
  packet: BusinessPacket,
  questionnaire: Questionnaire
  ...
  const schema = `{
  "summary": "2-3 sentence overview of pipeline velocity and deal flow health",
  "stages": [{
    "stage": "Pipeline stage name",
    "deals": 0,
    "value": "Total dollar value of deals in this stage",
    "avgAge": "Average age of deals in this stage",
    "conversionRate": "Conversion rate to next stage",
    "velocity": "Speed of deal movement through this stage"
  }],
  "overallVelocity": "Overall pipeline velocity metric (e.g., $X/day)",
  "avgDealSize": "Average deal size across pipeline",
  "pipelineCoverage": "Pipeline coverage ratio (pipeline value / quota)",
  "forecastAccuracy": "Historical forecast accuracy percentage",
  "recommendations": ["recommendation 1", "..."]
}`;

  const prompt = `You are a sales operations analyst specializing in pipeline velocity optimization, deal flow analysis, forecast accuracy, and pipeline coverage management.
...
Produce a comprehensive Pipeline Velocity analysis:

1. SUMMARY: 2-3 sentence overview of pipeline velocity and deal flow health.
2. STAGES (5-7): For each pipeline stage — stage name, deal count, total value, average age, conversion rate, velocity.
3. OVERALL VELOCITY: Overall pipeline velocity metric.
4. AVG DEAL SIZE: Average deal size across the entire pipeline.
5. PIPELINE COVERAGE: Pipeline coverage ratio relative to quota or targets.
...
${schema}`;
  ...
  try {
    console.log("[Pivot] Generating Pipeline Velocity (Wave 41)...");
    const result = await callJson(genai, prompt);
    return result as unknown as PipelineVelocity;
  } catch (e) {
    console.warn("[Pivot] Pipeline Velocity (Wave 41) synthesis failed:", e);
    return null;
  }
}

export async function synthesizeDealQualification(
  packet: BusinessPacket,
  questionnaire: Questionnaire
): Promise<DealQualification | null> {
  const apiKey = process.env.GEMINI_API_KEY;
  if (!apiKey) return null;
  ...
  const schema = `{
  "summary": "2-3 sentence overview of deal qualification effectiveness and quality",
  "criteria": [{
    "criterion": "Qualification criterion name (e.g., Budget, Authority, Need, Timeline)",
    "weight": 0,
    "scoring": "How this criterion is scored",
    "threshold": "Minimum score to pass",
    "passRate": "Percentage of deals passing this criterion",
    "impact": "Impact on win rate when criterion is met"
  }],
  "framework": "Qualification framework in use (BANT, MEDDIC, CHAMP, etc.)",
  "qualificationRate": "Percentage of leads that pass qualification",
  "avgScore": 0,
  "topDisqualifier": "Most common reason deals fail qualification",
  "recommendations": ["recommendation 1", "..."]
}`;

  const prompt = `You are a sales qualification specialist focusing on lead scoring, deal qualification frameworks, pipeline quality management, and conversion optimization.
...
Produce a comprehensive Deal Qualification analysis:

1. SUMMARY: 2-3 sentence overview of deal qualification effectiveness.
2. CRITERIA (5-7): For each qualification criterion — criterion name, weight (numeric), scoring method, threshold, pass rate, impact on win rate.
3. FRAMEWORK: The qualification framework in use or recommended.
4. QUALIFICATION RATE: Percentage of leads that pass full qualification.
5. AVG SCORE: Average qualification score across all deals (numeric).
...
${schema}`;
  ...
  try {
    console.log("[Pivot] Generating Deal Qualification (Wave 41)...");
    const result = await callJson(genai, prompt);
    return result as unknown as DealQualification;
  } catch (e) {
    console.warn("[Pivot] Deal Qualification (Wave 41) synthesis failed:", e);
    return null;
  }
}

export async function synthesizeSalesCoaching(
  packet: BusinessPacket,
  questionnaire: Questionnaire
): Promise<SalesCoaching | null> {
  const apiKey = process.env.GEMINI_API_KEY;
  if (!apiKey) return null;
  ...
  const schema = `{
  "summary": "2-3 sentence overview of sales coaching program and team development",
  "areas": [{
    "area": "Coaching area (e.g., Discovery, Objection Handling, Closing, etc.)",
    "currentSkill": "Current skill level assessment",
    "targetSkill": "Target skill level",
    "gap": "Gap between current and target",
    "method": "Coaching method or approach",
    "timeline": "Timeline to close the gap"
  }],
  "teamAvgScore": 0,
  "topPerformerGap": "Gap between top performers and team average",
  "coachingHours": "Average coaching hours per rep per month",
  "impactOnQuota": "Measured impact of coaching on quota attainment",
  "recommendations": ["recommendation 1", "..."]
}`;

  const prompt = `You are a sales coaching and enablement expert specializing in rep development, skill gap analysis, coaching program design, and performance improvement.
...
Produce a comprehensive Sales Coaching analysis:

1. SUMMARY: 2-3 sentence overview of sales coaching program effectiveness.
2. AREAS (5-7): For each coaching area — area name, current skill level, target skill level, gap, coaching method, timeline.
3. TEAM AVG SCORE (numeric): Average coaching/skill score across the sales team.
4. TOP PERFORMER GAP: Gap between top performers and team average.
5. COACHING HOURS: Average coaching hours per rep per month.
...
${schema}`;
  ...
  try {
    console.log("[Pivot] Generating Sales Coaching (Wave 41)...");
    const result = await callJson(genai, prompt);
    return result as unknown as SalesCoaching;
  } catch (e) {
    console.warn("[Pivot] Sales Coaching (Wave 41) synthesis failed:", e);
    return null;
  }
}

export async function synthesizeAccountPlanning(
  packet: BusinessPacket,
  questionnaire: Questionnaire
): Promise<AccountPlanning | null> {
  const apiKey = process.env.GEMINI_API_KEY;
  if (!apiKey) return null;
  ...
  const schema = `{
  "summary": "2-3 sentence overview of account planning and expansion strategy",
  "plans": [{
    "account": "Account name or segment",
    "tier": "Account tier (Strategic, Enterprise, Mid-Market, etc.)",
    "revenue": "Current revenue from this account",
    "whitespace": "Identified whitespace / upsell opportunity",
    "strategy": "Account growth strategy",
    "nextAction": "Next recommended action"
  }],
  "totalWhitespace": "Total identified whitespace across all accounts",
  "topAccounts": 0,
  "penetrationRate": "Average account penetration rate",
  "expansionPipeline": "Total expansion pipeline value",
  "recommendations": ["recommendation 1", "..."]
}`;

  const prompt = `You are a strategic account planning consultant specializing in account expansion, whitespace analysis, account tiering, and customer penetration strategy.
...
Produce a comprehensive Account Planning analysis:

1. SUMMARY: 2-3 sentence overview of account planning maturity and expansion potential.
2. PLANS (5-7): For each key account — account name, tier, current revenue, whitespace, growth strategy, next action.
3. TOTAL WHITESPACE: Total identified whitespace across all accounts.
4. TOP ACCOUNTS (numeric): Number of strategic/top-tier accounts.
5. PENETRATION RATE: Average account penetration rate.
...
${schema}`;
  ...
  try {
    console.log("[Pivot] Generating Account Planning (Wave 41)...");
    const result = await callJson(genai, prompt);
    return result as unknown as AccountPlanning;
  } catch (e) {
    console.warn("[Pivot] Account Planning (Wave 41) synthesis failed:", e);
    return null;
  }
}

export async function synthesizeCompetitiveBattlecards(
  packet: BusinessPacket,
  questionnaire: Questionnaire
): Promise<CompetitiveBattlecards | null> {
  const apiKey = process.env.GEMINI_API_KEY;
  if (!apiKey) return null;
  ...
  const schema = `{
  "summary": "2-3 sentence overview of competitive landscape and battlecard readiness",
  "cards": [{
    "competitor": "Competitor name",
    "strengths": "Key competitor strengths",
    "weaknesses": "Key competitor weaknesses",
    "counterStrategy": "Recommended counter-positioning strategy",
    "winRate": "Win rate against this competitor",
    "keyDifferentiator": "Primary differentiator vs. this competitor"
  }],
  "totalCompetitors": 0,
  "overallWinRate": "Overall win rate against all tracked competitors",
  "topThreat": "The single biggest competitive threat",
  "bestCounter": "The most effective counter-strategy across competitors",
  "recommendations": ["recommendation 1", "..."]
}`;

  const prompt = `You are a competitive intelligence analyst specializing in battlecard creation, competitive positioning, win/loss analysis, and sales enablement for competitive deals.
...
Produce a comprehensive Competitive Battlecards analysis:

1. SUMMARY: 2-3 sentence overview of competitive landscape and battlecard readiness.
2. CARDS (4-6): For each competitor — competitor name, strengths, weaknesses, counter-strategy, win rate, key differentiator.
3. TOTAL COMPETITORS (numeric): Number of tracked competitors.
4. OVERALL WIN RATE: Overall win rate against all tracked competitors.
5. TOP THREAT: The single biggest competitive threat.
...
${schema}`;
  ...
  try {
    console.log("[Pivot] Generating Competitive Battlecards (Wave 41)...");
    const result = await callJson(genai, prompt);
    return result as unknown as CompetitiveBattlecards;
  } catch (e) {
    console.warn("[Pivot] Competitive Battlecards (Wave 41) synthesis failed:", e);
    return null;
  }
}
...
export async function synthesizeCashBurnAnalysis(
  packet: BusinessPacket,
  questionnaire: Questionnaire
): Promise<CashBurnAnalysis | null> {
  const apiKey = process.env.GEMINI_API_KEY;
  if (!apiKey) return null;
  ...
  const schema = `{
  "summary": "2-3 sentence overview of cash burn rate and efficiency",
  "categories": [{
    "category": "Burn category (e.g., Payroll, Infrastructure, Marketing, etc.)",
    "monthly": "Monthly spend in this category",
    "annual": "Annualized spend",
    "trend": "Spend trend (increasing, stable, decreasing)",
    "essential": true,
    "optimizable": "Potential optimization opportunity"
  }],
  "monthlyBurn": "Total monthly cash burn",
  "runway": "Remaining cash runway at current burn rate",
  "burnEfficiency": "Burn efficiency ratio (revenue generated per dollar burned)",
  "topBurnArea": "Largest single burn category",
  "recommendations": ["recommendation 1", "..."]
}`;

  const prompt = `You are a financial analyst specializing in cash burn analysis, runway optimization, burn rate efficiency, and capital allocation for growth-stage businesses.
...
Produce a comprehensive Cash Burn Analysis:

1. SUMMARY: 2-3 sentence overview of cash burn rate and efficiency.
2. CATEGORIES (5-7): For each burn category — category name, monthly spend, annual spend, trend, whether essential (boolean), optimization opportunity.
3. MONTHLY BURN: Total monthly cash burn.
4. RUNWAY: Remaining cash runway at current burn rate.
5. BURN EFFICIENCY: Burn efficiency ratio (revenue per dollar burned).
...
${schema}`;
  ...
  try {
    console.log("[Pivot] Generating Cash Burn Analysis (Wave 42)...");
    const result = await callJson(genai, prompt);
    return result as unknown as CashBurnAnalysis;
  } catch (e) {
    console.warn("[Pivot] Cash Burn Analysis (Wave 42) synthesis failed:", e);
    return null;
  }
}

export async function synthesizeRevenuePerEmployee(
  packet: BusinessPacket,
  questionnaire: Questionnaire
): Promise<RevenuePerEmployee | null> {
  const apiKey = process.env.GEMINI_API_KEY;
  if (!apiKey) return null;
  ...
  const schema = `{
  "summary": "2-3 sentence overview of revenue per employee efficiency",
  "departments": [{
    "department": "Department name",
    "headcount": 0,
    "revenue": "Revenue attributed to this department",
    "revenuePerHead": "Revenue per employee in this department",
    "benchmark": "Industry benchmark for this department",
    "gap": "Gap vs. benchmark"
  }],
  "overallRPE": "Overall revenue per employee",
  "industryBenchmark": "Industry benchmark for revenue per employee",
  "topPerformer": "Department with highest revenue per employee",
  "improvementPotential": "Estimated improvement potential in dollars",
  "recommendations": ["recommendation 1", "..."]
}`;

  const prompt = `You are a workforce productivity analyst specializing in revenue per employee metrics, departmental efficiency, headcount optimization, and workforce ROI analysis.
...
Produce a comprehensive Revenue Per Employee analysis:

1. SUMMARY: 2-3 sentence overview of revenue per employee efficiency.
2. DEPARTMENTS (5-7): For each department — department name, headcount (numeric), revenue attributed, revenue per head, industry benchmark, gap vs. benchmark.
3. OVERALL RPE: Overall revenue per employee across the organization.
4. INDUSTRY BENCHMARK: Industry benchmark for revenue per employee.
5. TOP PERFORMER: Department with the highest revenue per employee.
...
${schema}`;
  ...
  try {
    console.log("[Pivot] Generating Revenue Per Employee (Wave 42)...");
    const result = await callJson(genai, prompt);
    return result as unknown as RevenuePerEmployee;
  } catch (e) {
    console.warn("[Pivot] Revenue Per Employee (Wave 42) synthesis failed:", e);
    return null;
  }
}

export async function synthesizeFinancialBenchmarking(
  packet: BusinessPacket,
  questionnaire: Questionnaire
): Promise<FinancialBenchmarking | null> {
  const apiKey = process.env.GEMINI_API_KEY;
  if (!apiKey) return null;
  ...
  const schema = `{
  "summary": "2-3 sentence overview of financial benchmarking vs. industry peers",
  "metrics": [{
    "metric": "Financial metric name (e.g., Gross Margin, CAC, LTV, etc.)",
    "companyValue": "Company's current value for this metric",
    "industryMedian": "Industry median value",
    "topQuartile": "Top quartile value",
    "percentile": "Company's percentile rank",
    "action": "Recommended action to improve"
  }],
  "overallPercentile": "Overall percentile rank across all metrics",
  "strongestMetric": "Best-performing metric vs. benchmarks",
  "weakestMetric": "Worst-performing metric vs. benchmarks",
  "peerGroup": "Description of the peer group used for comparison",
  "recommendations": ["recommendation 1", "..."]
}`;

  const prompt = `You are a financial benchmarking analyst specializing in peer comparison, industry metric analysis, percentile ranking, and performance gap identification for businesses.
...
Produce a comprehensive Financial Benchmarking analysis:

1. SUMMARY: 2-3 sentence overview of financial performance vs. industry peers.
2. METRICS (6-8): For each financial metric — metric name, company value, industry median, top quartile, percentile rank, recommended action.
3. OVERALL PERCENTILE: Overall percentile rank across all benchmarked metrics.
4. STRONGEST METRIC: Best-performing metric vs. benchmarks.
5. WEAKEST METRIC: Worst-performing metric vs. benchmarks.
...
${schema}`;
  ...
  try {
    console.log("[Pivot] Generating Financial Benchmarking (Wave 42)...");
    const result = await callJson(genai, prompt);
    return result as unknown as FinancialBenchmarking;
  } catch (e) {
    console.warn("[Pivot] Financial Benchmarking (Wave 42) synthesis failed:", e);
    return null;
  }
}

export async function synthesizeInvestmentPortfolio(
  packet: BusinessPacket,
  questionnaire: Questionnaire
  ...
  const schema = `{
  "summary": "2-3 sentence overview of investment portfolio and returns",
  "investments": [{
    "investment": "Investment name or initiative",
    "amount": "Amount invested",
    "expectedReturn": "Expected return or ROI",
    "timeline": "Timeline to realize returns",
    "risk": "Risk level (Low, Medium, High)",
    "status": "Current status (Active, Planned, Completed, etc.)"
  }],
  "totalInvested": "Total capital invested across all initiatives",
  "weightedReturn": "Weighted average expected return",
  "riskProfile": "Overall portfolio risk profile",
  "topPerformer": "Best-performing investment",
  "recommendations": ["recommendation 1", "..."]
}`;

  const prompt = `You are an investment portfolio analyst specializing in capital allocation, ROI analysis, investment risk assessment, and portfolio optimization for business investments.
...
Produce a comprehensive Investment Portfolio analysis:

1. SUMMARY: 2-3 sentence overview of investment portfolio health and returns.
2. INVESTMENTS (5-7): For each investment — investment name, amount invested, expected return, timeline, risk level, current status.
3. TOTAL INVESTED: Total capital invested across all initiatives.
4. WEIGHTED RETURN: Weighted average expected return across portfolio.
5. RISK PROFILE: Overall portfolio risk profile.
...
${schema}`;
  ...
  try {
    console.log("[Pivot] Generating Investment Portfolio (Wave 42)...");
    const result = await callJson(genai, prompt);
    return result as unknown as InvestmentPortfolio;
  } catch (e) {
    console.warn("[Pivot] Investment Portfolio (Wave 42) synthesis failed:", e);
    return null;
  }
}

export async function synthesizeCostAllocationModel(
  packet: BusinessPacket,
  questionnaire: Questionnaire
): Promise<CostAllocationModel | null> {
  const apiKey = process.env.GEMINI_API_KEY;
  if (!apiKey) return null;
  ...
  const schema = `{
  "summary": "2-3 sentence overview of cost allocation accuracy and efficiency",
  "centers": [{
    "center": "Cost center name",
    "allocated": "Allocated budget",
    "actual": "Actual spend",
    "variance": "Variance between allocated and actual",
    "driver": "Primary cost driver",
    "optimization": "Optimization opportunity"
  }],
  "totalAllocated": "Total allocated budget across all cost centers",
  "unallocated": "Unallocated or untracked costs",
  "accuracyScore": "Cost allocation accuracy score or rating",
  "topVariance": "Cost center with the largest variance",
  "recommendations": ["recommendation 1", "..."]
}`;

  const prompt = `You are a cost management specialist focusing on cost allocation modeling, activity-based costing, variance analysis, and cost center optimization.
...
Produce a comprehensive Cost Allocation Model analysis:

1. SUMMARY: 2-3 sentence overview of cost allocation accuracy and efficiency.
2. CENTERS (5-7): For each cost center — center name, allocated budget, actual spend, variance, primary cost driver, optimization opportunity.
3. TOTAL ALLOCATED: Total allocated budget across all cost centers.
4. UNALLOCATED: Unallocated or untracked costs.
5. ACCURACY SCORE: Cost allocation accuracy score or rating.
...
${schema}`;
  ...
  try {
    console.log("[Pivot] Generating Cost Allocation Model (Wave 42)...");
    const result = await callJson(genai, prompt);
    return result as unknown as CostAllocationModel;
  } catch (e) {
    console.warn("[Pivot] Cost Allocation Model (Wave 42) synthesis failed:", e);
    return null;
  }
}

export async function synthesizeMarginWaterfall(
  packet: BusinessPacket,
  questionnaire: Questionnaire
): Promise<MarginWaterfall | null> {
  const apiKey = process.env.GEMINI_API_KEY;
  if (!apiKey) return null;
  ...
  const schema = `{
  "summary": "2-3 sentence overview of margin waterfall and profitability layers",
  "layers": [{
    "layer": "Margin layer name (e.g., Gross Revenue, COGS, Gross Profit, OpEx, etc.)",
    "revenue": "Revenue or inflow at this layer",
    "cost": "Cost or deduction at this layer",
    "margin": "Margin in dollars at this layer",
    "marginPercent": "Margin as a percentage",
    "trend": "Trend vs. prior period"
  }],
  "grossMargin": "Gross margin percentage",
  "operatingMargin": "Operating margin percentage",
  "netMargin": "Net margin percentage",
  "biggestLeakage": "Layer with the largest margin leakage",
  "recommendations": ["recommendation 1", "..."]
}`;

  const prompt = `You are a profitability analyst specializing in margin waterfall analysis, cost structure optimization, margin leakage identification, and profitability improvement.
...
Produce a comprehensive Margin Waterfall analysis:

1. SUMMARY: 2-3 sentence overview of margin waterfall from gross revenue to net income.
2. LAYERS (5-7): For each margin layer — layer name, revenue/inflow, cost/deduction, margin in dollars, margin percentage, trend vs. prior period.
3. GROSS MARGIN: Gross margin percentage.
4. OPERATING MARGIN: Operating margin percentage.
5. NET MARGIN: Net margin percentage.
...
${schema}`;
  ...
  try {
    console.log("[Pivot] Generating Margin Waterfall (Wave 42)...");
    const result = await callJson(genai, prompt);
    return result as unknown as MarginWaterfall;
  } catch (e) {
    console.warn("[Pivot] Margin Waterfall (Wave 42) synthesis failed:", e);
    return null;
  }
}
...
export async function synthesizeCustomerOnboardingMetrics(
  packet: BusinessPacket,
  questionnaire: Questionnaire
): Promise<CustomerOnboardingMetrics | null> {
  const apiKey = process.env.GEMINI_API_KEY;
  if (!apiKey) return null;
  ...
  const schema = `{
  "summary": "2-3 sentence overview of customer onboarding effectiveness",
  "steps": [{
    "step": "Onboarding step name",
    "completionRate": "Percentage of customers completing this step",
    "avgTime": "Average time to complete this step",
    "dropoffRate": "Percentage of customers dropping off at this step",
    "bottleneck": "Key bottleneck or friction point",
    "improvement": "Recommended improvement"
  }],
  "overallCompletion": "Overall onboarding completion rate",
  "avgTimeToValue": "Average time from signup to first value realization",
  "activationRate": "Percentage of customers reaching activation milestone",
  "topDropoff": "Step with the highest customer dropoff",
  "recommendations": ["recommendation 1", "..."]
}`;

  const prompt = `You are a customer onboarding specialist focusing on time-to-value optimization, activation rate improvement, onboarding funnel analysis, and customer experience during initial adoption.
...
Produce a comprehensive Customer Onboarding Metrics analysis:

1. SUMMARY: 2-3 sentence overview of onboarding effectiveness and activation.
2. STEPS (5-7): For each onboarding step — step name, completion rate, average time, dropoff rate, bottleneck, recommended improvement.
3. OVERALL COMPLETION: Overall onboarding completion rate.
4. AVG TIME TO VALUE: Average time from signup to first value realization.
5. ACTIVATION RATE: Percentage of customers reaching activation milestone.
...
${schema}`;
  ...
  try {
    console.log("[Pivot] Generating Customer Onboarding Metrics (Wave 43)...");
    const result = await callJson(genai, prompt);
    return result as unknown as CustomerOnboardingMetrics;
  } catch (e) {
    console.warn("[Pivot] Customer Onboarding Metrics (Wave 43) synthesis failed:", e);
    return null;
  }
}

export async function synthesizeHealthScoreModel(
  packet: BusinessPacket,
  questionnaire: Questionnaire
): Promise<HealthScoreModel | null> {
  const apiKey = process.env.GEMINI_API_KEY;
  if (!apiKey) return null;
  ...
  const schema = `{
  "summary": "2-3 sentence overview of customer health score model and distribution",
  "dimensions": [{
    "dimension": "Health dimension (e.g., Product Usage, Support Tickets, NPS, etc.)",
    "weight": 0,
    "score": 0,
    "threshold": 0,
    "signal": "Key signal or indicator for this dimension",
    "action": "Recommended action when below threshold"
  }],
  "overallScore": 0,
  "healthyPercentage": "Percentage of customers in healthy status",
  "atRiskPercentage": "Percentage of customers at risk",
  "topPredictor": "Dimension most predictive of churn or expansion",
  "recommendations": ["recommendation 1", "..."]
}`;

  const prompt = `You are a customer success analytics expert specializing in health score modeling, churn prediction, customer segmentation by health, and proactive intervention strategies.
...
Produce a comprehensive Health Score Model analysis:

1. SUMMARY: 2-3 sentence overview of customer health score model and distribution.
2. DIMENSIONS (5-7): For each health dimension — dimension name, weight (numeric), score (numeric 0-100), threshold (numeric), key signal, recommended action.
3. OVERALL SCORE (numeric 0-100): Composite customer health score.
4. HEALTHY PERCENTAGE: Percentage of customers in healthy status.
5. AT-RISK PERCENTAGE: Percentage of customers at risk.
...
${schema}`;
  ...
  try {
    console.log("[Pivot] Generating Health Score Model (Wave 43)...");
    const result = await callJson(genai, prompt);
    return result as unknown as HealthScoreModel;
  } catch (e) {
    console.warn("[Pivot] Health Score Model (Wave 43) synthesis failed:", e);
    return null;
  }
}

export async function synthesizeCsExpansionPlaybook(
  packet: BusinessPacket,
  questionnaire: Questionnaire
): Promise<CsExpansionPlaybook | null> {
  const apiKey = process.env.GEMINI_API_KEY;
  if (!apiKey) return null;
  ...
  const schema = `{
  "summary": "2-3 sentence overview of CS-driven expansion opportunities",
  "opportunities": [{
    "account": "Account name or segment",
    "opportunity": "Expansion opportunity description",
    "value": "Estimated expansion value",
    "probability": "Probability of conversion",
    "trigger": "Trigger or signal indicating readiness",
    "playbook": "Recommended playbook or approach"
  }],
  "totalPipeline": "Total CS expansion pipeline value",
  "avgExpansionRate": "Average expansion rate across customer base",
  "topPlay": "Most effective expansion play",
  "conversionRate": "Overall expansion opportunity conversion rate",
  "recommendations": ["recommendation 1", "..."]
}`;

  const prompt = `You are a customer success expansion strategist specializing in upsell/cross-sell identification, expansion revenue generation, customer-led growth, and CS-driven pipeline creation.
...
Produce a comprehensive CS Expansion Playbook analysis:

1. SUMMARY: 2-3 sentence overview of CS-driven expansion opportunities.
2. OPPORTUNITIES (5-7): For each opportunity — account name, opportunity description, estimated value, probability, trigger signal, recommended playbook.
3. TOTAL PIPELINE: Total CS expansion pipeline value.
4. AVG EXPANSION RATE: Average expansion rate across the customer base.
5. TOP PLAY: Most effective expansion play or strategy.
...
${schema}`;
  ...
  try {
    console.log("[Pivot] Generating CS Expansion Playbook (Wave 43)...");
    const result = await callJson(genai, prompt);
    return result as unknown as CsExpansionPlaybook;
  } catch (e) {
    console.warn("[Pivot] CS Expansion Playbook (Wave 43) synthesis failed:", e);
    return null;
  }
}

export async function synthesizeRenewalForecasting(
  packet: BusinessPacket,
  questionnaire: Questionnaire
): Promise<RenewalForecasting | null> {
  const apiKey = process.env.GEMINI_API_KEY;
  if (!apiKey) return null;
  ...
  const schema = `{
  "summary": "2-3 sentence overview of renewal forecasting and retention outlook",
  "cohorts": [{
    "cohort": "Renewal cohort (e.g., Q1 2025, Enterprise, etc.)",
    "accounts": 0,
    "arr": "Annual recurring revenue for this cohort",
    "renewalRate": "Expected renewal rate",
    "risk": "Risk level or key risk factor",
    "action": "Recommended action for this cohort"
  }],
  "overallRenewalRate": "Overall expected renewal rate",
  "atRiskArr": "Total ARR at risk of non-renewal",
  "forecastAccuracy": "Historical renewal forecast accuracy",
  "topRisk": "Single biggest risk to renewals",
  "recommendations": ["recommendation 1", "..."]
}`;

  const prompt = `You are a renewal forecasting analyst specializing in renewal prediction, at-risk revenue identification, cohort analysis, and retention strategy optimization.
...
Produce a comprehensive Renewal Forecasting analysis:

1. SUMMARY: 2-3 sentence overview of renewal forecasting and retention outlook.
2. COHORTS (5-7): For each renewal cohort — cohort name, account count (numeric), ARR, expected renewal rate, risk, recommended action.
3. OVERALL RENEWAL RATE: Overall expected renewal rate across all cohorts.
4. AT-RISK ARR: Total ARR at risk of non-renewal.
5. FORECAST ACCURACY: Historical renewal forecast accuracy.
...
${schema}`;
  ...
  try {
    console.log("[Pivot] Generating Renewal Forecasting (Wave 43)...");
    const result = await callJson(genai, prompt);
    return result as unknown as RenewalForecasting;
  } catch (e) {
    console.warn("[Pivot] Renewal Forecasting (Wave 43) synthesis failed:", e);
    return null;
  }
}

export async function synthesizeCsOperations(
  packet: BusinessPacket,
  questionnaire: Questionnaire
): Promise<CsOperations | null> {
  const apiKey = process.env.GEMINI_API_KEY;
  if (!apiKey) return null;
  ...
  const schema = `{
  "summary": "2-3 sentence overview of CS operations efficiency and effectiveness",
  "metrics": [{
    "metric": "CS operations metric name",
    "current": "Current value",
    "target": "Target value",
    "trend": "Trend direction (improving, stable, declining)",
  ...
  "teamEfficiency": "Overall team efficiency rating or metric",
  "caseloadPerCsm": "Average caseload per CSM",
  "responseTime": "Average response time to customer requests",
  "escalationRate": "Percentage of cases escalated",
  "recommendations": ["recommendation 1", "..."]
}`;

  const prompt = `You are a CS operations consultant specializing in CS team efficiency, caseload optimization, process automation, and customer success operational excellence.
...
Produce a comprehensive CS Operations analysis:

1. SUMMARY: 2-3 sentence overview of CS operations efficiency and effectiveness.
2. METRICS (5-7): For each CS operations metric — metric name, current value, target value, trend, owner, recommended action.
3. TEAM EFFICIENCY: Overall team efficiency rating or metric.
4. CASELOAD PER CSM: Average caseload per customer success manager.
5. RESPONSE TIME: Average response time to customer requests.
...
${schema}`;
  ...
  try {
    console.log("[Pivot] Generating CS Operations (Wave 43)...");
    const result = await callJson(genai, prompt);
    return result as unknown as CsOperations;
  } catch (e) {
    console.warn("[Pivot] CS Operations (Wave 43) synthesis failed:", e);
    return null;
  }
}

export async function synthesizeCustomerMilestones(
  packet: BusinessPacket,
  questionnaire: Questionnaire
): Promise<CustomerMilestones | null> {
  const apiKey = process.env.GEMINI_API_KEY;
  if (!apiKey) return null;
  ...
  const schema = `{
  "summary": "2-3 sentence overview of customer milestone tracking and achievement",
  "milestones": [{
    "milestone": "Milestone name (e.g., First Value, Full Adoption, Expansion, etc.)",
    "segment": "Customer segment this applies to",
    "avgTimeToReach": "Average time to reach this milestone",
    "completionRate": "Percentage of customers reaching this milestone",
    "impact": "Impact on retention or expansion when milestone is reached",
    "blocker": "Key blocker preventing customers from reaching this milestone"
  }],
  "avgMilestonesReached": 0,
  "topMilestone": "Most impactful milestone for customer retention",
  "biggestGap": "Milestone with the largest gap between expected and actual completion",
  "correlationToRetention": "Correlation between milestone completion and retention rate",
  "recommendations": ["recommendation 1", "..."]
}`;

  const prompt = `You are a customer journey analyst specializing in milestone tracking, customer lifecycle management, success milestone definition, and outcome-based customer success.
...
Produce a comprehensive Customer Milestones analysis:

1. SUMMARY: 2-3 sentence overview of customer milestone tracking and achievement.
2. MILESTONES (5-7): For each milestone — milestone name, customer segment, average time to reach, completion rate, impact on retention, key blocker.
3. AVG MILESTONES REACHED (numeric): Average number of milestones reached per customer.
4. TOP MILESTONE: Most impactful milestone for customer retention.
5. BIGGEST GAP: Milestone with the largest gap between expected and actual completion.
...
${schema}`;
  ...
  try {
    console.log("[Pivot] Generating Customer Milestones (Wave 43)...");
    const result = await callJson(genai, prompt);
    return result as unknown as CustomerMilestones;
  } catch (e) {
    console.warn("[Pivot] Customer Milestones (Wave 43) synthesis failed:", e);
    return null;
  }
}
...
export async function synthesizeOkrFramework(
  packet: BusinessPacket,
  questionnaire: Questionnaire
): Promise<OkrFramework | null> {
  const apiKey = process.env.GEMINI_API_KEY;
  if (!apiKey) return null;
  ...
  const schema = `{
  "summary": "2-3 sentence overview of OKR framework and goal alignment",
  "okrs": [{
    "objective": "Objective statement",
    "keyResults": "Key results (comma-separated or bulleted)",
    "progress": "Progress percentage or status",
    "owner": "Responsible team or person",
    "status": "On Track / At Risk / Off Track",
    "blockers": "Key blockers or risks"
  }],
  "overallProgress": "Overall OKR progress percentage",
  "onTrackPercentage": "Percentage of OKRs on track",
  "topObjective": "Highest-priority objective",
  "alignmentScore": 0,
  "recommendations": ["recommendation 1", "..."]
}`;

  const prompt = `You are an OKR and goal-setting strategist specializing in objective alignment, key result definition, progress tracking, and organizational goal cascading.
...
Produce a comprehensive OKR Framework analysis:

1. SUMMARY: 2-3 sentence overview of OKR framework effectiveness and alignment.
2. OKRS (5-7): For each OKR — objective statement, key results, progress, owner, status, blockers.
3. OVERALL PROGRESS: Overall OKR progress percentage.
4. ON-TRACK PERCENTAGE: Percentage of OKRs currently on track.
5. TOP OBJECTIVE: Highest-priority objective for the organization.
...
${schema}`;
  ...
  try {
    console.log("[Pivot] Generating OKR Framework (Wave 44)...");
    const result = await callJson(genai, prompt);
    return result as unknown as OkrFramework;
  } catch (e) {
    console.warn("[Pivot] OKR Framework (Wave 44) synthesis failed:", e);
    return null;
  }
}

export async function synthesizeStrategicPillars(
  packet: BusinessPacket,
  questionnaire: Questionnaire
): Promise<StrategicPillars | null> {
  const apiKey = process.env.GEMINI_API_KEY;
  if (!apiKey) return null;
  ...
  const schema = `{
  "summary": "2-3 sentence overview of strategic pillars and execution progress",
  "pillars": [{
    "pillar": "Strategic pillar name",
    "description": "Description of this strategic pillar",
    "initiatives": 0,
    "progress": "Progress percentage or status",
    "investment": "Investment allocated to this pillar",
    "impact": "Expected business impact"
  }],
  "totalInitiatives": 0,
  "overallProgress": "Overall progress across all pillars",
  "topPillar": "Highest-performing strategic pillar",
  "resourceAllocation": "Summary of resource allocation across pillars",
  "recommendations": ["recommendation 1", "..."]
}`;

  const prompt = `You are a strategic planning consultant specializing in strategic pillar definition, initiative management, resource allocation, and strategic execution tracking.
...
Produce a comprehensive Strategic Pillars analysis:

1. SUMMARY: 2-3 sentence overview of strategic pillars and execution progress.
2. PILLARS (4-6): For each strategic pillar — pillar name, description, initiative count (numeric), progress, investment allocated, expected impact.
3. TOTAL INITIATIVES (numeric): Total number of strategic initiatives across all pillars.
4. OVERALL PROGRESS: Overall progress across all pillars.
5. TOP PILLAR: Highest-performing or most impactful strategic pillar.
...
${schema}`;
  ...
  try {
    console.log("[Pivot] Generating Strategic Pillars (Wave 44)...");
    const result = await callJson(genai, prompt);
    return result as unknown as StrategicPillars;
  } catch (e) {
    console.warn("[Pivot] Strategic Pillars (Wave 44) synthesis failed:", e);
    return null;
  }
}
...
export async function synthesizeCompetitivePositioning(
  packet: BusinessPacket,
  questionnaire: Questionnaire
): Promise<CompetitivePositioning | null> {
  const apiKey = process.env.GEMINI_API_KEY;
  if (!apiKey) return null;
  ...
  const schema = `{
  "summary": "2-3 sentence overview of competitive positioning and market perception",
  "dimensions": [{
    "dimension": "Positioning dimension (e.g., Price, Quality, Innovation, Service, etc.)",
    "companyPosition": "Company's position on this dimension",
    "competitorAvg": "Average competitor position",
    "gap": "Gap between company and competitor average",
    "importance": "Importance of this dimension to buyers",
    "action": "Recommended action to improve positioning"
  }],
  "overallPosition": "Overall competitive position (Leader, Challenger, Follower, Niche)",
  "strongestDimension": "Dimension where the company is strongest",
  "weakestDimension": "Dimension where the company is weakest",
  "perceptionGap": "Gap between actual position and market perception",
  "recommendations": ["recommendation 1", "..."]
}`;

  const prompt = `You are a competitive positioning strategist specializing in market positioning analysis, perceptual mapping, competitive differentiation, and strategic positioning optimization.
...
Produce a comprehensive Competitive Positioning analysis:

1. SUMMARY: 2-3 sentence overview of competitive positioning and market perception.
2. DIMENSIONS (5-7): For each positioning dimension — dimension name, company position, competitor average, gap, buyer importance, recommended action.
3. OVERALL POSITION: Overall competitive position (Leader, Challenger, Follower, Niche).
4. STRONGEST DIMENSION: Dimension where the company has the strongest position.
5. WEAKEST DIMENSION: Dimension where the company has the weakest position.
...
${schema}`;
  ...
  try {
    console.log("[Pivot] Generating Competitive Positioning (Wave 44)...");
    const result = await callJson(genai, prompt);
    return result as unknown as CompetitivePositioning;
  } catch (e) {
    console.warn("[Pivot] Competitive Positioning (Wave 44) synthesis failed:", e);
    return null;
  }
}

export async function synthesizeMarketShareAnalysis(
  packet: BusinessPacket,
  questionnaire: Questionnaire
): Promise<MarketShareAnalysis | null> {
  const apiKey = process.env.GEMINI_API_KEY;
  if (!apiKey) return null;
  ...
  const schema = `{
  "summary": "2-3 sentence overview of market share position and growth trajectory",
  "segments": [{
    "segment": "Market segment name",
    "companyShare": "Company's market share in this segment",
    "leaderShare": "Market leader's share in this segment",
    "gap": "Gap between company and leader",
    "trend": "Share trend (growing, stable, declining)",
    "opportunity": "Key opportunity in this segment"
  }],
  "overallShare": "Overall market share across all segments",
  "samGrowth": "Serviceable addressable market growth rate",
  "shareGrowthRate": "Rate of market share growth or decline",
  "topOpportunity": "Single biggest market share opportunity",
  "recommendations": ["recommendation 1", "..."]
}`;

  const prompt = `You are a market intelligence analyst specializing in market share analysis, competitive market dynamics, segment-level share tracking, and market opportunity identification.
...
Produce a comprehensive Market Share Analysis:

1. SUMMARY: 2-3 sentence overview of market share position and growth trajectory.
2. SEGMENTS (5-7): For each market segment — segment name, company share, leader share, gap, trend, key opportunity.
3. OVERALL SHARE: Overall market share across all segments.
4. SAM GROWTH: Serviceable addressable market growth rate.
5. SHARE GROWTH RATE: Rate of market share growth or decline.
...
${schema}`;
  ...
  try {
    console.log("[Pivot] Generating Market Share Analysis (Wave 44)...");
    const result = await callJson(genai, prompt);
    return result as unknown as MarketShareAnalysis;
  } catch (e) {
    console.warn("[Pivot] Market Share Analysis (Wave 44) synthesis failed:", e);
    return null;
  }
}

export async function synthesizeGrowthCorridors(
  packet: BusinessPacket,
  questionnaire: Questionnaire
): Promise<GrowthCorridors | null> {
  const apiKey = process.env.GEMINI_API_KEY;
  if (!apiKey) return null;
  ...
  const schema = `{
  "summary": "2-3 sentence overview of growth corridors and expansion opportunities",
  "corridors": [{
    "corridor": "Growth corridor name (e.g., new geography, new vertical, new product, etc.)",
    "tam": "Total addressable market for this corridor",
    "currentPenetration": "Current penetration in this corridor",
    "growthRate": "Expected growth rate",
    "competition": "Competitive intensity assessment",
    "timeline": "Timeline to meaningful traction"
  }],
  "totalTam": "Total TAM across all growth corridors",
  "topCorridor": "Most attractive growth corridor",
  "avgGrowthRate": "Average growth rate across corridors",
  "investmentRequired": "Total estimated investment required",
  "recommendations": ["recommendation 1", "..."]
}`;

  const prompt = `You are a growth strategy consultant specializing in growth corridor identification, market opportunity assessment, expansion planning, and TAM analysis.
...
Produce a comprehensive Growth Corridors analysis:

1. SUMMARY: 2-3 sentence overview of growth corridors and expansion opportunities.
2. CORRIDORS (4-6): For each growth corridor — corridor name, TAM, current penetration, growth rate, competitive intensity, timeline to traction.
3. TOTAL TAM: Total TAM across all identified growth corridors.
4. TOP CORRIDOR: Most attractive growth corridor based on opportunity and fit.
5. AVG GROWTH RATE: Average growth rate across all corridors.
...
${schema}`;
  ...
  try {
    console.log("[Pivot] Generating Growth Corridors (Wave 44)...");
    const result = await callJson(genai, prompt);
    return result as unknown as GrowthCorridors;
  } catch (e) {
    console.warn("[Pivot] Growth Corridors (Wave 44) synthesis failed:", e);
    return null;
  }
}

export async function synthesizeValuePropCanvas(
  packet: BusinessPacket,
  questionnaire: Questionnaire
): Promise<ValuePropCanvas | null> {
  const apiKey = process.env.GEMINI_API_KEY;
  if (!apiKey) return null;
  ...
  const schema = `{
  "summary": "2-3 sentence overview of value proposition fit and effectiveness",
  "elements": [{
    "element": "Canvas element (e.g., Customer Job, Pain, Gain, Product, Pain Reliever, Gain Creator)",
    "current": "Current state of this element",
    "ideal": "Ideal or target state",
    "gap": "Gap between current and ideal",
    "evidence": "Evidence or data supporting the assessment",
    "action": "Recommended action to close the gap"
  }],
  "overallFit": 0,
  "strongestElement": "Canvas element with the best product-market fit",
  "biggestGap": "Canvas element with the largest gap to close",
  "customerValidation": "Level of customer validation achieved",
  "recommendations": ["recommendation 1", "..."]
}`;

  const prompt = `You are a value proposition strategist specializing in Value Proposition Canvas design, product-market fit assessment, customer job mapping, and value proposition optimization.
...
Produce a comprehensive Value Prop Canvas analysis:

1. SUMMARY: 2-3 sentence overview of value proposition fit and effectiveness.
2. ELEMENTS (6-8): For each canvas element — element name, current state, ideal state, gap, supporting evidence, recommended action.
3. OVERALL FIT (numeric 0-100): Overall product-market fit score.
4. STRONGEST ELEMENT: Canvas element with the best product-market fit.
5. BIGGEST GAP: Canvas element with the largest gap to close.
...
${schema}`;
  ...
  try {
    console.log("[Pivot] Generating Value Prop Canvas (Wave 44)...");
    const result = await callJson(genai, prompt);
    return result as unknown as ValuePropCanvas;
  } catch (e) {
    console.warn("[Pivot] Value Prop Canvas (Wave 44) synthesis failed:", e);
    return null;
  }
}
...
export async function synthesizeCompetitiveMonitoring(
  packet: BusinessPacket,
  questionnaire: Questionnaire
): Promise<CompetitiveMonitoring | null> {
  const apiKey = process.env.GEMINI_API_KEY;
  if (!apiKey) return null;
  ...
  const schema = `{
  "summary": "2-3 sentence overview of competitive landscape monitoring and key signals detected",
  "signals": [{
    "competitor": "Competitor name",
    "signalType": "Type of signal (e.g., product launch, pricing change, partnership, hiring surge)",
    "description": "Detailed description of the competitive signal",
    "severity": "Signal severity (critical, high, medium, low)",
    "source": "Source or basis for the signal",
    "actionRequired": "Recommended action in response to this signal"
  }],
  "totalSignals": 0,
  "criticalAlerts": 0,
  "topThreat": "The single most significant competitive threat detected",
  "monitoringCadence": "Recommended monitoring frequency",
  "recommendations": ["recommendation 1", "..."]
}`;

  const prompt = `You are a competitive intelligence analyst specializing in competitor monitoring, threat detection, market signal analysis, and strategic response planning.
...
Produce a comprehensive Competitive Monitoring analysis:

1. SUMMARY: 2-3 sentence overview of competitive landscape and key signals detected.
2. SIGNALS (5-8): For each competitor signal — competitor name, signal type, description, severity, source, and recommended action.
3. TOTAL SIGNALS (number): Total number of competitive signals identified.
4. CRITICAL ALERTS (number): Number of critical-severity signals requiring immediate attention.
5. TOP THREAT: The single most significant competitive threat detected.
...
${schema}`;
  ...
  try {
    console.log("[Pivot] Generating Competitive Monitoring (Wave 45)...");
    const result = await callJson(genai, prompt);
    return result as unknown as CompetitiveMonitoring;
  } catch (e) {
    console.warn("[Pivot] Competitive Monitoring (Wave 45) synthesis failed:", e);
    return null;
  }
}

export async function synthesizeMarketTrendRadar(
  packet: BusinessPacket,
  questionnaire: Questionnaire
): Promise<MarketTrendRadar | null> {
  const apiKey = process.env.GEMINI_API_KEY;
  if (!apiKey) return null;
  ...
  const schema = `{
  "summary": "2-3 sentence overview of market trends and their strategic implications",
  "trends": [{
    "trend": "Trend name or description",
    "category": "Trend category (e.g., technology, regulation, consumer behavior, economic)",
    "momentum": "Trend momentum (accelerating, steady, decelerating, emerging)",
    "relevance": "Relevance to this business (critical, high, medium, low)",
    "timeHorizon": "Expected time horizon for impact (e.g., 6 months, 1-2 years)",
    "implication": "Strategic implication for the business"
  }],
  "topTrend": "The most impactful trend for this business",
  "emergingThemes": ["emerging theme 1", "..."],
  "disruptionRisk": "Overall disruption risk level from market trends",
  "opportunityWindow": "Window of opportunity to capitalize on trends",
  "recommendations": ["recommendation 1", "..."]
}`;

  const prompt = `You are a market trends analyst specializing in trend identification, disruption detection, strategic foresight, and market opportunity analysis.
...
Produce a comprehensive Market Trend Radar analysis:

1. SUMMARY: 2-3 sentence overview of market trends and their strategic implications.
2. TRENDS (5-8): For each trend — name, category, momentum, relevance, time horizon, and strategic implication.
3. TOP TREND: The most impactful trend for this specific business.
4. EMERGING THEMES (3-5): Emerging themes that could shape the market in the next 1-3 years.
5. DISRUPTION RISK: Overall disruption risk level from market trends.
...
${schema}`;
  ...
  try {
    console.log("[Pivot] Generating Market Trend Radar (Wave 45)...");
    const result = await callJson(genai, prompt);
    return result as unknown as MarketTrendRadar;
  } catch (e) {
    console.warn("[Pivot] Market Trend Radar (Wave 45) synthesis failed:", e);
    return null;
  }
}

export async function synthesizeIndustryBenchmarkIndex(
  packet: BusinessPacket,
  questionnaire: Questionnaire
): Promise<IndustryBenchmarkIndex | null> {
  const apiKey = process.env.GEMINI_API_KEY;
  if (!apiKey) return null;
  ...
  const schema = `{
  "summary": "2-3 sentence overview of how the business benchmarks against industry standards",
  "metrics": [{
    "metric": "Benchmark metric name (e.g., gross margin, revenue per employee)",
    "companyValue": "Company's value for this metric",
    "industryAvg": "Industry average for this metric",
    "topQuartile": "Top quartile benchmark value",
    "percentileRank": "Company's percentile rank",
    "trend": "Trend direction (improving, stable, declining)"
  }],
  "overallPercentile": "Overall percentile ranking across all metrics",
  "strongestMetric": "Metric where the company outperforms most significantly",
  "weakestMetric": "Metric with the largest gap to industry average",
  "peerGroup": "Description of the peer group used for benchmarking",
  "recommendations": ["recommendation 1", "..."]
}`;

  const prompt = `You are an industry benchmarking analyst specializing in performance comparison, peer group analysis, KPI benchmarking, and competitive positioning assessment.
...
Produce a comprehensive Industry Benchmark Index:

1. SUMMARY: 2-3 sentence overview of how the business benchmarks against industry standards.
2. METRICS (6-8): For each benchmark metric — metric name, company value, industry average, top quartile value, percentile rank, and trend direction.
3. OVERALL PERCENTILE: Overall percentile ranking across all measured metrics.
4. STRONGEST METRIC: Metric where the company outperforms most significantly.
5. WEAKEST METRIC: Metric with the largest gap to industry average.
...
${schema}`;
  ...
  try {
    console.log("[Pivot] Generating Industry Benchmark Index (Wave 45)...");
    const result = await callJson(genai, prompt);
    return result as unknown as IndustryBenchmarkIndex;
  } catch (e) {
    console.warn("[Pivot] Industry Benchmark Index (Wave 45) synthesis failed:", e);
    return null;
  }
}

export async function synthesizeCustomerIntelPlatform(
  packet: BusinessPacket,
  questionnaire: Questionnaire
): Promise<CustomerIntelPlatform | null> {
  const apiKey = process.env.GEMINI_API_KEY;
  if (!apiKey) return null;
  ...
  const schema = `{
  "summary": "2-3 sentence overview of customer intelligence capabilities and insights",
  "sources": [{
    "source": "Intelligence source name (e.g., CRM data, support tickets, surveys)",
    "dataType": "Type of data collected from this source",
    "frequency": "How often data is collected or refreshed",
    "reliability": "Reliability rating of this source (high, medium, low)",
    "coverage": "Coverage of customer base by this source",
    "insight": "Key insight derived from this source"
  }],
  "totalDataPoints": "Total number of customer data points available",
  "integrationLevel": "Level of integration across customer data sources",
  "topInsight": "The single most valuable customer insight discovered",
  "blindSpots": ["blind spot 1", "..."],
  "recommendations": ["recommendation 1", "..."]
}`;

  const prompt = `You are a customer intelligence strategist specializing in customer data integration, insight extraction, behavioral analysis, and customer-centric decision making.
...
Produce a comprehensive Customer Intel Platform analysis:

1. SUMMARY: 2-3 sentence overview of customer intelligence capabilities and insights.
2. SOURCES (5-7): For each intelligence source — source name, data type, frequency, reliability, coverage, and key insight.
3. TOTAL DATA POINTS: Total number of customer data points available across sources.
4. INTEGRATION LEVEL: Level of integration across customer data sources (e.g., "Fragmented", "Partially integrated", "Unified").
5. TOP INSIGHT: The single most valuable customer insight discovered.
...
${schema}`;
  ...
  try {
    console.log("[Pivot] Generating Customer Intel Platform (Wave 45)...");
    const result = await callJson(genai, prompt);
    return result as unknown as CustomerIntelPlatform;
  } catch (e) {
    console.warn("[Pivot] Customer Intel Platform (Wave 45) synthesis failed:", e);
    return null;
  }
}

export async function synthesizePriceSensitivityModel(
  packet: BusinessPacket,
  questionnaire: Questionnaire
): Promise<PriceSensitivityModel | null> {
  const apiKey = process.env.GEMINI_API_KEY;
  if (!apiKey) return null;
  ...
  const schema = `{
  "summary": "2-3 sentence overview of price sensitivity across customer segments",
  "segments": [{
    "segment": "Customer segment name",
    "elasticity": "Price elasticity for this segment (e.g., highly elastic, moderately elastic, inelastic)",
    "optimalRange": "Optimal price range for this segment",
    "currentPrice": "Current price point for this segment",
    "willingness": "Willingness to pay assessment",
    "switchingCost": "Switching cost level for this segment (high, medium, low)"
  }],
  "overallElasticity": "Overall price elasticity across all segments",
  "priceFloor": "Minimum viable price before losing market credibility",
  "priceCeiling": "Maximum price the market will bear",
  "optimalPrice": "Recommended optimal price point",
  "recommendations": ["recommendation 1", "..."]
}`;

  const prompt = `You are a pricing strategist specializing in price sensitivity analysis, elasticity modeling, willingness-to-pay research, and segment-based pricing optimization.
...
Produce a comprehensive Price Sensitivity Model:

1. SUMMARY: 2-3 sentence overview of price sensitivity across customer segments.
2. SEGMENTS (4-6): For each customer segment — segment name, elasticity, optimal price range, current price, willingness to pay, and switching cost.
3. OVERALL ELASTICITY: Overall price elasticity across all segments.
4. PRICE FLOOR: Minimum viable price before losing market credibility.
5. PRICE CEILING: Maximum price the market will bear.
...
${schema}`;
  ...
  try {
    console.log("[Pivot] Generating Price Sensitivity Model (Wave 45)...");
    const result = await callJson(genai, prompt);
    return result as unknown as PriceSensitivityModel;
  } catch (e) {
    console.warn("[Pivot] Price Sensitivity Model (Wave 45) synthesis failed:", e);
    return null;
  }
}

export async function synthesizeDemandSignalAnalysis(
  packet: BusinessPacket,
  questionnaire: Questionnaire
): Promise<DemandSignalAnalysis | null> {
  const apiKey = process.env.GEMINI_API_KEY;
  if (!apiKey) return null;
  ...
  const schema = `{
  "summary": "2-3 sentence overview of demand signals and their implications for growth",
  "signals": [{
    "signal": "Demand signal name or description",
    "source": "Source of the demand signal (e.g., pipeline data, web traffic, RFP volume)",
    "strength": "Signal strength (strong, moderate, weak)",
    "trend": "Trend direction (increasing, stable, decreasing)",
    "leadTime": "Lead time before this signal converts to revenue",
    "confidence": "Confidence level in this signal (high, medium, low)"
  }],
  "overallDemand": "Overall demand outlook assessment",
  "growthIndicator": "Primary growth indicator direction",
  "seasonalPattern": "Key seasonal or cyclical pattern identified",
  "forecastAccuracy": "Estimated accuracy of demand forecasting",
  "recommendations": ["recommendation 1", "..."]
}`;

  const prompt = `You are a demand planning analyst specializing in demand signal detection, leading indicator analysis, demand forecasting, and market demand assessment.
...
Produce a comprehensive Demand Signal Analysis:

1. SUMMARY: 2-3 sentence overview of demand signals and their implications for growth.
2. SIGNALS (5-7): For each demand indicator — signal name, source, strength, trend, lead time, and confidence level.
3. OVERALL DEMAND: Overall demand outlook assessment.
4. GROWTH INDICATOR: Primary growth indicator direction.
5. SEASONAL PATTERN: Key seasonal or cyclical pattern identified.
...
${schema}`;
  ...
  try {
    console.log("[Pivot] Generating Demand Signal Analysis (Wave 45)...");
    const result = await callJson(genai, prompt);
    return result as unknown as DemandSignalAnalysis;
  } catch (e) {
    console.warn("[Pivot] Demand Signal Analysis (Wave 45) synthesis failed:", e);
    return null;
  }
}
...
export async function synthesizeDigitalMaturityIndex(
  packet: BusinessPacket,
  questionnaire: Questionnaire
): Promise<DigitalMaturityIndex | null> {
  const apiKey = process.env.GEMINI_API_KEY;
  if (!apiKey) return null;
  ...
  const schema = `{
  "summary": "2-3 sentence overview of digital maturity across the organization",
  "pillars": [{
    "pillar": "Digital maturity pillar (e.g., Strategy, Customer Experience, Operations, Technology, Culture)",
    "currentLevel": 0,
    "targetLevel": 0,
    "gap": "Gap description between current and target",
    "investment": "Estimated investment to close the gap",
    "timeline": "Timeline to reach target maturity level"
  }],
  "overallScore": 0,
  "industryAvg": 0,
  "topGap": "Pillar with the largest maturity gap",
  "digitalReadiness": "Overall digital readiness assessment",
  "recommendations": ["recommendation 1", "..."]
}`;

  const prompt = `You are a digital transformation strategist specializing in digital maturity assessment, transformation roadmapping, technology adoption analysis, and digital capability building.
...
Produce a comprehensive Digital Maturity Index:

1. SUMMARY: 2-3 sentence overview of digital maturity across the organization.
2. PILLARS (5-7): For each maturity pillar — pillar name, current level (1-5 numeric), target level (1-5 numeric), gap description, estimated investment, and timeline.
3. OVERALL SCORE (numeric 0-100): Overall digital maturity score.
4. INDUSTRY AVG (numeric 0-100): Industry average digital maturity score for comparison.
5. TOP GAP: Pillar with the largest maturity gap.
...
${schema}`;
  ...
  try {
    console.log("[Pivot] Generating Digital Maturity Index (Wave 46)...");
    const result = await callJson(genai, prompt);
    return result as unknown as DigitalMaturityIndex;
  } catch (e) {
    console.warn("[Pivot] Digital Maturity Index (Wave 46) synthesis failed:", e);
    return null;
  }
}

export async function synthesizeCloudMigrationReadiness(
  packet: BusinessPacket,
  questionnaire: Questionnaire
): Promise<CloudMigrationReadiness | null> {
  const apiKey = process.env.GEMINI_API_KEY;
  if (!apiKey) return null;
  ...
  const schema = `{
  "summary": "2-3 sentence overview of cloud migration readiness and strategy",
  "workloads": [{
    "workload": "Workload or application name",
    "currentState": "Current hosting state (e.g., on-premise, legacy, partial cloud)",
    "targetState": "Target cloud state (e.g., IaaS, PaaS, SaaS, serverless)",
    "complexity": "Migration complexity (high, medium, low)",
    "estimatedCost": "Estimated migration cost",
    "timeline": "Expected migration timeline"
  }],
  "overallReadiness": "Overall cloud migration readiness level",
  "totalCost": "Total estimated migration cost across all workloads",
  "riskLevel": "Overall migration risk level",
  "quickWins": ["quick win 1", "..."],
  "recommendations": ["recommendation 1", "..."]
}`;

  const prompt = `You are a cloud strategy consultant specializing in cloud migration readiness assessment, workload analysis, migration planning, and cloud cost optimization.
...
Produce a comprehensive Cloud Migration Readiness assessment:

1. SUMMARY: 2-3 sentence overview of cloud migration readiness and strategy.
2. WORKLOADS (5-7): For each workload — workload name, current state, target state, complexity, estimated cost, and timeline.
3. OVERALL READINESS: Overall cloud migration readiness level (e.g., "Ready", "Partially Ready", "Not Ready").
4. TOTAL COST: Total estimated migration cost across all workloads.
5. RISK LEVEL: Overall migration risk level.
...
${schema}`;
  ...
  try {
    console.log("[Pivot] Generating Cloud Migration Readiness (Wave 46)...");
    const result = await callJson(genai, prompt);
    return result as unknown as CloudMigrationReadiness;
  } catch (e) {
    console.warn("[Pivot] Cloud Migration Readiness (Wave 46) synthesis failed:", e);
    return null;
  }
}

export async function synthesizeAutomationRoi(
  packet: BusinessPacket,
  questionnaire: Questionnaire
): Promise<AutomationRoi | null> {
  const apiKey = process.env.GEMINI_API_KEY;
  if (!apiKey) return null;
  ...
  const schema = `{
  "summary": "2-3 sentence overview of automation opportunities and expected ROI",
  "opportunities": [{
    "process": "Process or task name to automate",
    "currentEffort": "Current effort or time spent on this process",
    "automationPotential": "Automation potential (full, partial, augmented)",
    "estimatedSavings": "Estimated annual savings from automation",
    "implementationCost": "Estimated implementation cost",
    "roiTimeline": "Timeline to achieve positive ROI"
  }],
  "totalSavings": "Total estimated annual savings across all opportunities",
  "totalInvestment": "Total estimated investment required",
  "overallRoi": "Overall ROI percentage or multiple",
  "topOpportunity": "Single highest-impact automation opportunity",
  "recommendations": ["recommendation 1", "..."]
}`;

  const prompt = `You are an automation strategist specializing in process automation assessment, ROI analysis, intelligent automation roadmapping, and operational efficiency optimization.
...
Produce a comprehensive Automation ROI analysis:

1. SUMMARY: 2-3 sentence overview of automation opportunities and expected ROI.
2. OPPORTUNITIES (5-7): For each automation candidate — process name, current effort, automation potential, estimated savings, implementation cost, and ROI timeline.
3. TOTAL SAVINGS: Total estimated annual savings across all opportunities.
4. TOTAL INVESTMENT: Total estimated investment required for automation.
5. OVERALL ROI: Overall ROI percentage or multiple across all opportunities.
...
${schema}`;
  ...
  try {
    console.log("[Pivot] Generating Automation ROI (Wave 46)...");
    const result = await callJson(genai, prompt);
    return result as unknown as AutomationRoi;
  } catch (e) {
    console.warn("[Pivot] Automation ROI (Wave 46) synthesis failed:", e);
    return null;
  }
}

export async function synthesizeDigitalWorkplace(
  packet: BusinessPacket,
  questionnaire: Questionnaire
): Promise<DigitalWorkplace | null> {
  const apiKey = process.env.GEMINI_API_KEY;
  if (!apiKey) return null;
  ...
  const schema = `{
  "summary": "2-3 sentence overview of digital workplace capabilities and employee experience",
  "capabilities": [{
    "capability": "Digital workplace capability (e.g., collaboration tools, knowledge management, remote work)",
    "adoption": "Adoption level across the organization",
    "effectiveness": "Effectiveness rating of this capability",
    "gap": "Gap or pain point identified",
    "userSatisfaction": "User satisfaction level",
    "improvement": "Recommended improvement action"
  }],
  "overallAdoption": "Overall digital workplace adoption level",
  "productivityImpact": "Estimated productivity impact of current digital workplace",
  "employeeSatisfaction": "Overall employee satisfaction with digital tools",
  "topGap": "Biggest gap in digital workplace capabilities",
  "recommendations": ["recommendation 1", "..."]
}`;

  const prompt = `You are a digital workplace strategist specializing in employee experience design, collaboration technology assessment, remote work enablement, and workplace productivity optimization.
...
Produce a comprehensive Digital Workplace analysis:

1. SUMMARY: 2-3 sentence overview of digital workplace capabilities and employee experience.
2. CAPABILITIES (5-7): For each capability — capability name, adoption level, effectiveness, gap, user satisfaction, and improvement action.
3. OVERALL ADOPTION: Overall digital workplace adoption level across the organization.
4. PRODUCTIVITY IMPACT: Estimated productivity impact of current digital workplace setup.
5. EMPLOYEE SATISFACTION: Overall employee satisfaction with digital tools and environment.
...
${schema}`;
  ...
  try {
    console.log("[Pivot] Generating Digital Workplace (Wave 46)...");
    const result = await callJson(genai, prompt);
    return result as unknown as DigitalWorkplace;
  } catch (e) {
    console.warn("[Pivot] Digital Workplace (Wave 46) synthesis failed:", e);
    return null;
  }
}

export async function synthesizeCybersecurityPosture(
  packet: BusinessPacket,
  questionnaire: Questionnaire
): Promise<CybersecurityPosture | null> {
  const apiKey = process.env.GEMINI_API_KEY;
  if (!apiKey) return null;
  ...
  const schema = `{
  "summary": "2-3 sentence overview of cybersecurity posture and risk exposure",
  "domains": [{
    "domain": "Security domain (e.g., network security, identity management, data protection, endpoint security)",
    "maturityLevel": "Maturity level (advanced, intermediate, basic, nascent)",
    "riskScore": 0,
    "controls": 0,
    "gaps": "Key gaps or vulnerabilities in this domain",
    "investment": "Recommended investment to address gaps"
  }],
  "overallScore": 0,
  "criticalGaps": 0,
  "complianceStatus": "Overall compliance status with relevant frameworks",
  "incidentReadinessLevel": "Level of readiness for security incidents",
  "recommendations": ["recommendation 1", "..."]
}`;

  const prompt = `You are a cybersecurity assessment specialist focusing on security posture evaluation, risk quantification, compliance gap analysis, and incident readiness planning.
...
Produce a comprehensive Cybersecurity Posture assessment:

1. SUMMARY: 2-3 sentence overview of cybersecurity posture and risk exposure.
2. DOMAINS (5-7): For each security domain — domain name, maturity level, risk score (0-100 numeric), number of controls, key gaps, and recommended investment.
3. OVERALL SCORE (numeric 0-100): Overall cybersecurity posture score.
4. CRITICAL GAPS (number): Number of critical security gaps requiring immediate attention.
5. COMPLIANCE STATUS: Overall compliance status with relevant security frameworks.
...
${schema}`;
  ...
  try {
    console.log("[Pivot] Generating Cybersecurity Posture (Wave 46)...");
    const result = await callJson(genai, prompt);
    return result as unknown as CybersecurityPosture;
  } catch (e) {
    console.warn("[Pivot] Cybersecurity Posture (Wave 46) synthesis failed:", e);
    return null;
  }
}

export async function synthesizeTechVendorConsolidation(
  packet: BusinessPacket,
  questionnaire: Questionnaire
): Promise<TechVendorConsolidation | null> {
  const apiKey = process.env.GEMINI_API_KEY;
  if (!apiKey) return null;
  ...
  const schema = `{
  "summary": "2-3 sentence overview of technology vendor landscape and consolidation opportunities",
  "vendors": [{
    "vendor": "Vendor name",
    "category": "Vendor category (e.g., CRM, marketing, infrastructure, analytics)",
    "annualSpend": "Annual spend with this vendor",
    "overlap": "Overlap with other vendors or tools",
    "consolidationPotential": "Consolidation potential (high, medium, low, none)",
    "recommendation": "Specific recommendation for this vendor"
  }],
  "totalSpend": "Total annual technology vendor spend",
  "potentialSavings": "Total potential savings from consolidation",
  "redundantTools": 0,
  "consolidationPlan": "High-level consolidation plan summary",
  "recommendations": ["recommendation 1", "..."]
}`;

  const prompt = `You are a technology procurement strategist specializing in vendor landscape analysis, tool consolidation, spend optimization, and vendor relationship management.
...
Produce a comprehensive Tech Vendor Consolidation analysis:

1. SUMMARY: 2-3 sentence overview of technology vendor landscape and consolidation opportunities.
2. VENDORS (6-8): For each vendor — vendor name, category, annual spend, overlap with others, consolidation potential, and specific recommendation.
3. TOTAL SPEND: Total annual technology vendor spend.
4. POTENTIAL SAVINGS: Total potential savings from vendor consolidation.
5. REDUNDANT TOOLS (number): Number of redundant or overlapping tools identified.
...
${schema}`;
  ...
  try {
    console.log("[Pivot] Generating Tech Vendor Consolidation (Wave 46)...");
    const result = await callJson(genai, prompt);
    return result as unknown as TechVendorConsolidation;
  } catch (e) {
    console.warn("[Pivot] Tech Vendor Consolidation (Wave 46) synthesis failed:", e);
    return null;
  }
}
...
export async function synthesizeRevenueSourceMapping(
  packet: BusinessPacket,
  questionnaire: Questionnaire
): Promise<RevenueSourceMapping | null> {
  const apiKey = process.env.GEMINI_API_KEY;
  if (!apiKey) return null;
  ...
  const schema = `{
  "summary": "2-3 sentence overview of revenue source composition and health",
  "sources": [{
    "source": "Revenue source name (e.g., product line, service, segment)",
    "contribution": "Revenue contribution percentage or amount",
    "growth": "Growth rate for this source",
    "margin": "Margin for this revenue source",
    "reliability": "Reliability of this source (recurring, semi-recurring, one-time)",
    "optimization": "Key optimization opportunity for this source"
  }],
  "topSource": "Largest revenue source by contribution",
  "fastestGrowing": "Fastest growing revenue source",
  "concentrationRisk": "Revenue concentration risk assessment",
  "diversificationScore": "Revenue diversification score or rating",
  "recommendations": ["recommendation 1", "..."]
}`;

  const prompt = `You are a revenue strategist specializing in revenue stream analysis, source mapping, concentration risk assessment, and revenue diversification planning.
...
Produce a comprehensive Revenue Source Mapping:

1. SUMMARY: 2-3 sentence overview of revenue source composition and health.
2. SOURCES (5-7): For each revenue source — source name, contribution, growth rate, margin, reliability, and optimization opportunity.
3. TOP SOURCE: Largest revenue source by contribution.
4. FASTEST GROWING: Fastest growing revenue source.
5. CONCENTRATION RISK: Revenue concentration risk assessment.
...
${schema}`;
  ...
  try {
    console.log("[Pivot] Generating Revenue Source Mapping (Wave 47)...");
    const result = await callJson(genai, prompt);
    return result as unknown as RevenueSourceMapping;
  } catch (e) {
    console.warn("[Pivot] Revenue Source Mapping (Wave 47) synthesis failed:", e);
    return null;
  }
}

export async function synthesizeChannelMixOptimization(
  packet: BusinessPacket,
  questionnaire: Questionnaire
): Promise<ChannelMixOptimization | null> {
  const apiKey = process.env.GEMINI_API_KEY;
  if (!apiKey) return null;
  ...
  const schema = `{
  "summary": "2-3 sentence overview of channel mix performance and optimization opportunities",
  "channels": [{
    "channel": "Channel name (e.g., direct sales, online, partner, marketplace)",
    "revenue": "Revenue generated through this channel",
    "cost": "Cost of this channel",
    "roi": "ROI for this channel",
    "growth": "Growth rate of this channel",
    "saturation": "Saturation level of this channel"
  }],
  "topChannel": "Best performing channel by ROI",
  "underperforming": "Most underperforming channel relative to investment",
  "optimalMix": "Recommended optimal channel mix allocation",
  "reallocationPotential": "Revenue potential from reallocating channel investment",
  "recommendations": ["recommendation 1", "..."]
}`;

  const prompt = `You are a channel strategy consultant specializing in channel mix analysis, performance optimization, multi-channel strategy, and resource allocation across sales and marketing channels.
...
Produce a comprehensive Channel Mix Optimization analysis:

1. SUMMARY: 2-3 sentence overview of channel mix performance and optimization opportunities.
2. CHANNELS (5-7): For each channel — channel name, revenue, cost, ROI, growth rate, and saturation level.
3. TOP CHANNEL: Best performing channel by ROI.
4. UNDERPERFORMING: Most underperforming channel relative to investment.
5. OPTIMAL MIX: Recommended optimal channel mix allocation.
...
${schema}`;
  ...
  try {
    console.log("[Pivot] Generating Channel Mix Optimization (Wave 47)...");
    const result = await callJson(genai, prompt);
    return result as unknown as ChannelMixOptimization;
  } catch (e) {
    console.warn("[Pivot] Channel Mix Optimization (Wave 47) synthesis failed:", e);
    return null;
  }
}

export async function synthesizeCrossSellEngine(
  packet: BusinessPacket,
  questionnaire: Questionnaire
): Promise<CrossSellEngine | null> {
  const apiKey = process.env.GEMINI_API_KEY;
  if (!apiKey) return null;
  ...
  const schema = `{
  "summary": "2-3 sentence overview of cross-sell and upsell opportunities",
  "opportunities": [{
    "product": "Product or service to cross-sell/upsell",
    "targetSegment": "Target customer segment for this opportunity",
    "probability": "Conversion probability estimate",
    "estimatedRevenue": "Estimated incremental revenue",
    "effort": "Implementation effort (high, medium, low)",
    "trigger": "Trigger or signal indicating readiness to buy"
  }],
  "totalPotential": "Total revenue potential from all cross-sell/upsell opportunities",
  "topOpportunity": "Single highest-value cross-sell opportunity",
  "conversionRate": "Expected overall conversion rate for cross-sell initiatives",
  "implementationPlan": "High-level implementation plan for cross-sell program",
  "recommendations": ["recommendation 1", "..."]
}`;

  const prompt = `You are a revenue growth specialist focusing on cross-sell and upsell strategy, customer expansion analytics, product bundling, and customer lifecycle revenue optimization.
...
Produce a comprehensive Cross-Sell Engine analysis:

1. SUMMARY: 2-3 sentence overview of cross-sell and upsell opportunities.
2. OPPORTUNITIES (5-7): For each opportunity — product/service, target segment, conversion probability, estimated revenue, effort level, and buying trigger.
3. TOTAL POTENTIAL: Total revenue potential from all identified cross-sell/upsell opportunities.
4. TOP OPPORTUNITY: Single highest-value cross-sell opportunity.
5. CONVERSION RATE: Expected overall conversion rate for cross-sell initiatives.
...
${schema}`;
  ...
  try {
    console.log("[Pivot] Generating Cross-Sell Engine (Wave 47)...");
    const result = await callJson(genai, prompt);
    return result as unknown as CrossSellEngine;
  } catch (e) {
    console.warn("[Pivot] Cross-Sell Engine (Wave 47) synthesis failed:", e);
    return null;
  }
}

export async function synthesizePriceOptimizationModel(
  packet: BusinessPacket,
  questionnaire: Questionnaire
): Promise<PriceOptimizationModel | null> {
  const apiKey = process.env.GEMINI_API_KEY;
  if (!apiKey) return null;
  ...
  const schema = `{
  "summary": "2-3 sentence overview of pricing optimization scenarios and revenue impact",
  "scenarios": [{
    "scenario": "Pricing scenario description",
    "priceChange": "Proposed price change (e.g., +10%, -5%, restructure)",
    "volumeImpact": "Expected volume impact from this price change",
    "revenueImpact": "Expected revenue impact",
    "marginImpact": "Expected margin impact",
    "risk": "Risk level associated with this scenario"
  }],
  "currentMargin": "Current overall margin",
  "optimalPrice": "Recommended optimal price point or structure",
  "elasticity": "Overall price elasticity assessment",
  "competitivePosition": "Current competitive pricing position",
  "recommendations": ["recommendation 1", "..."]
}`;

  const prompt = `You are a pricing optimization specialist focusing on price modeling, scenario analysis, margin optimization, and competitive pricing strategy.
...
Produce a comprehensive Price Optimization Model:

1. SUMMARY: 2-3 sentence overview of pricing optimization scenarios and revenue impact.
2. SCENARIOS (4-6): For each pricing scenario — scenario description, price change, volume impact, revenue impact, margin impact, and risk level.
3. CURRENT MARGIN: Current overall margin.
4. OPTIMAL PRICE: Recommended optimal price point or structure.
5. ELASTICITY: Overall price elasticity assessment.
...
${schema}`;
  ...
  try {
    console.log("[Pivot] Generating Price Optimization Model (Wave 47)...");
    const result = await callJson(genai, prompt);
    return result as unknown as PriceOptimizationModel;
  } catch (e) {
    console.warn("[Pivot] Price Optimization Model (Wave 47) synthesis failed:", e);
    return null;
  }
}

export async function synthesizePromotionEffectiveness(
  packet: BusinessPacket,
  questionnaire: Questionnaire
): Promise<PromotionEffectiveness | null> {
  const apiKey = process.env.GEMINI_API_KEY;
  if (!apiKey) return null;
  ...
  const schema = `{
  "summary": "2-3 sentence overview of promotion and campaign effectiveness",
  "campaigns": [{
    "campaign": "Campaign or promotion name/type",
    "type": "Campaign type (e.g., discount, bundle, seasonal, referral)",
    "cost": "Total cost of the campaign",
    "revenue": "Revenue generated by the campaign",
    "roi": "ROI of the campaign",
    "incrementalLift": "Incremental revenue lift attributable to the campaign"
  }],
  "totalSpend": "Total promotional spend across all campaigns",
  "avgRoi": "Average ROI across all promotions",
  "topCampaign": "Best performing campaign by ROI",
  "wastedSpend": "Estimated wasted promotional spend",
  "recommendations": ["recommendation 1", "..."]
}`;

  const prompt = `You are a marketing effectiveness analyst specializing in promotion ROI analysis, campaign performance measurement, marketing spend optimization, and incrementality testing.
...
Produce a comprehensive Promotion Effectiveness analysis:

1. SUMMARY: 2-3 sentence overview of promotion and campaign effectiveness.
2. CAMPAIGNS (5-7): For each campaign — campaign name/type, type, cost, revenue, ROI, and incremental lift.
3. TOTAL SPEND: Total promotional spend across all campaigns.
4. AVG ROI: Average ROI across all promotions.
5. TOP CAMPAIGN: Best performing campaign by ROI.
...
${schema}`;
  ...
  try {
    console.log("[Pivot] Generating Promotion Effectiveness (Wave 47)...");
    const result = await callJson(genai, prompt);
    return result as unknown as PromotionEffectiveness;
  } catch (e) {
    console.warn("[Pivot] Promotion Effectiveness (Wave 47) synthesis failed:", e);
    return null;
  }
}

export async function synthesizeRevenueHealthIndex(
  packet: BusinessPacket,
  questionnaire: Questionnaire
): Promise<RevenueHealthIndex | null> {
  const apiKey = process.env.GEMINI_API_KEY;
  if (!apiKey) return null;
  ...
  const schema = `{
  "summary": "2-3 sentence overview of overall revenue health and quality",
  "metrics": [{
    "metric": "Revenue health metric name (e.g., recurring revenue ratio, revenue growth rate, churn impact)",
    "value": "Current value of this metric",
    "benchmark": "Industry benchmark for this metric",
    "trend": "Trend direction (improving, stable, declining)",
    "risk": "Risk level for this metric (critical, high, medium, low)",
    "action": "Recommended action to improve this metric"
  }],
  "overallScore": 0,
  "topRisk": "Single biggest risk to revenue health",
  "growthTrajectory": "Revenue growth trajectory assessment",
  "qualityScore": "Revenue quality score or rating",
  "recommendations": ["recommendation 1", "..."]
}`;

  const prompt = `You are a revenue analytics specialist focusing on revenue health assessment, quality metrics, growth trajectory analysis, and revenue risk identification.
...
Produce a comprehensive Revenue Health Index:

1. SUMMARY: 2-3 sentence overview of overall revenue health and quality.
2. METRICS (6-8): For each revenue health metric — metric name, current value, industry benchmark, trend direction, risk level, and recommended action.
3. OVERALL SCORE (numeric 0-100): Overall revenue health score.
4. TOP RISK: Single biggest risk to revenue health.
5. GROWTH TRAJECTORY: Revenue growth trajectory assessment.
...
${schema}`;
  ...
  try {
    console.log("[Pivot] Generating Revenue Health Index (Wave 47)...");
    const result = await callJson(genai, prompt);
    return result as unknown as RevenueHealthIndex;
  } catch (e) {
    console.warn("[Pivot] Revenue Health Index (Wave 47) synthesis failed:", e);
    return null;
  }
}
...
export async function synthesizeOrganizationalNetwork(
  packet: BusinessPacket,
  questionnaire: Questionnaire
): Promise<OrganizationalNetwork | null> {
  const apiKey = process.env.GEMINI_API_KEY;
  if (!apiKey) return null;
  ...
  const schema = `{
  "summary": "2-3 sentence overview of organizational network health and collaboration patterns",
  "nodes": [{
    "team": "Team or department name",
    "connections": 0,
    "centrality": "Centrality in the organizational network (hub, connector, peripheral)",
    "collaboration": "Collaboration effectiveness assessment",
    "bottleneck": false,
    "influence": "Influence level within the organization"
  }],
  "overallConnectivity": "Overall organizational connectivity assessment",
  "siloRisk": "Risk of organizational silos",
  "keyConnectors": ["key connector 1", "..."],
  "collaborationScore": 0,
  "recommendations": ["recommendation 1", "..."]
}`;

  const prompt = `You are an organizational network analyst specializing in organizational network analysis, collaboration mapping, silo detection, and cross-functional connectivity optimization.
...
Produce a comprehensive Organizational Network analysis:

1. SUMMARY: 2-3 sentence overview of organizational network health and collaboration patterns.
2. NODES (5-7): For each team/department — team name, number of cross-functional connections (numeric), centrality level, collaboration effectiveness, whether it is a bottleneck (boolean), and influence level.
3. OVERALL CONNECTIVITY: Overall organizational connectivity assessment.
4. SILO RISK: Risk of organizational silos forming or existing.
5. KEY CONNECTORS (3-5): Teams or individuals that serve as critical connectors.
...
${schema}`;
  ...
  try {
    console.log("[Pivot] Generating Organizational Network (Wave 48)...");
    const result = await callJson(genai, prompt);
    return result as unknown as OrganizationalNetwork;
  } catch (e) {
    console.warn("[Pivot] Organizational Network (Wave 48) synthesis failed:", e);
    return null;
  }
}

export async function synthesizeDecisionEfficiency(
  packet: BusinessPacket,
  questionnaire: Questionnaire
): Promise<DecisionEfficiency | null> {
  const apiKey = process.env.GEMINI_API_KEY;
  if (!apiKey) return null;
  ...
  const schema = `{
  "summary": "2-3 sentence overview of decision-making efficiency and governance",
  "areas": [{
    "area": "Decision area (e.g., strategic, operational, financial, hiring)",
    "avgDecisionTime": "Average time to reach a decision in this area",
    "stakeholders": 0,
    "bottleneck": "Primary bottleneck in the decision process",
    "impact": "Impact of slow decisions in this area",
    "improvement": "Recommended improvement for decision speed"
  }],
  "overallVelocity": "Overall decision velocity assessment",
  "slowestArea": "Area with the slowest decision-making process",
  "delegationScore": "Effectiveness of delegation in decision-making",
  "dataUsage": "How effectively data is used in decision-making",
  "recommendations": ["recommendation 1", "..."]
}`;

  const prompt = `You are an organizational efficiency consultant specializing in decision-making process analysis, governance optimization, decision velocity improvement, and delegation effectiveness.
...
Produce a comprehensive Decision Efficiency analysis:

1. SUMMARY: 2-3 sentence overview of decision-making efficiency and governance.
2. AREAS (5-7): For each decision area — area name, average decision time, number of stakeholders (numeric), primary bottleneck, impact of delays, and improvement recommendation.
3. OVERALL VELOCITY: Overall decision velocity assessment.
4. SLOWEST AREA: Area with the slowest decision-making process.
5. DELEGATION SCORE: Effectiveness of delegation in decision-making.
...
${schema}`;
  ...
  try {
    console.log("[Pivot] Generating Decision Efficiency (Wave 48)...");
    const result = await callJson(genai, prompt);
    return result as unknown as DecisionEfficiency;
  } catch (e) {
    console.warn("[Pivot] Decision Efficiency (Wave 48) synthesis failed:", e);
    return null;
  }
}

export async function synthesizeMeetingEfficiency(
  packet: BusinessPacket,
  questionnaire: Questionnaire
): Promise<MeetingEfficiency | null> {
  const apiKey = process.env.GEMINI_API_KEY;
  if (!apiKey) return null;
  ...
  const schema = `{
  "summary": "2-3 sentence overview of meeting culture and efficiency",
  "categories": [{
    "category": "Meeting category (e.g., status updates, planning, brainstorming, 1:1s, all-hands)",
    "hoursPerWeek": "Hours per week spent in this category of meetings",
    "attendees": 0,
    "effectiveness": "Effectiveness rating (highly effective, effective, neutral, ineffective)",
    "actionItemRate": "Rate at which meetings produce actionable outcomes",
    "recommendation": "Specific recommendation for this meeting category"
  }],
  "totalHoursPerWeek": "Total hours per week spent in meetings across the organization",
  "effectiveRate": "Percentage of meetings rated as effective",
  "reclaimableHours": "Estimated hours per week that could be reclaimed",
  "topIssue": "Biggest meeting efficiency issue",
  "recommendations": ["recommendation 1", "..."]
}`;

  const prompt = `You are a workplace productivity consultant specializing in meeting effectiveness analysis, time optimization, meeting culture assessment, and organizational productivity improvement.
...
Produce a comprehensive Meeting Efficiency analysis:

1. SUMMARY: 2-3 sentence overview of meeting culture and efficiency.
2. CATEGORIES (5-7): For each meeting category — category name, hours per week, average attendees (numeric), effectiveness rating, action item rate, and specific recommendation.
3. TOTAL HOURS PER WEEK: Total hours per week spent in meetings across the organization.
4. EFFECTIVE RATE: Percentage of meetings rated as effective.
5. RECLAIMABLE HOURS: Estimated hours per week that could be reclaimed through optimization.
...
${schema}`;
  ...
  try {
    console.log("[Pivot] Generating Meeting Efficiency (Wave 48)...");
    const result = await callJson(genai, prompt);
    return result as unknown as MeetingEfficiency;
  } catch (e) {
    console.warn("[Pivot] Meeting Efficiency (Wave 48) synthesis failed:", e);
    return null;
  }
}

export async function synthesizeKnowledgeCapital(
  packet: BusinessPacket,
  questionnaire: Questionnaire
): Promise<KnowledgeCapital | null> {
  const apiKey = process.env.GEMINI_API_KEY;
  if (!apiKey) return null;
  ...
  const schema = `{
  "summary": "2-3 sentence overview of organizational knowledge capital and management",
  "areas": [{
    "area": "Knowledge area (e.g., technical expertise, customer knowledge, process knowledge, industry expertise)",
    "documentation": "Documentation quality and coverage",
    "accessibility": "Accessibility of knowledge in this area",
    "freshness": "How current the knowledge is",
    "singlePointRisk": "Risk of knowledge loss if key individuals leave",
    "improvement": "Recommended improvement action"
  }],
  "overallScore": 0,
  "criticalGaps": 0,
  "documentationCoverage": "Overall documentation coverage assessment",
  "knowledgeSharingIndex": "Overall knowledge sharing effectiveness",
  "recommendations": ["recommendation 1", "..."]
}`;

  const prompt = `You are a knowledge management specialist focusing on intellectual capital assessment, knowledge capture and sharing, documentation analysis, and organizational learning optimization.
...
Produce a comprehensive Knowledge Capital analysis:

1. SUMMARY: 2-3 sentence overview of organizational knowledge capital and management.
2. AREAS (5-7): For each knowledge area — area name, documentation quality, accessibility, freshness, single-point-of-failure risk, and improvement recommendation.
3. OVERALL SCORE (numeric 0-100): Overall knowledge capital score.
4. CRITICAL GAPS (number): Number of critical knowledge gaps identified.
5. DOCUMENTATION COVERAGE: Overall documentation coverage assessment.
...
${schema}`;
  ...
  try {
    console.log("[Pivot] Generating Knowledge Capital (Wave 48)...");
    const result = await callJson(genai, prompt);
    return result as unknown as KnowledgeCapital;
  } catch (e) {
    console.warn("[Pivot] Knowledge Capital (Wave 48) synthesis failed:", e);
    return null;
  }
}

export async function synthesizeChangeManagementScore(
  packet: BusinessPacket,
  questionnaire: Questionnaire
): Promise<ChangeManagementScore | null> {
  const apiKey = process.env.GEMINI_API_KEY;
  if (!apiKey) return null;
  ...
  const schema = `{
  "summary": "2-3 sentence overview of change management readiness and capability",
  "areas": [{
    "area": "Change readiness area (e.g., leadership alignment, communication, training, stakeholder buy-in)",
    "readinessLevel": "Readiness level for change in this area (ready, partially ready, not ready)",
    "resistance": "Level of resistance expected",
    "leadership": "Leadership support level for change in this area",
    "communication": "Communication effectiveness regarding change",
    "training": "Training readiness and capability"
  }],
  "overallReadiness": "Overall change management readiness assessment",
  "biggestBarrier": "Single biggest barrier to successful change",
  "changeFatigue": "Level of change fatigue in the organization",
  "successRate": "Historical or estimated change initiative success rate",
  "recommendations": ["recommendation 1", "..."]
}`;

  const prompt = `You are a change management consultant specializing in organizational change readiness assessment, resistance analysis, change capability building, and transformation success planning.
...
Produce a comprehensive Change Management Score:

1. SUMMARY: 2-3 sentence overview of change management readiness and capability.
2. AREAS (5-7): For each change readiness area — area name, readiness level, expected resistance, leadership support, communication effectiveness, and training readiness.
3. OVERALL READINESS: Overall change management readiness assessment.
4. BIGGEST BARRIER: Single biggest barrier to successful change implementation.
5. CHANGE FATIGUE: Level of change fatigue in the organization.
...
${schema}`;
  ...
  try {
    console.log("[Pivot] Generating Change Management Score (Wave 48)...");
    const result = await callJson(genai, prompt);
    return result as unknown as ChangeManagementScore;
  } catch (e) {
    console.warn("[Pivot] Change Management Score (Wave 48) synthesis failed:", e);
    return null;
  }
}

export async function synthesizeCultureAlignment(
  packet: BusinessPacket,
  questionnaire: Questionnaire
): Promise<CultureAlignment | null> {
  const apiKey = process.env.GEMINI_API_KEY;
  if (!apiKey) return null;
  ...
  const schema = `{
  "summary": "2-3 sentence overview of culture alignment between stated values and actual practices",
  "dimensions": [{
    "dimension": "Culture dimension (e.g., innovation, accountability, collaboration, customer focus, transparency)",
    "currentState": "Current state of this culture dimension",
    "desiredState": "Desired or aspirational state",
    "gap": "Gap between current and desired state",
    "sentiment": "Employee sentiment regarding this dimension",
    "initiative": "Recommended initiative to close the gap"
  }],
  "overallAlignment": 0,
  "strongestDimension": "Culture dimension with the strongest alignment",
  "biggestMisalignment": "Culture dimension with the largest gap",
  "engagementCorrelation": "Correlation between culture alignment and employee engagement",
  "recommendations": ["recommendation 1", "..."]
}`;

  const prompt = `You are an organizational culture consultant specializing in culture assessment, values alignment analysis, employee sentiment evaluation, and culture transformation planning.
...
Produce a comprehensive Culture Alignment analysis:

1. SUMMARY: 2-3 sentence overview of culture alignment between stated values and actual practices.
2. DIMENSIONS (5-7): For each culture dimension — dimension name, current state, desired state, gap, employee sentiment, and recommended initiative.
3. OVERALL ALIGNMENT (numeric 0-100): Overall culture alignment score.
4. STRONGEST DIMENSION: Culture dimension with the strongest alignment between stated and practiced values.
5. BIGGEST MISALIGNMENT: Culture dimension with the largest gap between stated and practiced values.
...
${schema}`;
  ...
  try {
    console.log("[Pivot] Generating Culture Alignment (Wave 48)...");
    const result = await callJson(genai, prompt);
    return result as unknown as CultureAlignment;
  } catch (e) {
    console.warn("[Pivot] Culture Alignment (Wave 48) synthesis failed:", e);
    return null;
  }
}
...
export async function synthesizePartnerPerformance(
  packet: BusinessPacket,
  questionnaire: Questionnaire
  ...
  const schema = `{
  "summary": "2-3 sentence overview of partner performance across the ecosystem",
  "partners": [{
    "partner": "Partner name",
    "type": "Partner type (e.g., reseller, technology, integration, referral)",
    "revenue": "Revenue contribution from this partner",
    "growth": "Revenue growth rate for this partner",
    "satisfaction": "Partner satisfaction score or rating",
    "tier": "Partner tier level"
  }],
  "totalPartnerRevenue": "Total revenue generated through partners",
  "topPartner": "Highest-performing partner by revenue",
  "avgSatisfaction": "Average partner satisfaction score",
  "churnRisk": "Partners at risk of churning or disengaging",
  "recommendations": ["recommendation 1", "..."]
}`;

  const prompt = `You are a partner ecosystem strategist specializing in channel management, alliance performance, partner program optimization, and ecosystem revenue analysis.
...
Produce a comprehensive Partner Performance analysis:

1. SUMMARY: 2-3 sentence overview of partner performance and ecosystem health.
2. PARTNERS (5-8): For each partner — partner name, type, revenue contribution, growth rate, satisfaction score, and tier level.
3. TOTAL PARTNER REVENUE: Total revenue generated through all partner channels.
4. TOP PARTNER: Highest-performing partner by revenue contribution.
5. AVG SATISFACTION: Average partner satisfaction score across the ecosystem.
...
${schema}`;
  ...
  try {
    console.log("[Pivot] Generating Partner Performance (Wave 49)...");
    const result = await callJson(genai, prompt);
    return result as unknown as PartnerPerformance;
  } catch (e) {
    console.warn("[Pivot] Partner Performance (Wave 49) synthesis failed:", e);
    return null;
  }
}

export async function synthesizeEcosystemMapping(
  packet: BusinessPacket,
  questionnaire: Questionnaire
): Promise<EcosystemMapping | null> {
  const apiKey = process.env.GEMINI_API_KEY;
  if (!apiKey) return null;
  ...
  const schema = `{
  "summary": "2-3 sentence overview of the business ecosystem and key players",
  "players": [{
    "player": "Entity name",
    "role": "Role in ecosystem (e.g., supplier, competitor, complementor, regulator)",
    "relationship": "Nature of relationship with the business",
    "value": "Value this entity brings to the ecosystem",
    "dependency": "Level of dependency on this entity",
    "opportunity": "Untapped opportunity with this entity"
  }],
  "ecosystemSize": "Estimated total size of the ecosystem",
  "networkEffect": "Strength and type of network effects present",
  "keyDependency": "Most critical ecosystem dependency",
  "whitespace": "Largest whitespace opportunity in the ecosystem",
  "recommendations": ["recommendation 1", "..."]
}`;

  const prompt = `You are an ecosystem strategist specializing in business network mapping, value chain analysis, ecosystem dynamics, and strategic positioning within interconnected markets.
...
Produce a comprehensive Ecosystem Mapping analysis:

1. SUMMARY: 2-3 sentence overview of the business ecosystem landscape.
2. PLAYERS (5-8): For each ecosystem entity — name, role, relationship, value, dependency level, and untapped opportunity.
3. ECOSYSTEM SIZE: Estimated total addressable size of the ecosystem.
4. NETWORK EFFECT: Assessment of network effects strength and type.
5. KEY DEPENDENCY: Most critical dependency within the ecosystem.
...
${schema}`;
  ...
  try {
    console.log("[Pivot] Generating Ecosystem Mapping (Wave 49)...");
    const result = await callJson(genai, prompt);
    return result as unknown as EcosystemMapping;
  } catch (e) {
    console.warn("[Pivot] Ecosystem Mapping (Wave 49) synthesis failed:", e);
    return null;
  }
}

export async function synthesizeAllianceStrategy(
  packet: BusinessPacket,
  questionnaire: Questionnaire
): Promise<AllianceStrategy | null> {
  const apiKey = process.env.GEMINI_API_KEY;
  if (!apiKey) return null;
  ...
  const schema = `{
  "summary": "2-3 sentence overview of alliance strategy and opportunities",
  "opportunities": [{
    "partner": "Potential alliance partner name",
    "type": "Alliance type (e.g., strategic, technology, distribution, co-marketing)",
    "synergy": "Expected synergy or value from alliance",
    "effort": "Effort required to establish the alliance",
    "expectedValue": "Expected revenue or strategic value",
    "timeline": "Timeline to realize value from the alliance"
  }],
  "topAlliance": "Most promising alliance opportunity",
  "totalSynergy": "Total estimated synergy value across all opportunities",
  "currentAlliances": 0,
  "successRate": "Historical or estimated alliance success rate",
  "recommendations": ["recommendation 1", "..."]
}`;

  const prompt = `You are a strategic alliances consultant specializing in partnership identification, alliance structuring, synergy valuation, and go-to-market partnership strategies.
...
Produce a comprehensive Alliance Strategy analysis:

1. SUMMARY: 2-3 sentence overview of alliance strategy and partnership landscape.
2. OPPORTUNITIES (5-8): For each alliance opportunity — partner name, type, synergy, effort level, expected value, and timeline.
3. TOP ALLIANCE: Most promising alliance opportunity with highest potential value.
4. TOTAL SYNERGY: Total estimated synergy value across all identified opportunities.
5. CURRENT ALLIANCES (numeric): Number of existing alliances or partnerships.
...
${schema}`;
  ...
  try {
    console.log("[Pivot] Generating Alliance Strategy (Wave 49)...");
    const result = await callJson(genai, prompt);
    return result as unknown as AllianceStrategy;
  } catch (e) {
    console.warn("[Pivot] Alliance Strategy (Wave 49) synthesis failed:", e);
    return null;
  }
}

export async function synthesizeChannelPartnerHealth(
  packet: BusinessPacket,
  questionnaire: Questionnaire
): Promise<ChannelPartnerHealth | null> {
  const apiKey = process.env.GEMINI_API_KEY;
  if (!apiKey) return null;
  ...
  const schema = `{
  "summary": "2-3 sentence overview of channel partner health and performance",
  "partners": [{
    "partner": "Channel partner name",
    "region": "Geographic region or territory",
    "revenue": "Revenue contribution from this partner",
    "pipeline": "Current pipeline value with this partner",
    "certificationLevel": "Partner certification or tier level",
    "health": "Overall health status (Healthy, At Risk, Critical)"
  }],
  "totalChannelRevenue": "Total revenue generated through channel partners",
  "avgHealth": "Average health score across all channel partners",
  "topPerformer": "Highest-performing channel partner",
  "atRiskPartners": 0,
  "recommendations": ["recommendation 1", "..."]
}`;

  const prompt = `You are a channel management expert specializing in channel partner health assessment, partner enablement, territory management, and indirect sales optimization.
...
Produce a comprehensive Channel Partner Health analysis:

1. SUMMARY: 2-3 sentence overview of channel partner ecosystem health.
2. PARTNERS (5-8): For each channel partner — name, region, revenue, pipeline, certification level, and health status.
3. TOTAL CHANNEL REVENUE: Total revenue flowing through channel partners.
4. AVG HEALTH: Average health assessment across the channel partner base.
5. TOP PERFORMER: Highest-performing channel partner by revenue and growth.
...
${schema}`;
  ...
  try {
    console.log("[Pivot] Generating Channel Partner Health (Wave 49)...");
    const result = await callJson(genai, prompt);
    return result as unknown as ChannelPartnerHealth;
  } catch (e) {
    console.warn("[Pivot] Channel Partner Health (Wave 49) synthesis failed:", e);
    return null;
  }
}

export async function synthesizeCoSellingPipeline(
  packet: BusinessPacket,
  questionnaire: Questionnaire
): Promise<CoSellingPipeline | null> {
  const apiKey = process.env.GEMINI_API_KEY;
  if (!apiKey) return null;
  ...
  const schema = `{
  "summary": "2-3 sentence overview of co-selling pipeline and partner-driven deals",
  "deals": [{
    "deal": "Deal name or account",
    "partner": "Co-selling partner name",
    "value": "Deal value",
    "stage": "Current pipeline stage",
    "winProbability": "Estimated win probability",
    "closeDate": "Expected close date"
  }],
  "totalPipeline": "Total co-selling pipeline value",
  "avgDealSize": "Average deal size in co-selling pipeline",
  "winRate": "Historical or estimated co-sell win rate",
  "topDeal": "Largest or most promising co-sell deal",
  "recommendations": ["recommendation 1", "..."]
}`;

  const prompt = `You are a co-selling and partner sales strategist specializing in joint pipeline management, partner-driven revenue, collaborative deal execution, and co-sell program optimization.
...
Produce a comprehensive Co-Selling Pipeline analysis:

1. SUMMARY: 2-3 sentence overview of co-selling pipeline health and partner-driven opportunities.
2. DEALS (5-8): For each co-sell deal — deal name, partner, value, stage, win probability, and close date.
3. TOTAL PIPELINE: Total value of the co-selling pipeline.
4. AVG DEAL SIZE: Average deal size in the co-sell pipeline.
5. WIN RATE: Historical or estimated win rate for co-sell deals.
...
${schema}`;
  ...
  try {
    console.log("[Pivot] Generating Co-Selling Pipeline (Wave 49)...");
    const result = await callJson(genai, prompt);
    return result as unknown as CoSellingPipeline;
  } catch (e) {
    console.warn("[Pivot] Co-Selling Pipeline (Wave 49) synthesis failed:", e);
    return null;
  }
}

export async function synthesizeIntegrationMarketplace(
  packet: BusinessPacket,
  questionnaire: Questionnaire
): Promise<IntegrationMarketplace | null> {
  const apiKey = process.env.GEMINI_API_KEY;
  if (!apiKey) return null;
  ...
  const schema = `{
  "summary": "2-3 sentence overview of integration marketplace and third-party ecosystem",
  "integrations": [{
    "integration": "Integration name",
    "category": "Category (e.g., CRM, analytics, payments, communication)",
    "users": "Number of users or adoption level",
    "satisfaction": "User satisfaction rating",
    "revenue": "Revenue generated from this integration",
    "status": "Status (Active, Beta, Planned, Deprecated)"
  }],
  "totalIntegrations": 0,
  "topCategory": "Most popular integration category",
  "revenueFromIntegrations": "Total revenue attributable to integrations",
  "adoptionRate": "Overall integration adoption rate across customer base",
  "recommendations": ["recommendation 1", "..."]
}`;

  const prompt = `You are a platform ecosystem strategist specializing in integration marketplace development, API strategy, developer ecosystem growth, and platform monetization.
...
Produce a comprehensive Integration Marketplace analysis:

1. SUMMARY: 2-3 sentence overview of integration marketplace health and ecosystem.
2. INTEGRATIONS (5-8): For each integration — name, category, user count, satisfaction, revenue, and status.
3. TOTAL INTEGRATIONS (numeric): Total number of integrations available.
4. TOP CATEGORY: Most popular or highest-adoption integration category.
5. REVENUE FROM INTEGRATIONS: Total revenue attributable to integrations or marketplace.
...
${schema}`;
  ...
  try {
    console.log("[Pivot] Generating Integration Marketplace (Wave 49)...");
    const result = await callJson(genai, prompt);
    return result as unknown as IntegrationMarketplace;
  } catch (e) {
    console.warn("[Pivot] Integration Marketplace (Wave 49) synthesis failed:", e);
    return null;
  }
}
...
export async function synthesizeBrandEquityIndex(
  packet: BusinessPacket,
  questionnaire: Questionnaire
): Promise<BrandEquityIndex | null> {
  const apiKey = process.env.GEMINI_API_KEY;
  if (!apiKey) return null;
  ...
  const schema = `{
  "summary": "2-3 sentence overview of brand equity and market positioning",
  "dimensions": [{
    "dimension": "Brand dimension (e.g., awareness, loyalty, perceived quality, associations)",
    "score": 0,
    "benchmark": 0,
    "trend": "Trend direction (improving, stable, declining)",
    "driver": "Key driver of this dimension's performance",
    "action": "Recommended action to improve this dimension"
  }],
  "overallScore": 0,
  "industryRank": "Brand ranking within industry",
  "brandValue": "Estimated brand value",
  "awareness": "Brand awareness level",
  "recommendations": ["recommendation 1", "..."]
}`;

  const prompt = `You are a brand strategist specializing in brand equity measurement, brand valuation, market positioning, and brand perception analysis.
...
Produce a comprehensive Brand Equity Index:

1. SUMMARY: 2-3 sentence overview of overall brand equity and market position.
2. DIMENSIONS (5-7): For each brand dimension — dimension name, score (0-100), benchmark, trend, key driver, and recommended action.
3. OVERALL SCORE (numeric 0-100): Overall brand equity score.
4. INDUSTRY RANK: Brand ranking relative to industry peers.
5. BRAND VALUE: Estimated brand value or equity.
...
${schema}`;
  ...
  try {
    console.log("[Pivot] Generating Brand Equity Index (Wave 50)...");
    const result = await callJson(genai, prompt);
    return result as unknown as BrandEquityIndex;
  } catch (e) {
    console.warn("[Pivot] Brand Equity Index (Wave 50) synthesis failed:", e);
    return null;
  }
}

export async function synthesizeSentimentDashboard(
  packet: BusinessPacket,
  questionnaire: Questionnaire
): Promise<SentimentDashboard | null> {
  const apiKey = process.env.GEMINI_API_KEY;
  if (!apiKey) return null;
  ...
  const schema = `{
  "summary": "2-3 sentence overview of brand sentiment across channels",
  "channels": [{
    "channel": "Channel name (e.g., Twitter, Reviews, Support, Forums)",
    "positivePct": "Percentage of positive sentiment",
    "negativePct": "Percentage of negative sentiment",
    "neutralPct": "Percentage of neutral sentiment",
    "volume": "Total volume of mentions or interactions",
    "trend": "Sentiment trend direction"
  }],
  "overallSentiment": "Overall sentiment classification",
  "netSentimentScore": 0,
  "topPositive": "Most common positive theme",
  "topNegative": "Most common negative theme",
  "recommendations": ["recommendation 1", "..."]
}`;

  const prompt = `You are a sentiment analytics expert specializing in brand perception monitoring, social listening, customer sentiment analysis, and reputation management.
...
Produce a comprehensive Sentiment Dashboard:

1. SUMMARY: 2-3 sentence overview of brand sentiment landscape.
2. CHANNELS (5-7): For each channel — channel name, positive %, negative %, neutral %, volume, and trend.
3. OVERALL SENTIMENT: Overall sentiment classification across all channels.
4. NET SENTIMENT SCORE (numeric -100 to 100): Net sentiment score.
5. TOP POSITIVE: Most common positive sentiment theme or driver.
...
${schema}`;
  ...
  try {
    console.log("[Pivot] Generating Sentiment Dashboard (Wave 50)...");
    const result = await callJson(genai, prompt);
    return result as unknown as SentimentDashboard;
  } catch (e) {
    console.warn("[Pivot] Sentiment Dashboard (Wave 50) synthesis failed:", e);
    return null;
  }
}

export async function synthesizeMediaShareOfVoice(
  packet: BusinessPacket,
  questionnaire: Questionnaire
): Promise<MediaShareOfVoice | null> {
  const apiKey = process.env.GEMINI_API_KEY;
  if (!apiKey) return null;
  ...
  const schema = `{
  "summary": "2-3 sentence overview of media share of voice and visibility",
  "channels": [{
    "channel": "Media channel (e.g., online news, social media, industry publications, podcasts)",
    "shareOfVoice": "Share of voice percentage in this channel",
    "mentions": 0,
    "reach": "Estimated reach or impressions",
    "sentiment": "Sentiment of coverage in this channel",
    "trend": "Trend direction for share of voice"
  }],
  "overallShare": "Overall share of voice across all channels",
  "topChannel": "Channel with highest share of voice",
  "competitorComparison": "How share of voice compares to key competitors",
  "viralMoments": "Notable viral or high-impact media moments",
  "recommendations": ["recommendation 1", "..."]
}`;

  const prompt = `You are a media analytics strategist specializing in share of voice measurement, media monitoring, competitive visibility analysis, and PR effectiveness tracking.
...
Produce a comprehensive Media Share of Voice analysis:

1. SUMMARY: 2-3 sentence overview of media visibility and share of voice.
2. CHANNELS (5-7): For each media channel — channel name, share of voice %, mention count, reach, sentiment, and trend.
3. OVERALL SHARE: Overall share of voice across all media channels.
4. TOP CHANNEL: Media channel with the highest share of voice.
5. COMPETITOR COMPARISON: How share of voice compares to primary competitors.
...
${schema}`;
  ...
  try {
    console.log("[Pivot] Generating Media Share of Voice (Wave 50)...");
    const result = await callJson(genai, prompt);
    return result as unknown as MediaShareOfVoice;
  } catch (e) {
    console.warn("[Pivot] Media Share of Voice (Wave 50) synthesis failed:", e);
    return null;
  }
}

export async function synthesizeCrisisCommsReadiness(
  packet: BusinessPacket,
  questionnaire: Questionnaire
): Promise<CrisisCommsReadiness | null> {
  const apiKey = process.env.GEMINI_API_KEY;
  if (!apiKey) return null;
  ...
  const schema = `{
  "summary": "2-3 sentence overview of crisis communications readiness",
  "scenarios": [{
    "scenario": "Crisis scenario (e.g., data breach, product recall, executive scandal, financial downturn)",
    "likelihood": "Likelihood of occurrence",
    "impact": "Potential business impact",
    "responseTime": "Expected or target response time",
    "preparedness": "Current preparedness level for this scenario",
    "gap": "Key gap in readiness for this scenario"
  }],
  "overallReadiness": "Overall crisis communications readiness level",
  "weakestArea": "Area with the lowest crisis preparedness",
  "spokespersonReady": false,
  "playbook": "Status of crisis communications playbook",
  "recommendations": ["recommendation 1", "..."]
}`;

  const prompt = `You are a crisis communications consultant specializing in crisis preparedness assessment, communications playbook development, media training, and reputation risk management.
...
Produce a comprehensive Crisis Communications Readiness assessment:

1. SUMMARY: 2-3 sentence overview of crisis communications preparedness.
2. SCENARIOS (5-7): For each crisis scenario — scenario description, likelihood, impact, response time, preparedness level, and key gap.
3. OVERALL READINESS: Overall crisis communications readiness assessment.
4. WEAKEST AREA: Area with the lowest crisis preparedness.
5. SPOKESPERSON READY (boolean): Whether designated spokesperson(s) are trained and ready.
...
${schema}`;
  ...
  try {
    console.log("[Pivot] Generating Crisis Comms Readiness (Wave 50)...");
    const result = await callJson(genai, prompt);
    return result as unknown as CrisisCommsReadiness;
  } catch (e) {
    console.warn("[Pivot] Crisis Comms Readiness (Wave 50) synthesis failed:", e);
    return null;
  }
}

export async function synthesizeThoughtLeadership(
  packet: BusinessPacket,
  questionnaire: Questionnaire
): Promise<ThoughtLeadership | null> {
  const apiKey = process.env.GEMINI_API_KEY;
  if (!apiKey) return null;
  ...
  const schema = `{
  "summary": "2-3 sentence overview of thought leadership positioning and authority",
  "topics": [{
    "topic": "Thought leadership topic or theme",
    "authority": "Level of authority established in this topic",
    "contentVolume": "Volume of content produced on this topic",
    "engagement": "Engagement level for content on this topic",
    "competitorActivity": "Competitor activity level in this topic area",
    "opportunity": "Opportunity to strengthen leadership in this topic"
  }],
  "overallAuthority": "Overall thought leadership authority assessment",
  "topTopic": "Topic with strongest thought leadership position",
  "contentGaps": ["gap 1", "gap 2"],
  "audienceGrowth": "Thought leadership audience growth trend",
  "recommendations": ["recommendation 1", "..."]
}`;

  const prompt = `You are a thought leadership strategist specializing in content authority building, expert positioning, industry influence analysis, and executive visibility programs.
...
Produce a comprehensive Thought Leadership analysis:

1. SUMMARY: 2-3 sentence overview of thought leadership position and authority.
2. TOPICS (5-7): For each topic — topic name, authority level, content volume, engagement, competitor activity, and opportunity.
3. OVERALL AUTHORITY: Overall thought leadership authority assessment.
4. TOP TOPIC: Topic where the business has the strongest thought leadership position.
5. CONTENT GAPS (3-5): Topics or areas where thought leadership content is missing or insufficient.
...
${schema}`;
  ...
  try {
    console.log("[Pivot] Generating Thought Leadership (Wave 50)...");
    const result = await callJson(genai, prompt);
    return result as unknown as ThoughtLeadership;
  } catch (e) {
    console.warn("[Pivot] Thought Leadership (Wave 50) synthesis failed:", e);
    return null;
  }
}

export async function synthesizeBrandConsistency(
  packet: BusinessPacket,
  questionnaire: Questionnaire
): Promise<BrandConsistency | null> {
  const apiKey = process.env.GEMINI_API_KEY;
  if (!apiKey) return null;
  ...
  const schema = `{
  "summary": "2-3 sentence overview of brand consistency across touchpoints",
  "touchpoints": [{
    "touchpoint": "Brand touchpoint (e.g., website, social media, sales materials, packaging)",
    "consistency": "Consistency score or rating for this touchpoint",
    "quality": "Quality assessment of brand expression",
    "alignment": "Alignment with brand guidelines",
    "audience": "Primary audience for this touchpoint",
    "improvement": "Recommended improvement for this touchpoint"
  }],
  "overallConsistency": 0,
  "strongestTouchpoint": "Touchpoint with the highest brand consistency",
  "weakestTouchpoint": "Touchpoint with the lowest brand consistency",
  "guidelinesAdherence": "Overall adherence to brand guidelines",
  "recommendations": ["recommendation 1", "..."]
}`;

  const prompt = `You are a brand consistency auditor specializing in brand guideline compliance, multi-channel brand expression analysis, visual identity assessment, and brand experience optimization.
...
Produce a comprehensive Brand Consistency analysis:

1. SUMMARY: 2-3 sentence overview of brand consistency across all touchpoints.
2. TOUCHPOINTS (5-7): For each touchpoint — name, consistency rating, quality, alignment with guidelines, audience, and improvement recommendation.
3. OVERALL CONSISTENCY (numeric 0-100): Overall brand consistency score.
4. STRONGEST TOUCHPOINT: Touchpoint with the best brand consistency and expression.
5. WEAKEST TOUCHPOINT: Touchpoint with the worst brand consistency or misalignment.
...
${schema}`;
  ...
  try {
    console.log("[Pivot] Generating Brand Consistency (Wave 50)...");
    const result = await callJson(genai, prompt);
    return result as unknown as BrandConsistency;
  } catch (e) {
    console.warn("[Pivot] Brand Consistency (Wave 50) synthesis failed:", e);
    return null;
  }
}
...
export async function synthesizeMonetizationModel(
  packet: BusinessPacket,
  questionnaire: Questionnaire
): Promise<MonetizationModel | null> {
  const apiKey = process.env.GEMINI_API_KEY;
  if (!apiKey) return null;
  ...
  const schema = `{
  "summary": "2-3 sentence overview of monetization model and revenue streams",
  "streams": [{
    "stream": "Revenue stream name",
    "model": "Monetization model (e.g., subscription, transactional, advertising, licensing)",
    "revenue": "Revenue generated by this stream",
    "margin": "Profit margin for this stream",
    "growth": "Growth rate for this stream",
    "maturity": "Maturity stage (Early, Growing, Mature, Declining)"
  }],
  "primaryModel": "Primary monetization model",
  "diversificationScore": "Revenue diversification score or assessment",
  "ltv": "Average customer lifetime value across models",
  "revenuePerUser": "Average revenue per user",
  "recommendations": ["recommendation 1", "..."]
}`;

  const prompt = `You are a monetization strategist specializing in revenue model design, pricing architecture, value capture optimization, and business model innovation.
...
Produce a comprehensive Monetization Model analysis:

1. SUMMARY: 2-3 sentence overview of the monetization model and revenue architecture.
2. STREAMS (5-7): For each revenue stream — name, model type, revenue, margin, growth rate, and maturity stage.
3. PRIMARY MODEL: The primary monetization model driving the business.
4. DIVERSIFICATION SCORE: Assessment of revenue diversification and concentration risk.
5. LTV: Average customer lifetime value across all monetization models.
...
${schema}`;
  ...
  try {
    console.log("[Pivot] Generating Monetization Model (Wave 51)...");
    const result = await callJson(genai, prompt);
    return result as unknown as MonetizationModel;
  } catch (e) {
    console.warn("[Pivot] Monetization Model (Wave 51) synthesis failed:", e);
    return null;
  }
}

export async function synthesizeFreeTrialConversion(
  packet: BusinessPacket,
  questionnaire: Questionnaire
): Promise<FreeTrialConversion | null> {
  const apiKey = process.env.GEMINI_API_KEY;
  if (!apiKey) return null;
  ...
  const schema = `{
  "summary": "2-3 sentence overview of free trial conversion performance",
  "cohorts": [{
    "cohort": "Cohort identifier (e.g., month, source, plan type)",
    "trialStarts": 0,
    "conversions": 0,
    "conversionRate": "Conversion rate percentage",
    "avgTimeToConvert": "Average time from trial start to conversion",
    "topFeature": "Feature most correlated with conversion"
  }],
  "overallConversion": "Overall trial-to-paid conversion rate",
  "avgTrialLength": "Average trial duration",
  "topBarrier": "Primary barrier to conversion",
  "activationRate": "Percentage of trials that reach activation milestone",
  "recommendations": ["recommendation 1", "..."]
}`;

  const prompt = `You are a conversion optimization specialist focusing on free trial funnel analysis, activation metrics, trial-to-paid conversion strategy, and product-led growth.
...
Produce a comprehensive Free Trial Conversion analysis:

1. SUMMARY: 2-3 sentence overview of free trial conversion performance and trends.
2. COHORTS (5-7): For each cohort — identifier, trial starts, conversions, conversion rate, avg time to convert, and top converting feature.
3. OVERALL CONVERSION: Overall trial-to-paid conversion rate.
4. AVG TRIAL LENGTH: Average trial duration before conversion or expiration.
5. TOP BARRIER: Primary barrier preventing trial users from converting.
...
${schema}`;
  ...
  try {
    console.log("[Pivot] Generating Free Trial Conversion (Wave 51)...");
    const result = await callJson(genai, prompt);
    return result as unknown as FreeTrialConversion;
  } catch (e) {
    console.warn("[Pivot] Free Trial Conversion (Wave 51) synthesis failed:", e);
    return null;
  }
}

export async function synthesizeUsageBasedPricing(
  packet: BusinessPacket,
  questionnaire: Questionnaire
): Promise<UsageBasedPricing | null> {
  const apiKey = process.env.GEMINI_API_KEY;
  if (!apiKey) return null;
  ...
  const schema = `{
  "summary": "2-3 sentence overview of usage-based pricing model and performance",
  "tiers": [{
    "tier": "Usage tier name",
    "usageRange": "Usage range for this tier",
    "pricePerUnit": "Price per unit at this tier",
    "customers": 0,
    "revenue": "Revenue from this tier",
    "margin": "Profit margin at this tier"
  }],
  "avgUsage": "Average usage across all customers",
  "revenuePerUnit": "Average revenue per unit of usage",
  "overageRate": "Rate of customers exceeding their tier",
  "predictability": "Revenue predictability assessment",
  "recommendations": ["recommendation 1", "..."]
}`;

  const prompt = `You are a usage-based pricing expert specializing in consumption-based models, metering strategy, tier design, and value-metric optimization.
...
Produce a comprehensive Usage-Based Pricing analysis:

1. SUMMARY: 2-3 sentence overview of usage-based pricing model and effectiveness.
2. TIERS (5-7): For each tier — tier name, usage range, price per unit, customer count, revenue, and margin.
3. AVG USAGE: Average usage level across all customers.
4. REVENUE PER UNIT: Average revenue per unit of usage.
5. OVERAGE RATE: Percentage of customers exceeding their usage tier.
...
${schema}`;
  ...
  try {
    console.log("[Pivot] Generating Usage-Based Pricing (Wave 51)...");
    const result = await callJson(genai, prompt);
    return result as unknown as UsageBasedPricing;
  } catch (e) {
    console.warn("[Pivot] Usage-Based Pricing (Wave 51) synthesis failed:", e);
    return null;
  }
}

export async function synthesizeBundleOptimization(
  packet: BusinessPacket,
  questionnaire: Questionnaire
): Promise<BundleOptimization | null> {
  const apiKey = process.env.GEMINI_API_KEY;
  if (!apiKey) return null;
  ...
  const schema = `{
  "summary": "2-3 sentence overview of bundle strategy and optimization opportunities",
  "bundles": [{
    "bundle": "Bundle name",
    "products": "Products included in this bundle",
    "price": "Bundle price",
    "takeRate": "Customer adoption rate for this bundle",
    "margin": "Profit margin on this bundle",
    "cannibalRisk": "Risk of cannibalizing individual product sales"
  }],
  "topBundle": "Best-performing bundle by revenue or adoption",
  "avgBundleSize": "Average number of products per bundle",
  "uplift": "Revenue uplift from bundling vs. individual sales",
  "cannibalRate": "Overall cannibalization rate across bundles",
  "recommendations": ["recommendation 1", "..."]
}`;

  const prompt = `You are a bundle pricing strategist specializing in product bundling optimization, cross-sell packaging, value perception engineering, and cannibalization analysis.
...
Produce a comprehensive Bundle Optimization analysis:

1. SUMMARY: 2-3 sentence overview of bundling strategy effectiveness.
2. BUNDLES (5-7): For each bundle — name, products included, price, take rate, margin, and cannibalization risk.
3. TOP BUNDLE: Best-performing bundle by revenue contribution or adoption rate.
4. AVG BUNDLE SIZE: Average number of products per bundle.
5. UPLIFT: Revenue uplift achieved through bundling vs. individual product sales.
...
${schema}`;
  ...
  try {
    console.log("[Pivot] Generating Bundle Optimization (Wave 51)...");
    const result = await callJson(genai, prompt);
    return result as unknown as BundleOptimization;
  } catch (e) {
    console.warn("[Pivot] Bundle Optimization (Wave 51) synthesis failed:", e);
    return null;
  }
}

export async function synthesizeDiscountDiscipline(
  packet: BusinessPacket,
  questionnaire: Questionnaire
): Promise<DiscountDiscipline | null> {
  const apiKey = process.env.GEMINI_API_KEY;
  if (!apiKey) return null;
  ...
  const schema = `{
  "summary": "2-3 sentence overview of discount practices and revenue impact",
  "categories": [{
    "category": "Discount category (e.g., volume, promotional, loyalty, competitive, negotiated)",
    "avgDiscount": "Average discount percentage in this category",
    "frequency": "Frequency of discount usage",
    "revenueImpact": "Revenue impact of discounts in this category",
    "necessity": "Assessment of discount necessity",
    "recommendation": "Specific recommendation for this category"
  }],
  "avgDiscountRate": "Overall average discount rate",
  "totalLeakage": "Total revenue leakage from discounting",
  "approvalCompliance": "Discount approval process compliance rate",
  "topAbuse": "Most abused or overused discount category",
  "recommendations": ["recommendation 1", "..."]
}`;

  const prompt = `You are a pricing discipline consultant specializing in discount management, price waterfall analysis, approval process design, and revenue leakage prevention through pricing governance.
...
Produce a comprehensive Discount Discipline analysis:

1. SUMMARY: 2-3 sentence overview of discount practices and their revenue impact.
2. CATEGORIES (5-7): For each discount category — category name, average discount %, frequency, revenue impact, necessity assessment, and recommendation.
3. AVG DISCOUNT RATE: Overall average discount rate across the business.
4. TOTAL LEAKAGE: Total revenue leakage attributable to discounting practices.
5. APPROVAL COMPLIANCE: Compliance rate with discount approval processes.
...
${schema}`;
  ...
  try {
    console.log("[Pivot] Generating Discount Discipline (Wave 51)...");
    const result = await callJson(genai, prompt);
    return result as unknown as DiscountDiscipline;
  } catch (e) {
    console.warn("[Pivot] Discount Discipline (Wave 51) synthesis failed:", e);
    return null;
  }
}

export async function synthesizeRevenueLeakageDetection(
  packet: BusinessPacket,
  questionnaire: Questionnaire
): Promise<RevenueLeakageDetection | null> {
  const apiKey = process.env.GEMINI_API_KEY;
  if (!apiKey) return null;
  ...
  const schema = `{
  "summary": "2-3 sentence overview of revenue leakage sources and recovery potential",
  "sources": [{
    "source": "Leakage source (e.g., billing errors, unused entitlements, scope creep, underpricing)",
    "amount": "Estimated leakage amount",
    "frequency": "How often this leakage occurs",
    "rootCause": "Root cause of this leakage",
    "detectability": "How easy or difficult this leakage is to detect",
    "fix": "Recommended fix for this leakage source"
  }],
  "totalLeakage": "Total estimated revenue leakage",
  "topSource": "Largest single source of revenue leakage",
  "detectionRate": "Current rate of leakage detection",
  "recoveryPotential": "Total recoverable revenue",
  "recommendations": ["recommendation 1", "..."]
}`;

  const prompt = `You are a revenue assurance specialist focusing on revenue leakage detection, billing accuracy audits, entitlement management, and revenue recovery strategies.
...
Produce a comprehensive Revenue Leakage Detection analysis:

1. SUMMARY: 2-3 sentence overview of revenue leakage sources and recovery opportunities.
2. SOURCES (5-7): For each leakage source — source name, estimated amount, frequency, root cause, detectability, and recommended fix.
3. TOTAL LEAKAGE: Total estimated revenue leakage across all sources.
4. TOP SOURCE: Largest single source of revenue leakage.
5. DETECTION RATE: Current rate of leakage detection and monitoring effectiveness.
...
${schema}`;
  ...
  try {
    console.log("[Pivot] Generating Revenue Leakage Detection (Wave 51)...");
    const result = await callJson(genai, prompt);
    return result as unknown as RevenueLeakageDetection;
  } catch (e) {
    console.warn("[Pivot] Revenue Leakage Detection (Wave 51) synthesis failed:", e);
    return null;
  }
}
...
export async function synthesizeCustomerAcademy(
  packet: BusinessPacket,
  questionnaire: Questionnaire
): Promise<CustomerAcademy | null> {
  const apiKey = process.env.GEMINI_API_KEY;
  if (!apiKey) return null;
  ...
  const schema = `{
  "summary": "2-3 sentence overview of customer academy and education program",
  "courses": [{
    "course": "Course name",
    "enrollments": 0,
    "completionRate": "Course completion rate",
    "satisfaction": "Student satisfaction rating",
    "impact": "Business impact of course completion (e.g., retention, expansion)",
    "format": "Course format (e.g., video, interactive, live, self-paced)"
  }],
  "totalEnrollments": 0,
  "avgCompletion": "Average course completion rate",
  "topCourse": "Most popular or impactful course",
  "revenueImpact": "Revenue impact of the education program",
  "recommendations": ["recommendation 1", "..."]
}`;

  const prompt = `You are a customer education strategist specializing in learning program design, customer academy management, course effectiveness analysis, and education-driven retention.
...
Produce a comprehensive Customer Academy analysis:

1. SUMMARY: 2-3 sentence overview of customer academy program and effectiveness.
2. COURSES (5-7): For each course — name, enrollments, completion rate, satisfaction, business impact, and format.
3. TOTAL ENROLLMENTS (numeric): Total enrollments across all courses.
4. AVG COMPLETION: Average course completion rate.
5. TOP COURSE: Most popular or highest-impact course.
...
${schema}`;
  ...
  try {
    console.log("[Pivot] Generating Customer Academy (Wave 52)...");
    const result = await callJson(genai, prompt);
    return result as unknown as CustomerAcademy;
  } catch (e) {
    console.warn("[Pivot] Customer Academy (Wave 52) synthesis failed:", e);
    return null;
  }
}

export async function synthesizeContentEngagement(
  packet: BusinessPacket,
  questionnaire: Questionnaire
  ...
  const schema = `{
  "summary": "2-3 sentence overview of content engagement and performance",
  "pieces": [{
    "content": "Content piece name or title",
    "type": "Content type (e.g., blog, whitepaper, webinar, case study, video)",
    "views": "View count or reach",
    "engagement": "Engagement rate or metric",
    "conversion": "Conversion rate from this content",
    "freshness": "Content freshness (e.g., current, needs update, outdated)"
  }],
  "totalViews": "Total views across all content",
  "avgEngagement": "Average engagement rate across content library",
  "topContent": "Highest-performing content piece",
  "contentGaps": ["gap 1", "gap 2"],
  "recommendations": ["recommendation 1", "..."]
}`;

  const prompt = `You are a content strategy analyst specializing in content performance analytics, engagement optimization, content library management, and content-driven conversion.
...
Produce a comprehensive Content Engagement analysis:

1. SUMMARY: 2-3 sentence overview of content engagement performance and trends.
2. PIECES (5-7): For each content piece — title, type, views, engagement, conversion rate, and freshness.
3. TOTAL VIEWS: Total views or reach across all content.
4. AVG ENGAGEMENT: Average engagement rate across the content library.
5. TOP CONTENT: Highest-performing content piece by engagement or conversion.
...
${schema}`;
  ...
  try {
    console.log("[Pivot] Generating Content Engagement (Wave 52)...");
    const result = await callJson(genai, prompt);
    return result as unknown as ContentEngagement;
  } catch (e) {
    console.warn("[Pivot] Content Engagement (Wave 52) synthesis failed:", e);
    return null;
  }
}

export async function synthesizeCommunityHealth(
  packet: BusinessPacket,
  questionnaire: Questionnaire
  ...
  const schema = `{
  "summary": "2-3 sentence overview of community health and engagement",
  "metrics": [{
    "metric": "Community metric name (e.g., active members, posts per day, response time)",
    "value": "Current metric value",
    "trend": "Trend direction (improving, stable, declining)",
    "benchmark": "Industry benchmark for this metric",
    "driver": "Key driver of this metric",
    "action": "Recommended action to improve this metric"
  }],
  "totalMembers": "Total community members",
  "activeRate": "Percentage of members who are active",
  "sentimentScore": "Overall community sentiment score",
  "topContributor": "Characteristics of top community contributors",
  "recommendations": ["recommendation 1", "..."]
}`;

  const prompt = `You are a community management expert specializing in community health metrics, engagement optimization, community-led growth strategies, and user community development.
...
Produce a comprehensive Community Health analysis:

1. SUMMARY: 2-3 sentence overview of community health and engagement levels.
2. METRICS (5-7): For each metric — metric name, current value, trend, benchmark, key driver, and recommended action.
3. TOTAL MEMBERS: Total community membership size.
4. ACTIVE RATE: Percentage of members who are actively participating.
5. SENTIMENT SCORE: Overall community sentiment and satisfaction.
...
${schema}`;
  ...
  try {
    console.log("[Pivot] Generating Community Health (Wave 52)...");
    const result = await callJson(genai, prompt);
    return result as unknown as CommunityHealth;
  } catch (e) {
    console.warn("[Pivot] Community Health (Wave 52) synthesis failed:", e);
    return null;
  }
}

export async function synthesizeCertificationProgram(
  packet: BusinessPacket,
  questionnaire: Questionnaire
  ...
  const schema = `{
  "summary": "2-3 sentence overview of certification program performance",
  "levels": [{
    "level": "Certification level name (e.g., Beginner, Professional, Expert, Architect)",
    "enrolled": 0,
    "certified": 0,
    "passRate": "Pass rate for this certification level",
    "avgScore": "Average score for this certification level",
    "revenueImpact": "Revenue impact of certified customers at this level"
  }],
  "totalCertified": 0,
  "overallPassRate": "Overall pass rate across all levels",
  "topLevel": "Most popular certification level",
  "renewalRate": "Certification renewal rate",
  "recommendations": ["recommendation 1", "..."]
}`;

  const prompt = `You are a certification program strategist specializing in professional certification design, credentialing programs, skills validation frameworks, and certification-driven customer success.
...
Produce a comprehensive Certification Program analysis:

1. SUMMARY: 2-3 sentence overview of certification program performance and impact.
2. LEVELS (4-6): For each certification level — level name, enrolled count, certified count, pass rate, avg score, and revenue impact.
3. TOTAL CERTIFIED (numeric): Total number of certified individuals.
4. OVERALL PASS RATE: Overall pass rate across all certification levels.
5. TOP LEVEL: Most popular or highest-demand certification level.
...
${schema}`;
  ...
  try {
    console.log("[Pivot] Generating Certification Program (Wave 52)...");
    const result = await callJson(genai, prompt);
    return result as unknown as CertificationProgram;
  } catch (e) {
    console.warn("[Pivot] Certification Program (Wave 52) synthesis failed:", e);
    return null;
  }
}

export async function synthesizeSelfServiceAdoption(
  packet: BusinessPacket,
  questionnaire: Questionnaire
): Promise<SelfServiceAdoption | null> {
  const apiKey = process.env.GEMINI_API_KEY;
  if (!apiKey) return null;
  ...
  const schema = `{
  "summary": "2-3 sentence overview of self-service adoption and effectiveness",
  "channels": [{
    "channel": "Self-service channel (e.g., knowledge base, chatbot, FAQ, community forums, tutorials)",
    "usage": "Usage volume or frequency",
    "resolutionRate": "Issue resolution rate through this channel",
    "satisfaction": "User satisfaction with this channel",
    "costPerInteraction": "Cost per interaction for this channel",
    "deflectionRate": "Support ticket deflection rate from this channel"
  }],
  "overallAdoption": "Overall self-service adoption rate",
  "avgResolution": "Average resolution rate across self-service channels",
  "costSavings": "Total cost savings from self-service adoption",
  "topChannel": "Highest-performing self-service channel",
  "recommendations": ["recommendation 1", "..."]
}`;

  const prompt = `You are a self-service strategy consultant specializing in knowledge management, self-service platform optimization, support automation, and customer empowerment through digital self-service.
...
Produce a comprehensive Self-Service Adoption analysis:

1. SUMMARY: 2-3 sentence overview of self-service adoption and effectiveness.
2. CHANNELS (5-7): For each self-service channel — channel name, usage, resolution rate, satisfaction, cost per interaction, and deflection rate.
3. OVERALL ADOPTION: Overall self-service adoption rate across the customer base.
4. AVG RESOLUTION: Average issue resolution rate through self-service.
5. COST SAVINGS: Total cost savings achieved through self-service adoption.
...
${schema}`;
  ...
  try {
    console.log("[Pivot] Generating Self-Service Adoption (Wave 52)...");
    const result = await callJson(genai, prompt);
    return result as unknown as SelfServiceAdoption;
  } catch (e) {
    console.warn("[Pivot] Self-Service Adoption (Wave 52) synthesis failed:", e);
    return null;
  }
}

export async function synthesizeSupportDeflection(
  packet: BusinessPacket,
  questionnaire: Questionnaire
): Promise<SupportDeflection | null> {
  const apiKey = process.env.GEMINI_API_KEY;
  if (!apiKey) return null;
  ...
  const schema = `{
  "summary": "2-3 sentence overview of support deflection performance and savings",
  "categories": [{
    "category": "Support category (e.g., billing, technical, onboarding, feature requests, account management)",
    "totalTickets": "Total tickets in this category",
    "deflected": "Number of tickets successfully deflected",
    "deflectionRate": "Deflection rate for this category",
    "savings": "Cost savings from deflection in this category",
    "topArticle": "Top help article driving deflection in this category"
  }],
  "overallDeflection": "Overall support ticket deflection rate",
  "totalSavings": "Total cost savings from support deflection",
  "topCategory": "Category with the highest deflection rate",
  "improvementArea": "Category with the most room for deflection improvement",
  "recommendations": ["recommendation 1", "..."]
}`;

  const prompt = `You are a support deflection analyst specializing in ticket deflection strategy, knowledge base optimization, proactive support, and cost-efficient customer service scaling.
...
Produce a comprehensive Support Deflection analysis:

1. SUMMARY: 2-3 sentence overview of support deflection performance and impact.
2. CATEGORIES (5-7): For each support category — category name, total tickets, deflected count, deflection rate, savings, and top deflecting article.
3. OVERALL DEFLECTION: Overall support ticket deflection rate.
4. TOTAL SAVINGS: Total cost savings achieved through support deflection.
5. TOP CATEGORY: Support category with the highest deflection rate.
...
${schema}`;
  ...
  try {
    console.log("[Pivot] Generating Support Deflection (Wave 52)...");
    const result = await callJson(genai, prompt);
    return result as unknown as SupportDeflection;
  } catch (e) {
    console.warn("[Pivot] Support Deflection (Wave 52) synthesis failed:", e);
    return null;
  }
}
...
export async function synthesizeInvestorDeck(
  packet: BusinessPacket,
  questionnaire: Questionnaire
): Promise<InvestorDeck | null> {
  const apiKey = process.env.GEMINI_API_KEY;
  if (!apiKey) return null;
  ...
  const schema = `{
  "summary": "2-3 sentence overview of investor deck readiness and quality",
  "sections": [{
    "section": "Deck section name (e.g., Problem, Solution, Market, Traction, Team, Financials)",
    "status": "Current status of this section (e.g., Strong, Needs Work, Missing)",
    "strength": "Key strength of this section",
    "weakness": "Key weakness or gap in this section",
    "investorAppeal": "How appealing this section is to investors",
    "improvement": "Specific improvement recommendation"
  }],
  "overallReadiness": "Overall investor deck readiness assessment",
  "strongestSection": "Strongest section of the deck",
  "weakestSection": "Weakest section needing the most improvement",
  "narrativeScore": 0,
  "recommendations": ["recommendation 1", "..."]
}`;

  const prompt = `You are an investor relations strategist specializing in pitch deck creation, fundraising narratives, investor presentation design, and startup storytelling.
...
Produce a comprehensive Investor Deck analysis:

1. SUMMARY: 2-3 sentence overview of investor deck readiness and narrative quality.
2. SECTIONS (6-8): For each deck section — section name, status, strength, weakness, investor appeal, and improvement recommendation.
3. OVERALL READINESS: Overall investor deck readiness assessment.
4. STRONGEST SECTION: The most compelling section of the deck.
5. WEAKEST SECTION: The section needing the most improvement.
...
${schema}`;
  ...
  try {
    console.log("[Pivot] Generating Investor Deck (Wave 53)...");
    const result = await callJson(genai, prompt);
    return result as unknown as InvestorDeck;
  } catch (e) {
    console.warn("[Pivot] Investor Deck (Wave 53) synthesis failed:", e);
    return null;
  }
}

export async function synthesizeFundingTimeline(
  packet: BusinessPacket,
  questionnaire: Questionnaire
): Promise<FundingTimeline | null> {
  const apiKey = process.env.GEMINI_API_KEY;
  if (!apiKey) return null;
  ...
  const schema = `{
  "summary": "2-3 sentence overview of funding timeline and milestones",
  "milestones": [{
    "milestone": "Funding milestone name (e.g., Seed Close, Series A Prep, Due Diligence Complete)",
    "targetDate": "Target completion date for this milestone",
    "status": "Current status (e.g., Completed, In Progress, Upcoming, At Risk)",
    "amount": "Funding amount associated with this milestone",
    "dependency": "Key dependency or prerequisite for this milestone",
    "risk": "Primary risk to achieving this milestone"
  }],
  "nextRound": "Next funding round being targeted",
  "targetAmount": "Target raise amount for the next round",
  "runway": "Current runway based on cash position and burn rate",
  "readinessScore": 0,
  "recommendations": ["recommendation 1", "..."]
}`;

  const prompt = `You are a fundraising strategist specializing in funding timeline planning, milestone tracking, capital raise preparation, and investor readiness.
...
Produce a comprehensive Funding Timeline analysis:

1. SUMMARY: 2-3 sentence overview of funding timeline and milestone progress.
2. MILESTONES (5-7): For each milestone — name, target date, status, amount, dependency, and risk.
3. NEXT ROUND: The next funding round being targeted.
4. TARGET AMOUNT: Target raise amount for the next round.
5. RUNWAY: Current runway based on cash and burn rate.
...
${schema}`;
  ...
  try {
    console.log("[Pivot] Generating Funding Timeline (Wave 53)...");
    const result = await callJson(genai, prompt);
    return result as unknown as FundingTimeline;
  } catch (e) {
    console.warn("[Pivot] Funding Timeline (Wave 53) synthesis failed:", e);
    return null;
  }
}

export async function synthesizeValuationModel(
  packet: BusinessPacket,
  questionnaire: Questionnaire
  ...
  const schema = `{
  "summary": "2-3 sentence overview of valuation model and key drivers",
  "drivers": [{
    "driver": "Valuation driver (e.g., Revenue Growth, Market Size, Gross Margin, Team Quality)",
    "impact": "Impact on valuation (e.g., High, Medium, Low)",
    "currentValue": "Current value or metric for this driver",
    "benchmark": "Industry benchmark for this driver",
    "improvement": "How to improve this driver",
    "confidence": "Confidence level in this assessment"
  }],
  "estimatedRange": "Estimated valuation range",
  "methodology": "Valuation methodology used (e.g., DCF, Revenue Multiple, Comparable)",
  "multipleUsed": "Revenue or EBITDA multiple applied",
  "keyAssumption": "Most critical assumption underlying the valuation",
  "recommendations": ["recommendation 1", "..."]
}`;

  const prompt = `You are a valuation analyst specializing in startup and business valuation, financial modeling, comparable analysis, and investor-grade valuation frameworks.
...
Produce a comprehensive Valuation Model analysis:

1. SUMMARY: 2-3 sentence overview of the valuation model and methodology.
2. DRIVERS (5-7): For each valuation driver — driver name, impact, current value, benchmark, improvement path, and confidence.
3. ESTIMATED RANGE: Estimated valuation range based on available data.
4. METHODOLOGY: Valuation methodology used.
5. MULTIPLE USED: Revenue or EBITDA multiple applied.
...
${schema}`;
  ...
  try {
    console.log("[Pivot] Generating Valuation Model (Wave 53)...");
    const result = await callJson(genai, prompt);
    return result as unknown as ValuationModel;
  } catch (e) {
    console.warn("[Pivot] Valuation Model (Wave 53) synthesis failed:", e);
    return null;
  }
}

export async function synthesizeCapTableManagement(
  packet: BusinessPacket,
  questionnaire: Questionnaire
): Promise<CapTableManagement | null> {
  const apiKey = process.env.GEMINI_API_KEY;
  if (!apiKey) return null;
  ...
  const schema = `{
  "summary": "2-3 sentence overview of cap table health and management",
  "stakeholders": [{
    "stakeholder": "Stakeholder name or group (e.g., Founders, Seed Investors, Option Pool, Advisors)",
    "ownership": "Ownership percentage",
    "shareClass": "Share class (e.g., Common, Preferred Series A, Options)",
    "votingRights": "Voting rights description",
    "vestingStatus": "Vesting status (e.g., Fully Vested, 2yr cliff remaining)",
    "notes": "Additional notes or concerns"
  }],
  "totalShares": "Total shares outstanding",
  "dilutionRisk": "Dilution risk assessment for upcoming rounds",
  "optionPoolRemaining": "Remaining option pool percentage",
  "cleanlinessScore": 0,
  "recommendations": ["recommendation 1", "..."]
}`;

  const prompt = `You are a cap table management specialist focusing on equity structure, dilution analysis, shareholder management, option pool planning, and governance best practices.
...
Produce a comprehensive Cap Table Management analysis:

1. SUMMARY: 2-3 sentence overview of cap table structure and health.
2. STAKEHOLDERS (5-7): For each stakeholder — name/group, ownership, share class, voting rights, vesting status, and notes.
3. TOTAL SHARES: Total shares outstanding.
4. DILUTION RISK: Dilution risk assessment for upcoming funding rounds.
5. OPTION POOL REMAINING: Remaining option pool as a percentage.
...
${schema}`;
  ...
  try {
    console.log("[Pivot] Generating Cap Table Management (Wave 53)...");
    const result = await callJson(genai, prompt);
    return result as unknown as CapTableManagement;
  } catch (e) {
    console.warn("[Pivot] Cap Table Management (Wave 53) synthesis failed:", e);
    return null;
  }
}

export async function synthesizeInvestorCommunication(
  packet: BusinessPacket,
  questionnaire: Questionnaire
): Promise<InvestorCommunication | null> {
  const apiKey = process.env.GEMINI_API_KEY;
  if (!apiKey) return null;
  ...
  const schema = `{
  "summary": "2-3 sentence overview of investor communication effectiveness",
  "updates": [{
    "topic": "Communication topic (e.g., Monthly Financials, Product Milestones, Fundraise Updates)",
    "frequency": "Communication frequency (e.g., Monthly, Quarterly, Ad-hoc)",
    "format": "Format used (e.g., Email, Deck, Dashboard, Video Call)",
    "engagement": "Investor engagement level with this communication",
    "effectiveness": "Effectiveness rating of this communication type",
    "improvement": "Specific improvement recommendation"
  }],
  "overallTransparency": "Overall transparency assessment",
  "responseTime": "Average response time to investor inquiries",
  "reportingCadence": "Current reporting cadence",
  "investorSatisfaction": "Overall investor satisfaction with communications",
  "recommendations": ["recommendation 1", "..."]
}`;

  const prompt = `You are an investor communications specialist focusing on stakeholder reporting, transparency best practices, investor update strategies, and board communication frameworks.
...
Produce a comprehensive Investor Communication analysis:

1. SUMMARY: 2-3 sentence overview of investor communication strategy and effectiveness.
2. UPDATES (5-7): For each communication type — topic, frequency, format, engagement, effectiveness, and improvement.
3. OVERALL TRANSPARENCY: Overall transparency assessment.
4. RESPONSE TIME: Average response time to investor inquiries.
5. REPORTING CADENCE: Current reporting cadence.
...
${schema}`;
  ...
  try {
    console.log("[Pivot] Generating Investor Communication (Wave 53)...");
    const result = await callJson(genai, prompt);
    return result as unknown as InvestorCommunication;
  } catch (e) {
    console.warn("[Pivot] Investor Communication (Wave 53) synthesis failed:", e);
    return null;
  }
}

export async function synthesizeBoardReporting(
  packet: BusinessPacket,
  questionnaire: Questionnaire
): Promise<BoardReporting | null> {
  const apiKey = process.env.GEMINI_API_KEY;
  if (!apiKey) return null;
  ...
  const schema = `{
  "summary": "2-3 sentence overview of board reporting quality and governance",
  "metrics": [{
    "metric": "Board metric (e.g., MRR, Burn Rate, Runway, CAC, NPS, Headcount)",
    "currentValue": "Current value of this metric",
    "target": "Target value for this metric",
    "status": "Status vs. target (e.g., On Track, Behind, Exceeded)",
    "trend": "Trend direction (e.g., Improving, Declining, Stable)",
    "actionItem": "Specific action item for this metric"
  }],
  "reportingFrequency": "Current board reporting frequency",
  "dashboardReadiness": "Readiness of real-time dashboard for the board",
  "keyDecisionsPending": 0,
  "governanceScore": 0,
  "recommendations": ["recommendation 1", "..."]
}`;

  const prompt = `You are a board governance consultant specializing in board reporting frameworks, KPI dashboards, governance best practices, and executive communication with board members.
...
Produce a comprehensive Board Reporting analysis:

1. SUMMARY: 2-3 sentence overview of board reporting quality and governance maturity.
2. METRICS (6-8): For each board metric — metric name, current value, target, status, trend, and action item.
3. REPORTING FREQUENCY: Current board reporting frequency.
4. DASHBOARD READINESS: Readiness of real-time dashboards for the board.
5. KEY DECISIONS PENDING (numeric): Number of key decisions pending board approval.
...
${schema}`;
  ...
  try {
    console.log("[Pivot] Generating Board Reporting (Wave 53)...");
    const result = await callJson(genai, prompt);
    return result as unknown as BoardReporting;
  } catch (e) {
    console.warn("[Pivot] Board Reporting (Wave 53) synthesis failed:", e);
    return null;
  }
}
...
export async function synthesizeGeoExpansionStrategy(
  packet: BusinessPacket,
  questionnaire: Questionnaire
): Promise<GeoExpansionStrategy | null> {
  const apiKey = process.env.GEMINI_API_KEY;
  if (!apiKey) return null;
  ...
  const schema = `{
  "summary": "2-3 sentence overview of geographic expansion strategy",
  "markets": [{
    "market": "Target market or region (e.g., EU, Southeast Asia, Latin America)",
    "tam": "Total addressable market size in this geography",
    "competition": "Competitive landscape assessment for this market",
    "readiness": "Company readiness to enter this market",
    "investmentRequired": "Estimated investment required for market entry",
    "timeline": "Estimated timeline to enter and establish presence"
  }],
  "topMarket": "Highest-priority market for expansion",
  "totalTam": "Combined TAM across all target markets",
  "expansionMode": "Recommended expansion mode (e.g., Direct, Partnership, Acquisition)",
  "riskLevel": "Overall risk level of the expansion strategy",
  "recommendations": ["recommendation 1", "..."]
}`;

  const prompt = `You are a geographic expansion strategist specializing in international market entry, global growth planning, market prioritization, and cross-border business strategy.
...
Produce a comprehensive Geographic Expansion Strategy analysis:

1. SUMMARY: 2-3 sentence overview of the geographic expansion opportunity.
2. MARKETS (5-7): For each target market — market name, TAM, competition, readiness, investment required, and timeline.
3. TOP MARKET: Highest-priority market for expansion.
4. TOTAL TAM: Combined TAM across all identified target markets.
5. EXPANSION MODE: Recommended expansion mode (Direct, Partnership, Acquisition, etc.).
...
${schema}`;
  ...
  try {
    console.log("[Pivot] Generating Geo Expansion Strategy (Wave 54)...");
    const result = await callJson(genai, prompt);
    return result as unknown as GeoExpansionStrategy;
  } catch (e) {
    console.warn("[Pivot] Geo Expansion Strategy (Wave 54) synthesis failed:", e);
    return null;
  }
}

export async function synthesizeLocalMarketEntry(
  packet: BusinessPacket,
  questionnaire: Questionnaire
): Promise<LocalMarketEntry | null> {
  const apiKey = process.env.GEMINI_API_KEY;
  if (!apiKey) return null;
  ...
  const schema = `{
  "summary": "2-3 sentence overview of local market entry barriers and strategy",
  "barriers": [{
    "barrier": "Entry barrier (e.g., Regulatory Licensing, Language, Payment Infrastructure)",
    "severity": "Severity of this barrier (e.g., High, Medium, Low)",
    "mitigationStrategy": "Strategy to overcome this barrier",
    "cost": "Estimated cost to mitigate this barrier",
    "timeline": "Timeline to address this barrier",
    "precedent": "Industry precedent or comparable company that overcame this barrier"
  }],
  "entryMode": "Recommended entry mode (e.g., JV, Subsidiary, Franchise, Direct)",
  "estimatedCost": "Total estimated cost for local market entry",
  "breakEvenTimeline": "Estimated time to break even in the new market",
  "localPartnerNeed": "Assessment of need for a local partner",
  "recommendations": ["recommendation 1", "..."]
}`;

  const prompt = `You are a local market entry consultant specializing in market entry barriers, regulatory navigation, localization strategies, and cross-border operational planning.
...
Produce a comprehensive Local Market Entry analysis:

1. SUMMARY: 2-3 sentence overview of entry barriers and recommended approach.
2. BARRIERS (5-7): For each barrier — name, severity, mitigation strategy, cost, timeline, and precedent.
3. ENTRY MODE: Recommended entry mode (JV, Subsidiary, Franchise, Direct, etc.).
4. ESTIMATED COST: Total estimated cost for local market entry.
5. BREAK-EVEN TIMELINE: Estimated time to break even in the new market.
...
${schema}`;
  ...
  try {
    console.log("[Pivot] Generating Local Market Entry (Wave 54)...");
    const result = await callJson(genai, prompt);
    return result as unknown as LocalMarketEntry;
  } catch (e) {
    console.warn("[Pivot] Local Market Entry (Wave 54) synthesis failed:", e);
    return null;
  }
}

export async function synthesizeMarketRegulations(
  packet: BusinessPacket,
  questionnaire: Questionnaire
): Promise<MarketRegulations | null> {
  const apiKey = process.env.GEMINI_API_KEY;
  if (!apiKey) return null;
  ...
  const schema = `{
  "summary": "2-3 sentence overview of market regulations and compliance landscape",
  "areas": [{
    "area": "Regulation area (e.g., Data Privacy, Labor Law, Tax Compliance, Licensing)",
    "requirement": "Specific regulatory requirement",
    "complianceStatus": "Current compliance status (e.g., Compliant, Partial, Non-Compliant)",
    "risk": "Risk level if non-compliant",
    "cost": "Estimated compliance cost",
    "deadline": "Compliance deadline or urgency"
  }],
  "overallCompliance": "Overall compliance posture assessment",
  "criticalGaps": 0,
  "estimatedComplianceCost": "Total estimated cost to achieve full compliance",
  "highestRisk": "Regulation area posing the highest risk",
  "recommendations": ["recommendation 1", "..."]
}`;

  const prompt = `You are a regulatory compliance analyst specializing in market regulations, cross-border compliance, licensing requirements, and regulatory risk assessment for expanding businesses.
...
Produce a comprehensive Market Regulations analysis:

1. SUMMARY: 2-3 sentence overview of regulatory landscape and compliance posture.
2. AREAS (5-7): For each regulation area — area name, requirement, compliance status, risk, cost, and deadline.
3. OVERALL COMPLIANCE: Overall compliance posture assessment.
4. CRITICAL GAPS (numeric): Number of critical compliance gaps.
5. ESTIMATED COMPLIANCE COST: Total estimated cost to achieve full compliance.
...
${schema}`;
  ...
  try {
    console.log("[Pivot] Generating Market Regulations (Wave 54)...");
    const result = await callJson(genai, prompt);
    return result as unknown as MarketRegulations;
  } catch (e) {
    console.warn("[Pivot] Market Regulations (Wave 54) synthesis failed:", e);
    return null;
  }
}

export async function synthesizePartnerLocalization(
  packet: BusinessPacket,
  questionnaire: Questionnaire
): Promise<PartnerLocalization | null> {
  const apiKey = process.env.GEMINI_API_KEY;
  if (!apiKey) return null;
  ...
  const schema = `{
  "summary": "2-3 sentence overview of partner and product localization readiness",
  "areas": [{
    "area": "Localization area (e.g., Language, Payments, Legal, UX, Marketing, Support)",
    "currentState": "Current localization state",
    "targetState": "Target localization state",
    "effort": "Estimated effort to localize (e.g., High, Medium, Low)",
    "cost": "Estimated cost for localization in this area",
    "priority": "Priority level (e.g., Critical, High, Medium, Low)"
  }],
  "overallReadiness": "Overall localization readiness assessment",
  "topPriority": "Highest-priority localization area",
  "totalInvestment": "Total estimated investment for full localization",
  "timeToMarket": "Estimated time to achieve market-ready localization",
  "recommendations": ["recommendation 1", "..."]
}`;

  const prompt = `You are a localization strategist specializing in product localization, partner localization, language adaptation, payment localization, and market-specific customization for global expansion.
...
Produce a comprehensive Partner Localization analysis:

1. SUMMARY: 2-3 sentence overview of localization readiness and priorities.
2. AREAS (5-7): For each localization area — area name, current state, target state, effort, cost, and priority.
3. OVERALL READINESS: Overall localization readiness assessment.
4. TOP PRIORITY: Highest-priority localization area.
5. TOTAL INVESTMENT: Total estimated investment for full localization.
...
${schema}`;
  ...
  try {
    console.log("[Pivot] Generating Partner Localization (Wave 54)...");
    const result = await callJson(genai, prompt);
    return result as unknown as PartnerLocalization;
  } catch (e) {
    console.warn("[Pivot] Partner Localization (Wave 54) synthesis failed:", e);
    return null;
  }
}

export async function synthesizeCulturalAdaptation(
  packet: BusinessPacket,
  questionnaire: Questionnaire
  ...
  const schema = `{
  "summary": "2-3 sentence overview of cultural adaptation needs for expansion",
  "factors": [{
    "factor": "Cultural factor (e.g., Communication Style, Decision-Making, Work Ethic, Negotiation)",
    "homeMarket": "How this factor manifests in the home market",
    "targetMarket": "How this factor differs in the target market",
    "gap": "Size of the cultural gap (e.g., Large, Moderate, Small)",
    "risk": "Risk if this gap is not addressed",
    "adaptation": "Specific adaptation strategy"
  }],
  "overallReadiness": "Overall cultural adaptation readiness",
  "biggestGap": "Cultural factor with the largest gap",
  "trainingNeed": "Assessment of cultural training needs for the team",
  "adaptationTimeline": "Estimated timeline for cultural adaptation",
  "recommendations": ["recommendation 1", "..."]
}`;

  const prompt = `You are a cultural adaptation consultant specializing in cross-cultural business strategy, cultural intelligence, international team management, and market-specific cultural adaptation.
...
Produce a comprehensive Cultural Adaptation analysis:

1. SUMMARY: 2-3 sentence overview of cultural adaptation needs and readiness.
2. FACTORS (5-7): For each cultural factor — factor name, home market behavior, target market behavior, gap size, risk, and adaptation strategy.
3. OVERALL READINESS: Overall cultural adaptation readiness.
4. BIGGEST GAP: Cultural factor with the largest gap to address.
5. TRAINING NEED: Assessment of cultural training needs for the team.
...
${schema}`;
  ...
  try {
    console.log("[Pivot] Generating Cultural Adaptation (Wave 54)...");
    const result = await callJson(genai, prompt);
    return result as unknown as CulturalAdaptation;
  } catch (e) {
    console.warn("[Pivot] Cultural Adaptation (Wave 54) synthesis failed:", e);
    return null;
  }
}

export async function synthesizeExpansionRoi(
  packet: BusinessPacket,
  questionnaire: Questionnaire
): Promise<ExpansionRoi | null> {
  const apiKey = process.env.GEMINI_API_KEY;
  if (!apiKey) return null;
  ...
  const schema = `{
  "summary": "2-3 sentence overview of expansion ROI projections",
  "costs": [{
    "category": "Cost category (e.g., Market Research, Hiring, Legal, Marketing, Infrastructure)",
    "investment": "Investment amount for this category",
    "expectedReturn": "Expected return from this investment",
    "paybackPeriod": "Payback period for this cost category",
    "risk": "Risk level (e.g., High, Medium, Low)",
    "confidence": "Confidence level in the projection"
  }],
  "totalInvestment": "Total investment required for expansion",
  "projectedRoi": "Projected ROI percentage",
  "breakEven": "Estimated break-even timeline",
  "irr": "Internal rate of return for the expansion",
  "recommendations": ["recommendation 1", "..."]
}`;

  const prompt = `You are an expansion ROI analyst specializing in market expansion financial modeling, investment analysis, break-even projections, and return on expansion investment calculations.
...
Produce a comprehensive Expansion ROI analysis:

1. SUMMARY: 2-3 sentence overview of expansion ROI projections and investment thesis.
2. COSTS (5-7): For each cost category — category name, investment, expected return, payback period, risk, and confidence.
3. TOTAL INVESTMENT: Total investment required for expansion.
4. PROJECTED ROI: Projected return on investment percentage.
5. BREAK-EVEN: Estimated break-even timeline.
...
${schema}`;
  ...
  try {
    console.log("[Pivot] Generating Expansion ROI (Wave 54)...");
    const result = await callJson(genai, prompt);
    return result as unknown as ExpansionRoi;
  } catch (e) {
    console.warn("[Pivot] Expansion ROI (Wave 54) synthesis failed:", e);
    return null;
  }
}
...
export async function synthesizeProductLedMetrics(
  packet: BusinessPacket,
  questionnaire: Questionnaire
): Promise<ProductLedMetrics | null> {
  const apiKey = process.env.GEMINI_API_KEY;
  if (!apiKey) return null;
  ...
  const schema = `{
  "summary": "2-3 sentence overview of product-led growth metrics and performance",
  "metrics": [{
    "metric": "PLG metric name (e.g., Time to Value, Activation Rate, Expansion Revenue, Viral Coefficient)",
    "value": "Current value of this metric",
    "benchmark": "Industry benchmark for this metric",
    "trend": "Trend direction (e.g., Improving, Declining, Stable)",
    "impact": "Business impact of this metric",
    "action": "Specific action to improve this metric"
  }],
  "overallPlgScore": 0,
  "topMetric": "Strongest PLG metric",
  "biggestGap": "PLG metric with the largest gap to benchmark",
  "growthRate": "Overall product-led growth rate",
  "recommendations": ["recommendation 1", "..."]
}`;

  const prompt = `You are a product-led growth strategist specializing in PLG metrics, self-serve growth models, product-driven acquisition, and usage-based expansion strategies.
...
Produce a comprehensive Product-Led Metrics analysis:

1. SUMMARY: 2-3 sentence overview of PLG metrics and product-led growth performance.
2. METRICS (6-8): For each PLG metric — metric name, value, benchmark, trend, impact, and action.
3. OVERALL PLG SCORE (numeric 0-100): Overall product-led growth maturity score.
4. TOP METRIC: Strongest PLG metric.
5. BIGGEST GAP: PLG metric with the largest gap to benchmark.
...
${schema}`;
  ...
  try {
    console.log("[Pivot] Generating Product-Led Metrics (Wave 55)...");
    const result = await callJson(genai, prompt);
    return result as unknown as ProductLedMetrics;
  } catch (e) {
    console.warn("[Pivot] Product-Led Metrics (Wave 55) synthesis failed:", e);
    return null;
  }
}

export async function synthesizeActivationFunnel(
  packet: BusinessPacket,
  questionnaire: Questionnaire
  ...
  const schema = `{
  "summary": "2-3 sentence overview of activation funnel performance",
  "stages": [{
    "stage": "Funnel stage name (e.g., Signup, Onboarding, First Value, Habit, Conversion)",
    "users": "Number or percentage of users at this stage",
    "conversionRate": "Conversion rate from this stage to the next",
    "dropoffRate": "Drop-off rate at this stage",
    "avgTime": "Average time spent in this stage",
    "improvement": "Specific improvement to increase conversion at this stage"
  }],
  "overallConversion": "Overall signup-to-activation conversion rate",
  "biggestDropoff": "Stage with the highest drop-off rate",
  "activationRate": "Product activation rate",
  "medianTimeToActivate": "Median time from signup to activation",
  "recommendations": ["recommendation 1", "..."]
}`;

  const prompt = `You are an activation funnel analyst specializing in user onboarding optimization, activation metrics, conversion rate analysis, and product-led activation strategies.
...
Produce a comprehensive Activation Funnel analysis:

1. SUMMARY: 2-3 sentence overview of activation funnel performance and bottlenecks.
2. STAGES (5-7): For each funnel stage — stage name, users, conversion rate, drop-off rate, avg time, and improvement.
3. OVERALL CONVERSION: Overall signup-to-activation conversion rate.
4. BIGGEST DROPOFF: Stage with the highest drop-off rate.
5. ACTIVATION RATE: Product activation rate.
...
${schema}`;
  ...
  try {
    console.log("[Pivot] Generating Activation Funnel (Wave 55)...");
    const result = await callJson(genai, prompt);
    return result as unknown as ActivationFunnel;
  } catch (e) {
    console.warn("[Pivot] Activation Funnel (Wave 55) synthesis failed:", e);
    return null;
  }
}

export async function synthesizeFeatureAdoption(
  packet: BusinessPacket,
  questionnaire: Questionnaire
  ...
  const schema = `{
  "summary": "2-3 sentence overview of feature adoption across the product",
  "features": [{
    "feature": "Feature name",
    "adoption": "Adoption rate (percentage of users using this feature)",
    "frequency": "Usage frequency (e.g., Daily, Weekly, Monthly)",
    "satisfaction": "User satisfaction with this feature",
    "retention": "Retention impact of this feature",
    "revenue": "Revenue contribution or impact of this feature"
  }],
  "overallAdoption": "Overall feature adoption rate across the product",
  "topFeature": "Most adopted and impactful feature",
  "underusedFeature": "Most underused feature with high potential",
  "stickinessScore": 0,
  "recommendations": ["recommendation 1", "..."]
}`;

  const prompt = `You are a product analytics specialist focusing on feature adoption analysis, usage patterns, product stickiness, and feature-level engagement optimization.
...
Produce a comprehensive Feature Adoption analysis:

1. SUMMARY: 2-3 sentence overview of feature adoption patterns and product stickiness.
2. FEATURES (6-8): For each feature — name, adoption rate, frequency, satisfaction, retention impact, and revenue contribution.
3. OVERALL ADOPTION: Overall feature adoption rate across the product.
4. TOP FEATURE: Most adopted and impactful feature.
5. UNDERUSED FEATURE: Most underused feature with high potential.
...
${schema}`;
  ...
  try {
    console.log("[Pivot] Generating Feature Adoption (Wave 55)...");
    const result = await callJson(genai, prompt);
    return result as unknown as FeatureAdoption;
  } catch (e) {
    console.warn("[Pivot] Feature Adoption (Wave 55) synthesis failed:", e);
    return null;
  }
}

export async function synthesizeVirality(
  packet: BusinessPacket,
  questionnaire: Questionnaire
): Promise<Virality | null> {
  const apiKey = process.env.GEMINI_API_KEY;
  if (!apiKey) return null;
  ...
  const schema = `{
  "summary": "2-3 sentence overview of virality and organic growth mechanics",
  "loops": [{
    "loop": "Viral loop name (e.g., Invite Flow, Social Sharing, Collaboration, Referral Program)",
    "kFactor": "K-factor for this viral loop",
    "cycleTime": "Average cycle time for this loop",
    "inviteRate": "Invitation or sharing rate",
    "acceptRate": "Acceptance or conversion rate of invitations",
    "contribution": "Contribution of this loop to overall growth"
  }],
  "overallKFactor": "Overall viral coefficient (K-factor)",
  "viralCycleTime": "Average viral cycle time",
  "organicGrowthRate": "Organic growth rate from viral loops",
  "topLoop": "Highest-performing viral loop",
  "recommendations": ["recommendation 1", "..."]
}`;

  const prompt = `You are a virality and growth loops strategist specializing in viral mechanics, K-factor optimization, referral loop design, and organic growth amplification.
...
Produce a comprehensive Virality analysis:

1. SUMMARY: 2-3 sentence overview of virality mechanics and organic growth potential.
2. LOOPS (4-6): For each viral loop — loop name, K-factor, cycle time, invite rate, accept rate, and growth contribution.
3. OVERALL K-FACTOR: Overall viral coefficient.
4. VIRAL CYCLE TIME: Average viral cycle time.
5. ORGANIC GROWTH RATE: Organic growth rate from viral loops.
...
${schema}`;
  ...
  try {
    console.log("[Pivot] Generating Virality (Wave 55)...");
    const result = await callJson(genai, prompt);
    return result as unknown as Virality;
  } catch (e) {
    console.warn("[Pivot] Virality (Wave 55) synthesis failed:", e);
    return null;
  }
}

export async function synthesizeProductQualifiedLeads(
  packet: BusinessPacket,
  questionnaire: Questionnaire
): Promise<ProductQualifiedLeads | null> {
  const apiKey = process.env.GEMINI_API_KEY;
  if (!apiKey) return null;
  ...
  const schema = `{
  "summary": "2-3 sentence overview of product-qualified leads pipeline",
  "criteria": [{
    "criteria": "PQL criteria (e.g., Feature Usage Threshold, Time in Product, Collaboration Activity)",
    "weight": "Weight of this criteria in PQL scoring",
    "threshold": "Threshold value that qualifies a lead",
    "currentRate": "Current rate of users meeting this criteria",
    "conversionToSql": "Conversion rate from PQL to SQL for this criteria",
    "optimization": "How to optimize this criteria for better PQL identification"
  }],
  "totalPqls": "Total number of product-qualified leads",
  "pqlToSqlRate": "PQL to SQL conversion rate",
  "avgDealSize": "Average deal size from PQL-sourced deals",
  "topCriteria": "Most predictive PQL criteria",
  "recommendations": ["recommendation 1", "..."]
}`;

  const prompt = `You are a product-qualified leads strategist specializing in PQL identification, product-to-sales handoff optimization, usage-based lead scoring, and PLG pipeline management.
...
Produce a comprehensive Product Qualified Leads analysis:

1. SUMMARY: 2-3 sentence overview of PQL pipeline and product-to-sales handoff.
2. CRITERIA (5-7): For each PQL criteria — criteria name, weight, threshold, current rate, SQL conversion, and optimization.
3. TOTAL PQLS: Total number of product-qualified leads.
4. PQL TO SQL RATE: Conversion rate from PQL to SQL.
5. AVG DEAL SIZE: Average deal size from PQL-sourced opportunities.
...
${schema}`;
  ...
  try {
    console.log("[Pivot] Generating Product Qualified Leads (Wave 55)...");
    const result = await callJson(genai, prompt);
    return result as unknown as ProductQualifiedLeads;
  } catch (e) {
    console.warn("[Pivot] Product Qualified Leads (Wave 55) synthesis failed:", e);
    return null;
  }
}

export async function synthesizeTimeToValue(
  packet: BusinessPacket,
  questionnaire: Questionnaire
): Promise<TimeToValue | null> {
  const apiKey = process.env.GEMINI_API_KEY;
  if (!apiKey) return null;
  ...
  const schema = `{
  "summary": "2-3 sentence overview of time-to-value performance and optimization",
  "milestones": [{
    "milestone": "Value milestone (e.g., First Login, Setup Complete, First Output, Aha Moment, Habit Formed)",
    "timeToReach": "Average time for users to reach this milestone",
    "usersReaching": "Percentage of users that reach this milestone",
    "impact": "Impact on retention and expansion when this milestone is reached",
    "correlationToRetention": "Correlation between reaching this milestone and long-term retention",
    "optimization": "Specific optimization to help more users reach this milestone faster"
  }],
  "medianTtv": "Median time to value for the product",
  "topMilestone": "Most impactful value milestone",
  "biggestDelay": "Milestone with the biggest delay or drop-off",
  "retentionCorrelation": "Correlation between TTV and retention",
  "recommendations": ["recommendation 1", "..."]
}`;

  const prompt = `You are a time-to-value optimization specialist focusing on user onboarding velocity, value realization metrics, activation speed, and user success milestone analysis.
...
Produce a comprehensive Time to Value analysis:

1. SUMMARY: 2-3 sentence overview of time-to-value performance and opportunities.
2. MILESTONES (5-7): For each value milestone — name, time to reach, users reaching, impact, retention correlation, and optimization.
3. MEDIAN TTV: Median time to value for the product.
4. TOP MILESTONE: Most impactful value milestone.
5. BIGGEST DELAY: Milestone with the biggest delay or user drop-off.
...
${schema}`;
  ...
  try {
    console.log("[Pivot] Generating Time to Value (Wave 55)...");
    const result = await callJson(genai, prompt);
    return result as unknown as TimeToValue;
  } catch (e) {
    console.warn("[Pivot] Time to Value (Wave 55) synthesis failed:", e);
    return null;
  }
}
...
export async function synthesizeAiReadinessScore(
  packet: BusinessPacket,
  questionnaire: Questionnaire
): Promise<AiReadinessScore | null> {
  const apiKey = process.env.GEMINI_API_KEY;
  if (!apiKey) return null;
  ...
  const schema = `{
  "summary": "2-3 sentence overview of AI readiness across the organization",
  "dimensions": [{
    "dimension": "AI readiness dimension (e.g., Data Quality, Talent, Infrastructure, Strategy, Culture, Governance)",
    "readinessLevel": 0,
    "gap": "Gap description for this dimension",
    "investment": "Estimated investment needed to close the gap",
    "priority": "Priority level (e.g., Critical, High, Medium, Low)",
    "timeline": "Estimated timeline to reach target readiness"
  }],
  "overallScore": 0,
  "topStrength": "Strongest AI readiness dimension",
  "topGap": "Dimension with the largest readiness gap",
  "estimatedInvestment": "Total estimated investment to achieve AI readiness",
  "recommendations": ["recommendation 1", "..."]
}`;

  const prompt = `You are an AI readiness assessor specializing in enterprise AI maturity evaluation, data readiness, AI infrastructure assessment, and organizational AI adoption strategy.
...
Produce a comprehensive AI Readiness Score analysis:

1. SUMMARY: 2-3 sentence overview of AI readiness across the organization.
2. DIMENSIONS (6-8): For each dimension — dimension name, readiness level (0-100), gap description, investment needed, priority, and timeline.
3. OVERALL SCORE (numeric 0-100): Overall AI readiness score.
4. TOP STRENGTH: Strongest AI readiness dimension.
5. TOP GAP: Dimension with the largest readiness gap.
...
${schema}`;
  ...
  try {
    console.log("[Pivot] Generating AI Readiness Score (Wave 56)...");
    const result = await callJson(genai, prompt);
    return result as unknown as AiReadinessScore;
  } catch (e) {
    console.warn("[Pivot] AI Readiness Score (Wave 56) synthesis failed:", e);
    return null;
  }
}

export async function synthesizeMlUseCasePriority(
  packet: BusinessPacket,
  questionnaire: Questionnaire
): Promise<MlUseCasePriority | null> {
  const apiKey = process.env.GEMINI_API_KEY;
  if (!apiKey) return null;
  ...
  const schema = `{
  "summary": "2-3 sentence overview of ML use case prioritization",
  "useCases": [{
    "useCase": "ML use case name (e.g., Demand Forecasting, Churn Prediction, Fraud Detection)",
    "businessValue": "Expected business value (e.g., High, Medium, Low, or dollar estimate)",
    "feasibility": "Technical feasibility (e.g., High, Medium, Low)",
    "dataReadiness": "Data readiness for this use case",
    "estimatedRoi": "Estimated ROI from implementing this use case",
    "priority": "Priority ranking (e.g., P0, P1, P2, P3)"
  }],
  "topUseCase": "Highest-priority ML use case",
  "totalEstimatedValue": "Total estimated value across all use cases",
  "avgFeasibility": "Average feasibility across identified use cases",
  "quickWins": 0,
  "recommendations": ["recommendation 1", "..."]
}`;

  const prompt = `You are an ML strategy consultant specializing in machine learning use case identification, feasibility assessment, AI project prioritization, and business value estimation for ML initiatives.
...
Produce a comprehensive ML Use Case Priority analysis:

1. SUMMARY: 2-3 sentence overview of ML use case landscape and prioritization.
2. USE CASES (6-8): For each use case — name, business value, feasibility, data readiness, estimated ROI, and priority.
3. TOP USE CASE: Highest-priority ML use case to pursue first.
4. TOTAL ESTIMATED VALUE: Total estimated value across all identified use cases.
5. AVG FEASIBILITY: Average feasibility level across identified use cases.
...
${schema}`;
  ...
  try {
    console.log("[Pivot] Generating ML Use Case Priority (Wave 56)...");
    const result = await callJson(genai, prompt);
    return result as unknown as MlUseCasePriority;
  } catch (e) {
    console.warn("[Pivot] ML Use Case Priority (Wave 56) synthesis failed:", e);
    return null;
  }
}

export async function synthesizeDataInfrastructure(
  packet: BusinessPacket,
  questionnaire: Questionnaire
): Promise<DataInfrastructure | null> {
  const apiKey = process.env.GEMINI_API_KEY;
  if (!apiKey) return null;
  ...
  const schema = `{
  "summary": "2-3 sentence overview of data infrastructure readiness",
  "components": [{
    "component": "Infrastructure component (e.g., Data Warehouse, ETL Pipeline, Data Lake, ML Platform)",
    "currentState": "Current state of this component",
    "targetState": "Target state for AI/ML readiness",
    "gap": "Gap between current and target state",
    "cost": "Estimated cost to close the gap",
    "urgency": "Urgency level (e.g., Immediate, Short-term, Medium-term)"
  }],
  "overallReadiness": "Overall data infrastructure readiness assessment",
  "biggestGap": "Component with the largest gap to close",
  "totalInvestment": "Total estimated investment to modernize data infrastructure",
  "modernizationScore": 0,
  "recommendations": ["recommendation 1", "..."]
}`;

  const prompt = `You are a data infrastructure architect specializing in data platform modernization, data pipeline design, ML infrastructure, and enterprise data architecture for AI readiness.
...
Produce a comprehensive Data Infrastructure analysis:

1. SUMMARY: 2-3 sentence overview of data infrastructure readiness for AI/ML.
2. COMPONENTS (5-7): For each component — component name, current state, target state, gap, cost, and urgency.
3. OVERALL READINESS: Overall data infrastructure readiness assessment.
4. BIGGEST GAP: Component with the largest gap to close.
5. TOTAL INVESTMENT: Total estimated investment to modernize data infrastructure.
...
${schema}`;
  ...
  try {
    console.log("[Pivot] Generating Data Infrastructure (Wave 56)...");
    const result = await callJson(genai, prompt);
    return result as unknown as DataInfrastructure;
  } catch (e) {
    console.warn("[Pivot] Data Infrastructure (Wave 56) synthesis failed:", e);
    return null;
  }
}

export async function synthesizeAiTalentGap(
  packet: BusinessPacket,
  questionnaire: Questionnaire
): Promise<AiTalentGap | null> {
  const apiKey = process.env.GEMINI_API_KEY;
  if (!apiKey) return null;
  ...
  const schema = `{
  "summary": "2-3 sentence overview of AI talent gaps and hiring needs",
  "needs": [{
    "role": "AI/ML role (e.g., ML Engineer, Data Scientist, AI Product Manager, MLOps Engineer)",
    "current": 0,
    "needed": 0,
    "gap": 0,
    "urgency": "Urgency of filling this role (e.g., Immediate, Q1, Q2, Q3+)",
    "hiringDifficulty": "Hiring difficulty (e.g., Very Hard, Hard, Moderate, Easy)"
  }],
  "totalGap": 0,
  "criticalRoles": 0,
  "buildVsBuy": "Build vs buy recommendation for AI talent",
  "trainingPotential": "Potential to upskill existing team members",
  "recommendations": ["recommendation 1", "..."]
}`;

  const prompt = `You are an AI talent strategist specializing in AI/ML team building, talent gap analysis, AI hiring strategies, and build-vs-buy decisions for AI capabilities.
...
Produce a comprehensive AI Talent Gap analysis:

1. SUMMARY: 2-3 sentence overview of AI talent gaps and team readiness.
2. NEEDS (5-7): For each role — role title, current headcount, needed headcount, gap, urgency, and hiring difficulty.
3. TOTAL GAP (numeric): Total AI talent gap across all roles.
4. CRITICAL ROLES (numeric): Number of critical roles that must be filled immediately.
5. BUILD VS BUY: Recommendation on building internal AI talent vs outsourcing.
...
${schema}`;
  ...
  try {
    console.log("[Pivot] Generating AI Talent Gap (Wave 56)...");
    const result = await callJson(genai, prompt);
    return result as unknown as AiTalentGap;
  } catch (e) {
    console.warn("[Pivot] AI Talent Gap (Wave 56) synthesis failed:", e);
    return null;
  }
}

export async function synthesizeEthicalAiFramework(
  packet: BusinessPacket,
  questionnaire: Questionnaire
): Promise<EthicalAiFramework | null> {
  const apiKey = process.env.GEMINI_API_KEY;
  if (!apiKey) return null;
  ...
  const schema = `{
  "summary": "2-3 sentence overview of ethical AI framework maturity",
  "principles": [{
    "principle": "Ethical AI principle (e.g., Fairness, Transparency, Accountability, Privacy, Safety)",
    "implementation": "Current implementation status of this principle",
    "compliance": "Compliance level (e.g., Full, Partial, None)",
    "gap": "Gap in implementing this principle",
    "risk": "Risk if this principle is not adequately addressed",
    "action": "Specific action to strengthen this principle"
  }],
  "overallMaturity": "Overall ethical AI maturity assessment",
  "biggestRisk": "Ethical AI principle posing the biggest risk",
  "auditFrequency": "Current or recommended AI ethics audit frequency",
  "transparencyScore": 0,
  "recommendations": ["recommendation 1", "..."]
}`;

  const prompt = `You are an ethical AI consultant specializing in responsible AI frameworks, AI governance, bias detection, AI transparency, and regulatory compliance for AI systems.
...
Produce a comprehensive Ethical AI Framework analysis:

1. SUMMARY: 2-3 sentence overview of ethical AI framework maturity and gaps.
2. PRINCIPLES (5-7): For each principle — principle name, implementation status, compliance level, gap, risk, and action.
3. OVERALL MATURITY: Overall ethical AI maturity assessment.
4. BIGGEST RISK: Ethical AI principle posing the biggest risk.
5. AUDIT FREQUENCY: Current or recommended AI ethics audit frequency.
...
${schema}`;
  ...
  try {
    console.log("[Pivot] Generating Ethical AI Framework (Wave 56)...");
    const result = await callJson(genai, prompt);
    return result as unknown as EthicalAiFramework;
  } catch (e) {
    console.warn("[Pivot] Ethical AI Framework (Wave 56) synthesis failed:", e);
    return null;
  }
}

export async function synthesizeAiRoiProjection(
  packet: BusinessPacket,
  questionnaire: Questionnaire
): Promise<AiRoiProjection | null> {
  const apiKey = process.env.GEMINI_API_KEY;
  if (!apiKey) return null;
  ...
  const schema = `{
  "summary": "2-3 sentence overview of AI ROI projections across scenarios",
  "projections": [{
    "scenario": "AI investment scenario (e.g., Conservative, Moderate, Aggressive, Quick Win)",
    "investment": "Investment amount for this scenario",
    "expectedReturn": "Expected return from this scenario",
    "timeline": "Timeline to realize returns",
    "risk": "Risk level of this scenario",
    "confidence": "Confidence level in this projection"
  }],
  "totalInvestment": "Total recommended AI investment",
  "expectedRoi": "Expected ROI from AI investments",
  "paybackPeriod": "Average payback period for AI investments",
  "topScenario": "Highest-ROI AI investment scenario",
  "recommendations": ["recommendation 1", "..."]
}`;

  const prompt = `You are an AI investment analyst specializing in AI ROI modeling, technology investment analysis, AI cost-benefit projections, and business case development for AI initiatives.
...
Produce a comprehensive AI ROI Projection analysis:

1. SUMMARY: 2-3 sentence overview of AI ROI projections and investment thesis.
2. PROJECTIONS (4-6): For each scenario — scenario name, investment, expected return, timeline, risk, and confidence.
3. TOTAL INVESTMENT: Total recommended AI investment.
4. EXPECTED ROI: Expected ROI percentage from AI investments.
5. PAYBACK PERIOD: Average payback period for AI investments.
...
${schema}`;
  ...
  try {
    console.log("[Pivot] Generating AI ROI Projection (Wave 56)...");
    const result = await callJson(genai, prompt);
